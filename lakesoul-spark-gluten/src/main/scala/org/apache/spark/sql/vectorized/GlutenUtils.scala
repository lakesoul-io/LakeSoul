...
import org.apache.gluten.extension.columnar.heuristic.{
  FallbackNode,
  HeuristicTransform
}
...
object GlutenUtils extends Logging {
  ...
  lazy val isGlutenEnabled: Boolean =
    SparkContext.getActive.exists(
      _.getConf
        .get("spark.plugins", "")
        .contains("org.apache.gluten.GlutenPlugin")
    )

  /** All following methods should test isGlutenEnabled first because logics can
    * be different when gluten is enabled or not.
    */

  /** This cannot be lazy because gluten's allocator is associated with each of
    * Spark's context
    */
  ...
  def setArrowAllocator(io: NativeIOBase): Unit = {
    if (isGlutenEnabled) {
      val allocator = getGlutenAllocator.newChildAllocator(
        "gluten",
        32 * 1024 * 1024,
        Long.MaxValue
      )
      ...
      io.setExternalAllocator(new BufferAllocator {
        ...
        override def buffer(l: Long, bufferManager: BufferManager): ArrowBuf =
          allocator.buffer(l, bufferManager)
        override def getRoot: BufferAllocator = allocator.getRoot
        override def newChildAllocator(
            s: String,
            l: Long,
            l1: Long
        ): BufferAllocator = allocator.newChildAllocator(s, l, l1)
        override def newChildAllocator(
            s: String,
          ...
          allocator.newChildAllocator(s, allocationListener, l, l1)
        ...
        override def getPeakMemoryAllocation: Long =
          allocator.getPeakMemoryAllocation
        override def getHeadroom: Long = allocator.getHeadroom
        override def forceAllocate(l: Long): Boolean =
          allocator.forceAllocate(l)
        ...
        override def getParentAllocator: BufferAllocator =
          allocator.getParentAllocator
        override def getChildAllocators: util.Collection[BufferAllocator] =
          allocator.getChildAllocators
        override def newReservation(): AllocationReservation =
          allocator.newReservation()
        ...
        override def getRoundingPolicy: RoundingPolicy =
          allocator.getRoundingPolicy
        override def wrapForeignAllocation(
            allocation: ForeignAllocation
        ): ArrowBuf = allocator.wrapForeignAllocation(allocation)
      })
    ...
    if (GlutenUtils.isGlutenEnabled) {
      val glutenPlan: SparkPlan = plan match {
        ...
        case _ =>
          ...
          val nativePlan =
            BackendTransitions.insert(nativePlan1, outputsColumnar = true)
          nativePlan match {
            case plan1: ValidatablePlan if (plan1.doValidate().ok()) =>
              if (plan1.isInstanceOf[WholeStageTransformer]) plan1
              else {
                ...
                WholeStageTransformer(plan2)(
                  ColumnarCollapseTransformStages
                    .getTransformStageCounter(plan1)
                    .incrementAndGet()
                )
              }
  ...
  def addLocalSortPlan(
    ...
    plan match {
      case aqe: AdaptiveSparkPlanExec =>
        ...
        sortPlan.setLogicalLink(
          Sort(orderingExpr, global = false, aqe.inputPlan.logicalLink.get)
        )
        AdaptiveSparkPlanExec(
          sortPlan,
          aqe.context,
      ...
      case _ =>
        if (GlutenUtils.isGlutenEnabled && supportTransform(plan)) {
          SortExecTransformer(
            orderingExpr,
            global = false,
  ...
  private def isSeparateBaseScanExecTransformer(plan: SparkPlan): Boolean =
    plan match {
  ...
  private def supportTransform(plan: SparkPlan): Boolean = plan match {
    case plan: TransformSupport if !isSeparateBaseScanExecTransformer(plan) =>
      true
    case _ => false
  }
    ...
    plan match {
      case t if supportTransform(t) =>
        WholeStageTransformer(
          t.withNewChildren(t.children.map(insertInputIteratorTransformer))
        )(
          ColumnarCollapseTransformStages
            .getTransformStageCounter(t)
            .incrementAndGet()
        )
      case other =>
