...
import org.apache.gluten.execution.{
  ColumnarToColumnarExec,
  GlutenColumnarToColumnarTransition
}
...
case class OffloadToVeloxExec(override val child: SparkPlan)
  ...
  override protected def mapIterator(
      in: Iterator[ColumnarBatch]
  ): Iterator[ColumnarBatch] = {
    in.map(b =>
      VeloxColumnarBatches.toVeloxBatch(
        ColumnarBatches.offload(ArrowBufferAllocators.contextInstance, b)
      )
    )
  }
