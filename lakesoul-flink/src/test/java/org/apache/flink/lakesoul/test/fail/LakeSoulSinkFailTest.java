...
public class LakeSoulSinkFailTest extends AbstractTestBase {
    ...
    private static final LakeSoulCatalog lakeSoulCatalog =
            LakeSoulTestUtils.createLakeSoulCatalog(true);
    public static Map<String, Tuple3<ResolvedSchema, String, MockTableSource.StopBehavior>>
            parameters;
    static String dropSourceSql = "drop table if exists test_source";
    static String createSourceSqlFormat =
            "create table if not exists test_source %s "
                    + "with ('connector'='lakesoul', 'path'='/', %s "
                    + "'discoveryinterval'='1000'"
                    + ")";
    static String dropSinkSql = "drop table if exists test_sink";
    static String createSinkSqlFormat =
            "create table if not exists test_sink %s %s"
    ...
    public static void setup() {
        ...
        parameters.put(
                "testLakeSoulSinkFailOnCheckpointing",
                Tuple3.of(
                        new ResolvedSchema(
                                Arrays.asList(
        ...
        parameters.put(
                "testLakeSoulSinkStopPostgresOnCheckpointing",
                Tuple3.of(
                        new ResolvedSchema(
                                Arrays.asList(
                                        Column.physical("hash", DataTypes.INT()),
        ...
        parameters.put(
                "testLakeSoulSinkFailOnCollectFinished",
                Tuple3.of(
                        new ResolvedSchema(
                                Arrays.asList(
                                        Column.physical("hash", DataTypes.INT().notNull()),
        ...
        parameters.put(
                "testLakeSoulSinkFailOnAssignSplitFinished",
                Tuple3.of(
                        new ResolvedSchema(
                                Arrays.asList(
                        ...
                        "PARTITIONED BY (`range`)",
                        MockTableSource.StopBehavior.FAIL_ON_ASSIGN_SPLIT_FINISHED));

        parameters.put(
                "testLakeSoulSinkFailOnBeforeAssignSplit",
                Tuple3.of(
                        new ResolvedSchema(
                                Arrays.asList(
                                        Column.physical("hash1", DataTypes.INT()),
                                ...
                                UniqueConstraint.primaryKey(
                                        "primary key", Arrays.asList("hash1", "hash2"))),
                        "PARTITIONED BY (`range1`, `range2`)",
                        MockTableSource.StopBehavior.FAIL_ON_BEFORE_ASSIGN_SPLIT));

        parameters.put(
                "testLakeSoulSinkWithoutPkStopPostgresOnCheckpointing",
                Tuple3.of(
                        new ResolvedSchema(
                                Arrays.asList(
                                        Column.physical("hash", DataTypes.INT()),
    ...
    public static Object generateObjectWithIndexByDatatype(Integer index, RowType.RowField field) {
        ...
        switch (field.getType().getTypeRoot().name().toLowerCase()) {
            ...
            case "varchar":
                return value % 2 == 0
                        ? StringData.fromString(String.format("'%d$", value))
                ...
                return DateTimeFormatter.ofPattern("yyyy-MM-dd  HH:mm:ss ")
                        .format(
                                LocalDateTime.ofInstant(
                                        Instant.ofEpochMilli((long) value * 3600 * 24 * 1000),
                                        ZoneId.of("UTC")))
                        .replace("  ", "T")
                        .replace(" ", "Z");
            case "double":
    ...
    public void testLakeSoulSinkFailOnCheckpointing() throws IOException {
        ...
        Tuple3<ResolvedSchema, String, MockTableSource.StopBehavior> tuple3 =
                parameters.get(testName);
        ResolvedSchema resolvedSchema = tuple3.f0;

        indexBound = (20 + new Random().nextInt(20));
        List<String> expectedData =
                IntStream.range(0, indexBound)
                        .boxed()
                        .map(
                                i ->
        ...
        List<String> actualData =
                CollectionUtil.iteratorToList(
                                batchEnv.executeSql("SELECT * FROM test_sink").collect())
                        .stream()
                        .map(Row::toString)
    ...
    private void testLakeSoulSink(
            ResolvedSchema resolvedSchema,
            MockTableSource.StopBehavior behavior,
            String partitionBy,
            String path,
            int timeout)
            throws IOException {
        ...
        streamTableEnv.executeSql(
                String.format(
                        createSourceSqlFormat,
                        resolvedSchema,
                        resolvedSchema.getPrimaryKey().isPresent() ? "'hashBucketNum'='2'," : ""));
    ...
    public void testMockTableSource() throws IOException {
        ...
        streamTableEnv.executeSql(
                "DROP TABLE IF EXISTS default_catalog.default_database.test_sink");
        streamTableEnv.executeSql(
                "CREATE TABLE default_catalog.default_database.test_sink "
                        + resolvedSchema
                        + " WITH ("
        ...
        TableResult tableResult =
                streamTableEnv.executeSql(
                        "insert into default_catalog.default_database.test_sink select * from"
                                + " test_source");
        try {
