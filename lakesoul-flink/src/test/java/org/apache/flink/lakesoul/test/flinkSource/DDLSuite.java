...
public class DDLSuite extends AbstractTestBase {
    ...
    public void dropTablePartition() throws ExecutionException, InterruptedException {
        ...
        tEnv.executeSql(
                ...
                .await();
        List<Row> results =
                CollectionUtil.iteratorToList(tEnv.executeSql("select * from user_info").collect());
    ...
    public void timeTypeTest() throws ExecutionException, InterruptedException {
        ...
        streamTableEnv
                .executeSql(
                        "INSERT INTO time_test_table VALUES "
                                + "("
                                + "time '10:10:10.999', "
                                + "time '10:10:11.999', "
                                + "time '10:10:12.999', "
        ...
        TestUtils.checkEqualInAnyOrder(
                results, new String[] {"+I[10:10:10, 10:10:11, 10:10:12, 10:10:13]"});
        ...
        Assert.assertThrows(
                ...
                () -> {
                    ...
                    streamTableEnv.executeSql(
                            "create table test_table (id int, primary key(id) not enforced)"
                                    + " partitioned by (id) with ('connector'='lakesoul',"
                                    + "'hashBucketNum'='2')");
                });
    }
