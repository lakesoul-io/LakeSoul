...
public class ReadWithTableAPI extends AbstractTestBase {
    ...
    @Test
    public void testLakesoulSourceViewSelectWhere()
        ...
        TestUtils.checkEqualInAnyOrder(
                results, new String[] {"+I[Bob, 90, 91]", "+I[Amy, 95, 96]"});
    }
    ...
    public void testLakesoulSourceSelectMultiRangeAndHash()
        ...
        Table filter =
                userInfo.filter($("region").isEqual("UK"))
                        .filter($("score").isGreater(80))
    ...
    @Test
    public void testLakesoulSourceSelectMultiRangeAndHash2()
            throws ExecutionException, InterruptedException {
        TableEnvironment createTableEnv = TestUtils.createTableEnv(BATCH_TYPE);
        ...
        List<Row> results = CollectionUtil.iteratorToList(filter.execute().collect());
        TestUtils.checkEqualInAnyOrder(
                results, new String[] {"+I[Amy, 95, 1990-10-10T10:10:10.100, UK]"});
    }
    ...
    public void testLakesoulSourceSelectJoin() throws ExecutionException, InterruptedException {
        ...
        Table join =
                userInfo.join(orderInfo)
                        .where(($("order_id")).isEqual($("id")))
                        .filter($("order_id").isGreater(2))
                        .select($("order_id"), $("name"), $("price"));
        Table result =
                join.groupBy($("order_id"))
                        .select(
                                $("order_id"),
                                $("price").sum().as("total_price"),
                                $("order_id").count().as("total"));
