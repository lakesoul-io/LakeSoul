...
public class DMLSuite extends AbstractTestBase {
    ...
    public void testInsertSQL() throws ExecutionException, InterruptedException {
        ...
        TestUtils.checkEqualInAnyOrder(
                results, new String[] {"+I[2, Alice, 80]", "+I[3, Jack, 75]"});
    ...
    public void testInsertPkPartitionTableSQL() throws ExecutionException, InterruptedException {
        ...
        List<Row> results2 =
                CollectionUtil.iteratorToList(
                        tEnv.executeSql("select order_id from user_info_1").collect());
        TestUtils.checkEqualInAnyOrder(results2, new String[] {"+I[2]", "+I[3]", "+I[4]"});
        List<Row> results3 =
                CollectionUtil.iteratorToList(
                        tEnv.executeSql(
                                        "select order_id, sum(score) from user_info_1 group by"
                                                + " order_id")
                                .collect());
        TestUtils.checkEqualInAnyOrder(
                results3, new String[] {"+I[2, 80]", "+I[3, 75]", "+I[4, 70]"});
    }
    ...
    public void testUpdateNonPkAndPartitionSQL() throws ExecutionException, InterruptedException {
        ...
        createLakeSoulSourceTableUser(tEnv);
        tEnv.executeSql(
                        "INSERT INTO user_info VALUES (2, 'Alice', 80),(3, 'Jack', 75),(3, 'Amy',"
                                + " 95),(4, 'Mike', 70)")
                .await();
        try {
        ...
        TestUtils.checkEqualInAnyOrder(
                results,
                new String[] {
                    "+I[2, Alice, 80]", "+I[3, Amy, 100]", "+I[3, Jack, 100]", "+I[4, Mike, 70]"
                });
    }
    ...
    public void testNonPkPartitionedTableSQL() throws ExecutionException, InterruptedException {
        ...
        tEnv.executeSql(
                        "INSERT INTO user_info_3 VALUES (2, 'Alice', 80),(3, 'Jack', 75),(3, 'Amy',"
                                + " 95),(4, 'Mike', 70)")
                .await();
        List<Row> results1 =
                CollectionUtil.iteratorToList(
                        tEnv.executeSql("select order_id from user_info_3").collect());
        TestUtils.checkEqualInAnyOrder(results1, new String[] {"+I[2]", "+I[3]", "+I[3]", "+I[4]"});
        List<Row> results2 =
                CollectionUtil.iteratorToList(
                        tEnv.executeSql(
                                        "select order_id, sum(score) from user_info_3 group by"
                                                + " order_id")
                                .collect());
        TestUtils.checkEqualInAnyOrder(
                results2, new String[] {"+I[2, 80]", "+I[3, 170]", "+I[4, 70]"});
    }
    ...
    public void testUpdate() throws ExecutionException, InterruptedException {
        ...
        tEnv.executeSql(
                        "INSERT INTO user_info_2 VALUES (2, 'Alice', 80),(3, 'Jack', 75),(3, 'Amy',"
                                + " 95),(4, 'Mike', 70)")
                .await();
        try {
            tEnv.executeSql(
                            "UPDATE user_info_2 set name = cast('Johny' as varchar) where order_id"
                                    + " = 4")
                    .await();
        } catch (Throwable e) {
            System.out.println("Unsupported UPDATE SQL");
        ...
        TestUtils.checkEqualInAnyOrder(
                results,
                new String[] {
                    "+I[2, Alice, 80]", "+I[3, Amy, 95]", "+I[3, Jack, 75]", "+I[4, Johny, 70]"
                });
    }
    ...
    @Test
    public void testUpdatePartitionSQLNotSupported()
            throws ExecutionException, InterruptedException {
        TableEnvironment tEnv = TestUtils.createTableEnv(BATCH_TYPE);
        createLakeSoulSourceTableUserWithRange(tEnv);
        tEnv.executeSql(
                        "INSERT INTO user_info_1 VALUES (2, 'Alice', 80),(3, 'Jack', 75),(3, 'Amy',"
                                + " 95),(4, 'Mike', 70)")
                .await();
        try {
    ...
    public void testUpdateCDCTableSQL() throws ExecutionException, InterruptedException {
        ...
        TestUtils.checkEqualInAnyOrder(
                results,
                new String[] {
                    "+I[2, Alice, 85]", "+I[3, Amy, 95]", "+I[3, Jack, 75]", "+I[4, Mike, 70]"
                });
    }
    ...
    public void testDeletePkSQL() throws ExecutionException, InterruptedException {
        ...
        List<Row> results = CollectionUtil.iteratorToList(flinkTable.execute().collect());
        TestUtils.checkEqualInAnyOrder(
                results, new String[] {"+I[2, Alice, 80]", "+I[3, Amy, 95]", "+I[4, Bob, 110]"});
    }
    ...
    public void testDeletePartitionAndPkSQL() throws ExecutionException, InterruptedException {
        ...
        tEnv.executeSql(
                ...
                .await();
        try {
            // LakeSoulTableSource::applyPartition will not be called and
            // LakeSoulTableSource::applyFilters will be called
    ...
    public void testDeletePartitionOnlySQL() throws ExecutionException, InterruptedException {
        ...
        tEnv.executeSql(
                ...
                .await();
        try {
            // LakeSoulTableSource::applyPartition will be called and
            // LakeSoulTableSource::applyFilters will not be called
        ...
        List<Row> results = CollectionUtil.iteratorToList(flinkTable.execute().collect());
        TestUtils.checkEqualInAnyOrder(
                results, new String[] {"+I[2, Alice, 80]", "+I[4, Bob, 110]"});
    }
    ...
    @Test
    public void testDeleteAllPartitionedDataExactlySQL()
            throws ExecutionException, InterruptedException {
        TableEnvironment tEnv = TestUtils.createTableEnv(BATCH_TYPE);
        createLakeSoulSourceTableUserWithRange(tEnv);
        tEnv.executeSql(
    ...
    public void testPKTypes() throws ExecutionException, InterruptedException {
        ...
        String createUserSql =
                "create table test_pk_types ("
                        ...
                        + "    'use_cdc'='true',"
                        + "    'path'='"
                        + getTempDirUri("/lakeSource/test_pk_types")
                        + "' )";
        tEnv.executeSql("DROP TABLE if exists test_pk_types");
        tEnv.executeSql(createUserSql);
        tEnv.executeSql(
                        "INSERT INTO test_pk_types VALUES (CAST(2 as DECIMAL(10, 5)), DATE"
                            + " '2024-01-01', TIMESTAMP '2024-02-03 01:01:01.123456',"
                            + " 'value1'),(CAST('1234.56' as DECIMAL(10, 5)), DATE '2024-02-01',"
                            + " TIMESTAMP '2024-03-03 01:01:01.123456', 'value2'),(CAST(55.78 as"
                            + " DECIMAL(10, 5)), DATE '2024-01-03', TIMESTAMP '2024-02-04"
                            + " 01:01:01.123456', 'value3'),(CAST('123' as DECIMAL(10, 5)), DATE"
                            + " '2024-05-01', TIMESTAMP '2024-02-05 01:01:01.123456', 'value4')")
                .await();
    ...
    public void testStableSort() throws ExecutionException, InterruptedException {
        ...
        String createUserSql =
                "create table test_stable_sort ("
                        ...
                        + "    'use_cdc'='true',"
                        + "    'path'='"
                        + getTempDirUri("/lakeSource/test_stable_sort")
                        + "' )";
        tEnv.executeSql("DROP TABLE if exists test_stable_sort");
        tEnv.executeSql(createUserSql);
        tEnv.executeSql(
                        "INSERT INTO test_stable_sort VALUES "
    ...
    private void createLakeSoulSourceNonPkTableUser(TableEnvironment tEnvs)
        ...
        String createUserSql =
                "create table user_info_2 ("
                        ...
                        + "    'format'='lakesoul',"
                        + "    'path'='"
                        + getTempDirUri("/lakeSource/user2")
                        + "' )";
    ...
    private void createLakeSoulSourceNonPkWithPartitionTableUser(TableEnvironment tEnvs)
        ...
        String createUserSql =
                "create table user_info_3 ("
                        ...
                        + "    'format'='lakesoul',"
                        + "    'path'='"
                        + getTempDirUri("/lakeSource/user_nonpk_partitioned")
                        + "' )";
        tEnvs.executeSql("DROP TABLE if exists user_info_3");
        tEnvs.executeSql(createUserSql);
    }

    private void createLakeSoulSourceTableUser(TableEnvironment tEnvs)
            throws ExecutionException, InterruptedException {
        String createUserSql =
    ...
    private void createLakeSoulSourceTableUserWithRange(TableEnvironment tEnvs)
        ...
        String createUserSql =
                "create table user_info_1 ("
                        ...
                        + "    'hashBucketNum'='2',"
                        + "    'path'='"
                        + getTempDirUri("/lakeSource/user1")
                        + "' )";
    ...
    public void testInsertSQLVortex() throws ExecutionException, InterruptedException {
        ...
        tEnv.executeSql(
                "create table user_info_vortex ("
                        ...
                        + path
                        + "' )");
        tEnv.executeSql("INSERT INTO user_info_vortex VALUES (2, 'Alice', 80),(3, 'Jack', 75)")
                .await();
        StreamTableEnvironment streamEnv = TestUtils.createStreamTableEnv(BATCH_TYPE);
    ...
    @Test
    public void testInsertPkPartitionTableSQLVortex()
            throws ExecutionException, InterruptedException {
        TableEnvironment tEnv = TestUtils.createTableEnv(BATCH_TYPE);
        ...
        tEnv.executeSql(
                "create table user_info_vortex_1 ("
                        ...
                        + path
                        + "' )");
        tEnv.executeSql("INSERT INTO user_info_vortex_1 VALUES (2, 'Alice', 80),(3, 'Jack', 75)")
                .await();
        StreamTableEnvironment streamEnv = TestUtils.createStreamTableEnv(BATCH_TYPE);
