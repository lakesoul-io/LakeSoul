...
public class CHBenchmark {
    ...
    public static final int insertTimes =
            (customRecordNum - tailCustomRecordNum)
                            / customerBatchSize
    ...
    static final String[] create_table = {
        ...
        //                    "WITH (" +
        //                    String.format("'format'='lakesoul','path'='%s', '%s'='%s',
        // '%s'='false', '%s' = 'true', '%s' = 'latest' ",
        //                            "tmp/oorder",
        //                            JobOptions.LOOKUP_JOIN_CACHE_TTL.key(),
        ...
        public void run() {
            ...
            for (int i = 0; i < batchNum; i++) {
                ...
                if (tableName == "customer")
                    System.out.println("customer id is begin at " + counter);
                ...
                for (int j = 0; j < batchSize; j++) {
                    ...
                    for (int r = 0; r < rep; r++)
                        ...
                        //                        valueList.add(String.format(format, counter++,
                        // j));
                        valueList.add(
                                String.format(
                                        format,
    ...
    public static void main(String[] args) throws Exception {
        ...
        //        GenDataThread newOrderThread = new GenDataThread(lakeSoulCatalog, "oorder",
        // newOrderInterval, insertTimes, orderNumPerBatch, "(1, 1, %s, %s, %s, %s, %s, %s, %s)",
        // 5);

        newOrderThread.start();
        //        tableEnv.executeSql("INSERT INTO default_catalog.default_database.sink " +
        // query_13_collection_streaming).await();

        GenDataThread newCustomerThread =
                new GenDataThread(
                        lakeSoulCatalog,
                        "customer",
                        updateCustomerInterval,
        ...
        try {
            tableEnv.executeSql(
                            "INSERT INTO default_catalog.default_database.sink "
                                    + query_13_lakesoul_streaming)
                    .await(
                            updateCustomerInterval * customerBatchNum + 10000,
        ...
        results.stream()
                ...
                .sorted(
                        Comparator.comparing(
                                row -> Integer.valueOf(row.substring(3, row.indexOf(",")))));
        ...
        List<Row> batchResult =
                CollectionUtil.iteratorToList(batchEnv.executeSql(query_13_batch).collect());
        ...
        try {
            ...
            assertThat(
                            batchResult
                                    .subList(customRecordNum - tailCustomRecordNum, customRecordNum)
                                    .toString())
                    .isEqualTo(
                            results.subList(customRecordNum - tailCustomRecordNum, customRecordNum)
                                    .toString());
