...
public class LakeSoulCatalogTest extends AbstractTestBase {
    ...
    public void createTable() {
        tEnvs.executeSql(
                "CREATE TABLE if not exists user_behaviorgg ( user_id BIGINT, dt STRING, name"
                    + " STRING,primary key (user_id) NOT ENFORCED ) PARTITIONED BY (dt) with"
                    + " ('lakesoul_cdc_change_column'='name',"
                    + " 'hashBucketNum'='2','lakesoul_meta_host'='127.0.0.2','lakesoul_meta_host_port'='9043',"
                    + " 'path'='"
                        + getTempDirUri("/user_behaviorgg")
                        + "')");
        tEnvs.executeSql("show tables").print();
        TableInfo info =
                DbManage.getTableInfoByNameAndNamespace("user_behaviorgg", "test_lakesoul_meta");
        assertEquals(
                info.getTableSchema(),
    ...
    public void createTableWithLike() {
        tEnvs.executeSql(
                "CREATE TABLE if not exists user_behaviorgg ( user_id BIGINT, dt STRING, name"
                    + " STRING NOT NULL,primary key (user_id) NOT ENFORCED ) PARTITIONED BY (dt)"
                    + " with ('lakesoul_cdc_change_column'='name',"
                    + " 'hashBucketNum'='2','lakesoul_meta_host_port'='9043',"
                    + " 'path'='/tmp/user_behaviorgg', 'use_cdc'='true')");

        TableInfo info =
                DbManage.getTableInfoByNameAndNamespace("user_behaviorgg", "test_lakesoul_meta");
        Assertions.assertThat(info.getTableSchema())
                .isEqualTo(
                        ArrowUtils.toArrowSchema(
                                        new StructType()
        ...
        tEnvs.executeSql(
                "CREATE TABLE if not exists like_table with ('path'='/tmp/like_table') like"
                        + " user_behaviorgg");
        TableInfo info2 =
                DbManage.getTableInfoByNameAndNamespace("like_table", "test_lakesoul_meta");
        Assertions.assertThat(info2.getTableSchema())
                .isEqualTo(
                        ArrowUtils.toArrowSchema(
                                        new StructType()
                                                .add("name", StringType, false)
        ...
        Assertions.assertThat(
                        JSON.parseObject(info.getProperties()).get("lakesoul_cdc_change_column"))
                .isEqualTo(
                        JSON.parseObject(info2.getProperties()).get("lakesoul_cdc_change_column"));
        Assertions.assertThat(JSON.parseObject(info.getProperties()).get("path"))
                .isEqualTo("/tmp/user_behaviorgg");
        Assertions.assertThat(JSON.parseObject(info2.getProperties()).get("path"))
                .isEqualTo("/tmp/like_table");
    ...
    public void createTableWithComments() {
        ...
        List<Row> showCreateTableResult =
        ...
        List<Row> descTableResult =
                CollectionUtil.iteratorToList(
                        tEnvs.executeSql("desc table_with_comments").collect());
        System.out.println(descTableResult);
        Assertions.assertThat(descTableResult.get(0).getField(6).toString()).contains("user id");
        Assertions.assertThat(descTableResult.get(1).getField(6).toString()).contains("a用b户c名d");
    ...
    public void testListPartitions()
        ...
        String createUserSql =
                "create table test_table ("
                        ...
                        + "    'connector'='lakesoul',"
                        + "    'path'='"
                        + getTempDirUri("/lakeSource/time_test")
                        + "' )";
        streamTableEnv.executeSql(createUserSql);
        streamTableEnv.executeSql("INSERT INTO test_table VALUES " + "(1, '1'), (2, '2')").await();
        assertTrue(
                "empty partition",
                lakesoulCatalog
        ...
        List<CatalogPartitionSpec> partitionSpecs =
                lakesoulCatalog.listPartitions(ObjectPath.fromString("default.test_table"));
        ...
        for (CatalogPartitionSpec partitionSpec : partitionSpecs) {
            ...
            assertTrue(
                    partitionSpec.getPartitionSpec().get("id").equals("1")
