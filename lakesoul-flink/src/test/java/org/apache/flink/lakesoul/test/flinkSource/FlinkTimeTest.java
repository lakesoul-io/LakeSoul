    ...
    public void testWatermark() throws ExecutionException, InterruptedException {
        ...
        String createUserSql =
                "create table user_actions ("
                        ...
                        "  user_action_time TIMESTAMP(3),   WATERMARK FOR user_action_time AS"
                      + " user_action_time - INTERVAL '10' SECOND ) WITH (    'format'='lakesoul', "
                      + "   'hashBucketNum'='2',    'path'='"
                        + getTempDirUri("/LakeSource/user_actions")
                        + "' )";
        tEnv.executeSql("DROP TABLE if exists user_actions");
        tEnv.executeSql(createUserSql);

        tEnv.executeSql(
                        "INSERT INTO user_actions VALUES (2, 'Alice', TO_TIMESTAMP('1999-01-01"
                            + " 12:09:59')), (3, 'Jack', TO_TIMESTAMP('1999-01-01 12:11:16')),(3,"
                            + " 'Amy', TO_TIMESTAMP('1999-01-01 12:10:00')),(4, 'Mike',"
                            + " TO_TIMESTAMP('1999-01-01 12:10:01'))")
                .await();
        ...
        TestUtils.checkEqualInAnyOrder(
                rows, new String[] {"+I[1999-01-01T12:00, 1, 1]", "+I[1999-01-01T12:10, 3, 3]"});
    }
        ...
        LakeSoulTestUtils.checkStreamingQueryAnswer(
                ...
                (s) -> {
                    try {
                        ...
                        tEnv.executeSql(
                                        "INSERT INTO user_actions VALUES (1000, 'Jack'),(10000,"
                                                + " 'Tom')")
                                .await();
