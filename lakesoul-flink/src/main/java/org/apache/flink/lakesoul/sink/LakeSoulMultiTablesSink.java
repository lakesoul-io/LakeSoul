...
public class LakeSoulMultiTablesSink<IN, OUT>
        implements StatefulSink<IN, LakeSoulWriterBucketState>,
    ...
    public LakeSoulMultiTablesSink(
            BucketsBuilder<IN, OUT, ? extends BucketsBuilder<IN, OUT, ?>> bucketsBuilder) {
    ...
    public static DefaultMultiTablesArrowFormatBuilder forMultiTablesArrowFormat(
            Configuration conf) {
        return new DefaultMultiTablesArrowFormatBuilder(
                new Path(conf.getString(LakeSoulSinkOptions.WAREHOUSE_PATH)), conf);
    }

    @Override
    public AbstractLakeSoulMultiTableSinkWriter<IN, OUT> createWriter(InitContext context)
            throws IOException {
        int subTaskId = context.getSubtaskId();
        AbstractLakeSoulMultiTableSinkWriter<IN, OUT> writer =
                bucketsBuilder.createWriter(context, subTaskId);
        return writer;
    }

    @Override
    public StatefulSinkWriter<IN, LakeSoulWriterBucketState> restoreWriter(
            InitContext context, Collection<LakeSoulWriterBucketState> recoveredState)
            throws IOException {
        int subTaskId = context.getSubtaskId();
        AbstractLakeSoulMultiTableSinkWriter<IN, OUT> writer =
                bucketsBuilder.createWriter(context, subTaskId);
    ...
    public SimpleVersionedSerializer<LakeSoulWriterBucketState> getWriterStateSerializer() {
        try {
            ...
            // LakeSoulMultiTablesSink requires (among other things) a call to FileSystem.get()
            // which declares
            // IOException.
    ...
    public Committer<LakeSoulMultiTableSinkCommittable> createCommitter() throws IOException {
        return new Committer<LakeSoulMultiTableSinkCommittable>() {
            ...
            @Override
            public void commit(
                    Collection<CommitRequest<LakeSoulMultiTableSinkCommittable>> committables)
    ...
    public void addPostCommitTopology(
            DataStream<CommittableMessage<LakeSoulMultiTableSinkCommittable>> committables) {
        StandardSinkTopologies.addGlobalCommitter(
                committables, GlobalCommitterAdapter::new, this::getCommittableSerializer);
    }

    public class GlobalCommitterAdapter implements Committer<LakeSoulMultiTableSinkCommittable> {
        final GlobalCommitter<
                        LakeSoulMultiTableSinkCommittable, LakeSoulMultiTableSinkGlobalCommittable>
                globalCommitter;
        final SimpleVersionedSerializer<LakeSoulMultiTableSinkGlobalCommittable>
                globalCommittableSerializer;

        GlobalCommitterAdapter() {
            try {
                globalCommitter =
                        LakeSoulMultiTablesSink.this.bucketsBuilder.createGlobalCommitter();
                globalCommittableSerializer =
                        LakeSoulMultiTablesSink.this.bucketsBuilder
        ...
        public void commit(
            ...
            List<LakeSoulMultiTableSinkGlobalCommittable> failures =
