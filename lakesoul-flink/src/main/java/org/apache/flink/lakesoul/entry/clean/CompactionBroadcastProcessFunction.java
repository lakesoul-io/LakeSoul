...
public class CompactionBroadcastProcessFunction
    ...
    private static final Logger log =
            LoggerFactory.getLogger(CompactionBroadcastProcessFunction.class);

    private final MapStateDescriptor<String, CompactProcessFunction.CompactionOut>
            broadcastStateDesc;
    ...
    public void open(Configuration parameters) throws Exception {
        ValueStateDescriptor<PartitionInfoRecordGets.PartitionInfo> desc =
                new ValueStateDescriptor<>(
                        "elementState", PartitionInfoRecordGets.PartitionInfo.class);
        timerTsState =
                getRuntimeContext()
    ...
    public void processBroadcastElement(
            ...
            Context ctx,
            Collector<PartitionInfoRecordGets.PartitionInfo> out)
            throws Exception {
        ...
        if (current == null) {
            ...
            if (value.getTimestamp() > current.getTimestamp()) {
                ...
                state.put(
                        key,
                        new CompactProcessFunction.CompactionOut(
                                value.getTableId(),
    ...
    public void processElement(
        ...
        if (valueTimestamp == -5L) {
            ...
            if (compaction != null) {
                ...
                if (valueTimestamp < compactTimstamp
                        && currTimestamp - valueTimestamp > expiredTime) {
                    ...
                    boolean belongOldCompaction =
                            value.version < state.get(key).switchVersion
                                    || latestCompactVersionIsOld;
                    try (Connection connection = dataSource.getConnection()) {
                        cleanUtils.deleteFileAndDataCommitInfo(
                                value.snapshot,
                                value.tableId,
                                value.partitionDesc,
                                connection,
                                belongOldCompaction);
                        cleanUtils.cleanPartitionInfo(
                                value.tableId, value.partitionDesc, value.version, connection);
                    } catch (Exception e) {
    ...
    public void onTimer(
        ...
        if (compactionOut != null) {
            ...
            if (eventTimestamp < compactTimestamp && timestamp - eventTimestamp > expiredTime) {
                ...
                boolean belongOldCompaction =
                        value.version < state.get(partitionDescKey).switchVersion
                                || latestCompactVersionIsOld;
