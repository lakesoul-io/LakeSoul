...
public class CompactProcessFunction
        extends KeyedProcessFunction<String, PartitionInfo, CompactProcessFunction.CompactionOut> {
    ...
    public void open(Configuration openContext) throws Exception {
        ...
        ValueStateDescriptor<Long> switchCompactionVersionDesc =
                new ValueStateDescriptor<>("switchCompactionVersion", Long.class, -1L);
    ...
    public void processElement(
            PartitionInfo value,
            KeyedProcessFunction<String, PartitionInfo, CompactionOut>.Context ctx,
            Collector<CompactionOut> out)
            throws Exception {
        ...
        if (snapshot.size() == 1) {
            try (Connection pgConnection = dataSource.getConnection()) { // ✅ 自动关闭连接
                boolean isOldCompaction =
                        commitOp.equals("UpdateCommit")
                                || cleanUtils.getCompactVersion(
                                        tableId, partitionDesc, version, pgConnection);
                ...
                out.collect(
                        new CompactionOut(
                                ...
                                timestamp,
                                isOldCompaction,
                                switchCompactionVersionState.value()));
            }
        }
    }
