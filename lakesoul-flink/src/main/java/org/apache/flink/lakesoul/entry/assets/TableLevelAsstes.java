...
public class TableLevelAsstes extends KeyedProcessFunction<String, PartitionCounts, TableCounts> {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        MapStateDescriptor<String, PartitionCounts> partitionCountsMapStateDescriptor =
                new MapStateDescriptor<>(
                        "partitionCountsMapStateValue", String.class, PartitionCounts.class);
        partitionCountsMapStateValue =
                getRuntimeContext().getMapState(partitionCountsMapStateDescriptor);

        // 初始化累加状态
        ValueStateDescriptor<Integer> tableBaseFileCountDescriptor =
        ...
        ValueStateDescriptor<Integer> tableTotalFileCountDescriptor =
                new ValueStateDescriptor<>(
        ...
        ValueStateDescriptor<Long> tableBaseSizeDescriptor =
                new ValueStateDescriptor<>(
        ...
        ValueStateDescriptor<Long> tableTotalSizeValueDescriptor =
                new ValueStateDescriptor<>(
        ...
        MapStateDescriptor<String, Boolean> partitionCountValueDescriptor =
                new MapStateDescriptor<>("partitionCountStateValue", String.class, Boolean.class);

        partitionCountStateValue = getRuntimeContext().getMapState(partitionCountValueDescriptor);

        ValueStateDescriptor<HashSet<Integer>> partitionHashSetCountValueDescriptor =
                new ValueStateDescriptor<HashSet<Integer>>(
                        "partitionHashCountStateValue",
                        TypeInformation.of(new TypeHint<HashSet<Integer>>() {}),
                        null);

        partitionHashSetStateValue =
                getRuntimeContext().getState(partitionHashSetCountValueDescriptor);
    }

    @Override
    public void processElement(
            PartitionCounts partitionCounts,
            KeyedProcessFunction<String, PartitionCounts, TableCounts>.Context context,
            Collector<TableCounts> collector)
            throws Exception {
        ...
        int oldPartitionBaseFileCount =
                partitionCountsMapStateValue.get(partitionDesc) == null
                        ? 0
                        : partitionCountsMapStateValue.get(partitionDesc).baseFileCounts;
        int oldPartitionFileCount =
                partitionCountsMapStateValue.get(partitionDesc) == null
                        ? 0
                        : partitionCountsMapStateValue.get(partitionDesc).allFileCounts;
        long oldPartitionBaseFileSize =
                partitionCountsMapStateValue.get(partitionDesc) == null
                        ? 0
                        : partitionCountsMapStateValue.get(partitionDesc).partitionSize;
        long oldPartitionFileSize =
                partitionCountsMapStateValue.get(partitionDesc) == null
                        ? 0
                        : partitionCountsMapStateValue.get(partitionDesc).totalPartitionSize;

        // 防止文件数目或文件大小计算为负值
        int newBaseFileCounts =
                Math.max(0, currentBaseFileCount + fileBaseCounts - oldPartitionBaseFileCount);
        int newFileCounts = currentFileCount + fileCounts - oldPartitionFileCount;
        long newBaseFileSize =
                Math.max(0, currentBaseFileSize + partitionBaseSize - oldPartitionBaseFileSize);
    ...
    public static class MergeFunction
            extends CoProcessFunction<
                    Tuple3<String, String, String[]>, TableCounts, TableCountsWithTableInfo> {
        ...
        public void processElement1(
                Tuple3<String, String, String[]> tableInfoTuple,
                CoProcessFunction<
                                        Tuple3<String, String, String[]>,
                                        TableCounts,
                                        TableCountsWithTableInfo>
                                .Context
                        context,
            ...
            if (tableCounts != null && tableInfoTuple.f1.equals(tableCounts.tableId)) {
                ...
                TableCountsWithTableInfo tableCountsWithTableInfo =
