...
/**
 * Wrapper class for both type of global committables in {@link LakeSoulMultiTablesSink}. One
 * committable might be either one or more pending files to commit, or one in-progress file to clean
 * up.
 */
public class LakeSoulMultiTableSinkGlobalCommittable implements Serializable {
    ...
    private final Map<Tuple2<TableSchemaIdentity, String>, List<LakeSoulMultiTableSinkCommittable>>
            groupedCommittable;
    ...
    public LakeSoulMultiTableSinkGlobalCommittable(
            Map<Tuple2<TableSchemaIdentity, String>, List<LakeSoulMultiTableSinkCommittable>>
                    groupedCommitables,
            boolean isBounded) {
        groupedCommitables.forEach(
                (key, disorderedCommitables) -> {
                    ...
                    for (LakeSoulMultiTableSinkCommittable committable : disorderedCommitables) {
                        if (mergedCommittables.isEmpty()) {
                            ...
                            LakeSoulMultiTableSinkCommittable tail =
                                    mergedCommittables.get(mergedCommittables.size() - 1);
                            if (isBounded
                                    || tail.getCreationTime() == committable.getCreationTime()) {
    ...
    public static LakeSoulMultiTableSinkGlobalCommittable
            fromLakeSoulMultiTableSinkGlobalCommittable(
                    List<LakeSoulMultiTableSinkGlobalCommittable> globalCommittables,
                    boolean isBounded) {
        Map<Tuple2<TableSchemaIdentity, String>, List<LakeSoulMultiTableSinkCommittable>>
                groupedCommitables = new HashMap<>();
        globalCommittables.forEach(
                globalCommittable ->
                        globalCommittable
                        ...
                        groupedCommitables
                                .computeIfAbsent(
                                        Tuple2.of(
                                                ...
                                                committable.getBucketId()),
                                        tuple2 -> new ArrayList<>())
                                .add(committable));
        return new LakeSoulMultiTableSinkGlobalCommittable(groupedCommitables, isBounded);
    }

    public Map<Tuple2<TableSchemaIdentity, String>, List<LakeSoulMultiTableSinkCommittable>>
            getGroupedCommittable() {
    ...
    public String toString() {
        return "LakeSoulMultiTableSinkGlobalCommittable{"
                + "groupedCommitables="
                + groupedCommittable
                + '}';
    }
