...
public class JdbcIngest {
    public static void main(String[] args)
            throws ExecutionException, InterruptedException, TableNotExistException {
        ...
        for (String tableName : tableArray) {
            tEnv.getConfig()
                    .getConfiguration()
                    .setString("pipeline.name", defaultDatabase + "-lakesoul-" + tableName);
            tEnv.getConfig()
                    .getConfiguration()
            ...
            CatalogBaseTable table =
                    mysqlCatalog.getTable(ObjectPath.fromString(defaultDatabase + "." + tableName));
            ...
            try (Connection conn =
                            DriverManager.getConnection(
                                    baseUrl + "/" + defaultDatabase, username, password);
            ...
            if (!lakesoulCatalog.tableExists(ObjectPath.fromString(lakesoulDBTable))) {
                ...
                TableDescriptor.Builder sinkTableBuilder =
            ...
            for (LocalDate date = start; !date.isAfter(end); date = date.plusDays(1)) {
                ...
                String selectColumns =
                        needPartition
                                ? String.format(
                                        "*, date_format(%s,'yyyy-MM') as %s",
                                        partitionColumn, formatedCol)
                                : "*";
                String sql =
                        String.format(
