...
public class NamespaceLevelAssets
        extends KeyedProcessFunction<String, TableCountsWithTableInfo, NameSpaceCount> {
    ...
    public void open(Configuration parameters) {
        MapStateDescriptor<String, TableCounts> tableStateDescriptor =
                new MapStateDescriptor<>("tableCountsMapState", String.class, TableCounts.class);
    ...
    public void processElement(
            ...
            KeyedProcessFunction<String, TableCountsWithTableInfo, NameSpaceCount>.Context context,
            Collector<NameSpaceCount> collector)
            throws Exception {
        ...
        int oldPartitionCounts =
                tableCountsMapState.get(tableId) == null
                        ? 0
                        : tableCountsMapState.get(tableId).partitionCounts;
        long oldBaseFileSize =
                tableCountsMapState.get(tableId) == null
                        ? 0
                        : tableCountsMapState.get(tableId).baseFileSize;
        long oldFileSize =
                tableCountsMapState.get(tableId) == null
                        ? 0
                        : tableCountsMapState.get(tableId).totalFileSize;
        int oldBaseFileCounts =
                tableCountsMapState.get(tableId) == null
                        ? 0
                        : tableCountsMapState.get(tableId).baseFileCounts;
        int oldFileCounts =
                tableCountsMapState.get(tableId) == null
                        ? 0
                        : tableCountsMapState.get(tableId).totalFileCounts;
        ...
        TableCounts newTableCounts =
                new TableCounts(
                        tableId, partitionCount, fileBaseCount, fileCount, fileBaseSize, fileSize);
        if (!tableCountsMapState.contains(tableId)) {
            NameSpaceCount newNameSpaceCount =
                    new NameSpaceCount(
                            namespace,
                            k,
                            currentFileCount + fileCount,
                            currentFileBaseCount + fileBaseCount,
                            currentPartitionCount + partitionCount,
                            currentFileSize + fileSize,
                            currentFileBaseSize + fileBaseSize);
            ...
            NameSpaceCount newNameSpaceCount =
                    new NameSpaceCount(
                            namespace,
                            k,
                            currentFileCount + fileCount - oldFileCounts,
                            currentFileBaseCount + fileBaseCount - oldBaseFileCounts,
                            currentPartitionCount + partitionCount - oldPartitionCounts,
                            currentFileSize + fileSize - oldFileSize,
                            currentFileBaseSize + fileBaseSize - oldBaseFileSize);
            nameSpaceCountValueState.update(newNameSpaceCount);
            collector.collect(newNameSpaceCount);
            tableCountsMapState.put(tableId, newTableCounts);
        }
