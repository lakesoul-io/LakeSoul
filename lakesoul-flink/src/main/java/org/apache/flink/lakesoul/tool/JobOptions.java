...
public class JobOptions {
    public static final ConfigOption<String> JOB_CHECKPOINT_MODE =
            ConfigOptions.key("job.checkpoint_mode")
                    .stringType()
                    .defaultValue("EXACTLY_ONCE")
                    .withDescription("job checkpoint mode");

    public static final ConfigOption<Integer> JOB_CHECKPOINT_INTERVAL =
            ConfigOptions.key("job.checkpoint_interval")
                    .intType()
                    .defaultValue(10 * 60 * 1000)
                    .withDescription("job checkpoint interval");

    public static final ConfigOption<String> FLINK_CHECKPOINT =
            ConfigOptions.key("flink.checkpoint")
                    .stringType()
                    .noDefaultValue()
                    .withDescription("flink checkpoint save path");

    public static final ConfigOption<String> FLINK_SAVEPOINT =
            ConfigOptions.key("flink.savepoint")
                    .stringType()
                    .noDefaultValue()
                    .withDescription(
                            "Flink savepoint save path. \n"
                                    + " Invalid config option for the reason:"
                                    + " https://issues.apache.org/jira/browse/FLINK-23515");
    ...
    public static final ConfigOption<Long> LOOKUP_JOIN_CACHE_SIZE =
            ConfigOptions.key("lookup.join.cache.size")
                    .longType()
                    .defaultValue(100000L)
                    .withDescription("The cache size limit for the build table in lookup join.");
            ...
            key("streaming-source.enable")
                    ...
                    .withDescription(
                            Description.builder()
                                    ...
                                    .linebreak()
                                    .text(
                                            " NOTES: Please make sure that each partition/file"
                                                    + " should be written atomically, otherwise the"
                                                    + " reader may get incomplete data.")
                                    .build());
    ...
    public static final ConfigOption<String> STREAMING_SOURCE_PARTITION_INCLUDE =
            key("streaming-source.partition.include")
                    ...
                    .withDescription(
                            Description.builder()
                                    .text(
                                            "Option to set the partitions to read, supported values"
                                                    + " are")
                                    .list(
                                            text("all (read all partitions)"),
                                            text(
                                                    "latest (read latest partition in order of"
                                                        + " 'streaming-source.partition.order',"
                                                        + " this only works when a streaming Hive"
                                                        + " source table is used as a temporal"
                                                        + " table)"))
                                    .build());
    ...
    public static final ConfigOption<Integer> STREAMING_SOURCE_LATEST_PARTITION_NUMBER =
            key("streaming-source.latest.partition.number")
                    ...
                    .defaultValue(1)
                    .withDescription(
                            "Option to set the latest partition number to read. It is only valid"
                                    + " when STREAMING_SOURCE_PARTITION_INCLUDE is 'latest'.");
    ...
    public static final ConfigOption<String> PARTITION_ORDER_KEYS =
            key("partition.order.keys")
                    ...
                    .noDefaultValue()
                    .withDescription(
                            "Option to set partition order keys (e.g. partition1,partition2) to"
                                + " sort multiple partitions. Using all partitions to sort if this"
                                + " value is not set.");
    ...
    public static final ConfigOption<String> KUBE_CLUSTER_ID =
            key("kubernetes.cluster-id")
                    ...
                    .withDescription(
                            Description.builder()
                                    .text(
                                            "The cluster-id, which should be no more than 45"
                                                + " characters, is used for identifying a unique"
                                                + " Flink cluster. The id must only contain"
                                                + " lowercase alphanumeric characters and \"-\"."
                                                + " The required format is %s. If not set, the"
                                                + " client will automatically generate it with a"
                                                + " random ID.",
