...
public abstract class LakeSoulSource<OUT>
    ...
    @Nullable final List<Map<String, String>> remainingPartitions;

    @Nullable protected final Plan pushedFilter;

    @Nullable final Plan partitionFilters;
    protected final RowType tableRowType;

    public LakeSoulSource(
            TableId tableId,
    ...
    public SplitEnumerator<LakeSoulPartitionSplit, LakeSoulPendingSplits> createEnumerator(
        ...
        List<String> readStartTimestampWithTimeZone =
                Arrays.asList(
                        optionParams.getOrDefault(LakeSoulOptions.READ_START_TIME(), ""),
        ...
        if (getBoundedness().equals(Boundedness.CONTINUOUS_UNBOUNDED)) {
            return new LakeSoulAllPartitionDynamicSplitEnumerator(
                    ...
                    this.tableRowType,
                    Long.parseLong(
                            optionParams.getOrDefault(
                                    LakeSoulOptions.DISCOVERY_INTERVAL(), "30000")),
            ...
            return staticSplitEnumerator(
                    enumContext, tableInfo, readStartTimestampWithTimeZone, readType);
        }
    }
    ...
    private LakeSoulStaticSplitEnumerator staticSplitEnumerator(
        ...
        List<String> readEndTimestampWithTimeZone =
                Arrays.asList(
                        optionParams.getOrDefault(LakeSoulOptions.READ_END_TIME(), ""),
                        optionParams.getOrDefault(LakeSoulOptions.TIME_ZONE(), ""));
        ...
        if (!FlinkUtil.isExistHashPartition(tableInfo)) {
            for (DataFileInfo dataFileInfo : dataFileInfoList) {
                ...
                splits.add(
                        new LakeSoulPartitionSplit(
                                String.valueOf(dataFileInfo.hashCode()),
                                tmp,
                                0,
            ...
            Map<String, Map<Integer, List<Path>>> splitByRangeAndHashPartition =
                    FlinkUtil.splitDataInfosToRangeAndHashPartition(
                            tableInfo, dataFileInfoList.toArray(new DataFileInfo[0]));
        ...
        return new LakeSoulStaticSplitEnumerator(
                enumContext, new LakeSoulSimpleSplitAssigner(splits));
    }

    private DataFileInfo[] getTargetDataFileInfo(TableInfo tableInfo) {
        return FlinkUtil.getTargetDataFileInfo(tableInfo, this.remainingPartitions);
    }
    ...
    public SplitEnumerator<LakeSoulPartitionSplit, LakeSoulPendingSplits> restoreEnumerator(
            SplitEnumeratorContext<LakeSoulPartitionSplit> enumContext,
            LakeSoulPendingSplits checkpoint)
            throws Exception {
        return new LakeSoulAllPartitionDynamicSplitEnumerator(
                enumContext,
                new LakeSoulDynSplitAssigner(
                        checkpoint.getSplits(), String.valueOf(checkpoint.getHashBucketNum())),
                this.tableRowType,
