...
Flink 引入 LakeSoul 依赖的方法：下載 lakesoul-flink-1.20_2.12-VAR::VERSION.jar，放入 `$FLINK_HOME/lib` ，或在启动时指定 jar 的路径。
...
```bash
# 启动 Flink SQL Client
bin/sql-client.sh embedded -j lakesoul-flink-1.20_2.12-VAR::VERSION.jar
```
