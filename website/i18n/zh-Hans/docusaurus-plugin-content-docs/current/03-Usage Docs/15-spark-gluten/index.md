...
$SPARK_HOME/bin/spark-shell --master local\[1\] --driver-memory 4g \
  ...
  --jars lakesoul-spark-gluten-3.5_2.12-VAR::VERSION.jar
```
以这样的方式启动 Spark 任务后，即可同时启用 Gluten 和 LakeSoul，实现 IO 性能、计算性能的双重加速。注意 LakeSoul 的包名为 `lakesoul-spark-gluten-3.5_2.12-VAR::VERSION.jar`，其中 `3.5` 为 Spark 版本，`VAR::VERSION` 为 LakeSoul 版本。
