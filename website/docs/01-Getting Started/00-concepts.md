...
2. NativeIO layer. Vectorized Parquet and Vortex file reading and writing implemented in Rust. LakeSoul 4.0 can scan both formats in one snapshot and defaults new writes to `vortex-compact`; see [Physical File Formats](05-physical-file-formats.md). For real-time update scenarios, a “single-layer” LSM-Tree-like method is used. Primary-key tables are sharded and sorted by primary-key hash, then automatically merged during reads to implement Upsert. NativeIO also provides object-store optimizations, column pruning, and filter pushdown.
...
4. [Python SDK and ecosystem integrations](../03-Usage%20Docs/11-lakesoul-python/01-overview.md). The LakeSoul Python SDK provides catalog and table IO APIs, with adapters for PyArrow, Pandas, DuckDB, PyTorch, Hugging Face Datasets, Ray Data, and Daft.
...
Table schemas use Apache Arrow types and support nested structures such as `Struct`. The schema is independent of the selected [physical file format](05-physical-file-formats.md): a table snapshot may contain Parquet and Vortex files.
