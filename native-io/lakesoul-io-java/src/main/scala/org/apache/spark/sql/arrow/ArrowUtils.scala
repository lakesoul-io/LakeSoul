...
object ArrowUtils {
  ...
  def fromArrowType(dt: ArrowType): DataType = dt match {
    ...
    case ts: ArrowType.Timestamp if ts.getTimezone == null => TimestampNTZType
  ...
  private def toArrowFieldInternal(
    ...
    val generated = dt match {
      case ArrayType(elementType, containsNull) =>
        val fieldType =
          new FieldType(nullable, ArrowType.List.INSTANCE, null, metadata)
        new Field(
          name,
          fieldType,
          Seq(
      ...
      case MapType(keyType, valueType, valueContainsNull) =>
        val mapType =
          new FieldType(nullable, new ArrowType.Map(false), null, metadata)
      ...
      case dataType =>
        val fieldType = new FieldType(
          nullable,
          toArrowType(dataType, timeZoneId),
          null,
          metadata
        )
    ...
    if (!sparkFieldMetadata.contains(LAKESOUL_ARROW_FIELD)) {
      ...
      val original = try {
        reader.readValue[Field](
          sparkFieldMetadata.getString(LAKESOUL_ARROW_FIELD)
        )
      } catch {
        case NonFatal(error) =>
          throw new IllegalArgumentException(
            s"Invalid $LAKESOUL_ARROW_FIELD metadata for field '$name'",
            error
          )
      }
      mergeArrowField(
  ...
  private def mergeArrowField(
    ...
    val restoredType =
      if (!preserveLargeTypes && isLargeOffsetType(original.getType)) {
        generated.getType
    ...
    (generated.getType, original.getType) match {
      case (_: ArrowType.Struct, _: ArrowType.Struct) =>
        generatedChildren.map { child =>
          originalChildren
            .find(_.getName.equalsIgnoreCase(child.getName))
            .map(
              mergeArrowField(
                child,
                _,
  ...
  private def arrowTypesAreSparkEquivalent(
      generated: ArrowType,
      original: ArrowType
  ): Boolean = {
    if (generated == original) {
      ...
      generated.isInstanceOf[ArrowType.Struct] && original
        .isInstanceOf[ArrowType.Struct]
    ) {
      true
    } else if (
      generated.isInstanceOf[ArrowType.Map] && original
        .isInstanceOf[ArrowType.Map]
    ) {
      true
    } else {
      try {
  ...
  private def isListType(dataType: ArrowType): Boolean =
    dataType.isInstanceOf[ArrowType.List] || dataType
      .isInstanceOf[ArrowType.LargeList]
  ...
  private def mergeArrowMetadata(
      ...
      original: util.Map[String, String]
      ...
      case ArrowType.Struct.INSTANCE =>
        val fields = field.getChildren().asScala.map { child =>
          ...
          if (comment == null)
            ...
            StructField(child.getName, dt, child.isNullable).withComment(
              comment
            )
        }
  ...
  def fromArrowField(field: Field): StructField = {
    ...
    val metadata =
      Option(field.getMetadata).getOrElse(new util.HashMap[String, String])
    ...
    metadata.forEach((key, value) =>
      if (key != "spark_comment") {
        newMetadata.putString(key, value)
      }
    )
    if (requiresArrowFieldRoundTrip(field)) {
      newMetadata.putString(
        LAKESOUL_ARROW_FIELD,
        writer.writeValueAsString(field)
      )
    }
  ...
  private def requiresArrowFieldRoundTrip(field: Field): Boolean = {
    val requiresCurrentType = field.getType match {
      case _: ArrowType.LargeUtf8 | _: ArrowType.LargeBinary |
          _: ArrowType.LargeList =>
        true
    ...
    requiresCurrentType || field.getChildren.asScala.exists(
      requiresArrowFieldRoundTrip
    )
  }
  ...
  /** Maps a Spark schema to the lossless Arrow schema persisted in LakeSoul
    * metadata.
    */
  def toMetadataArrowSchema(
      schema: StructType,
