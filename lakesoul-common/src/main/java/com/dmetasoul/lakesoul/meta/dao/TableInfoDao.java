...
public class TableInfoDao {
    ...
    public List<TableInfo> selectByNamespace(String namespace) {
        String sql =
                String.format("select * from table_info where table_namespace='%s'", namespace);
    ...
    public TableInfo selectByTableNameAndNameSpace(String tableName, String namespace) {
        ...
        String sql =
                String.format(
                        "select * from table_info where table_name = '%s'"
    ...
    public TableInfo selectByIdAndTablePath(String tableId, String tablePath) {
        ...
        String sql =
                String.format(
                        "select * from table_info where table_id = '%s' and table_path = '%s' ",
                        tableId, tablePath);
        return getTableInfo(sql);
    }
    ...
    public TableInfo selectByIdAndTableName(String tableId, String tableName) {
        String sql =
                String.format(
                        "select * from table_info where table_id = '%s' and table_name = '%s' ",
                        tableId, tableName);
        return getTableInfo(sql);
    }
        ...
        try {
            ...
            pstmt =
                    conn.prepareStatement(
                            "insert into table_info(table_id, table_name, table_path, table_schema,"
                                + " table_schema_arrow_ipc, table_schema_arrow_ipc_json_hash,"
                                + " properties, partitions, table_namespace, domain) values (?, ?,"
                                + " ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public void deleteByIdAndPath(String tableId, String tablePath) {
        ...
        String sql =
                String.format(
                        "delete from table_info where table_id = '%s' and table_path = '%s'",
                        tableId, tablePath);
        try {
            conn = DBConnector.getConn();
            pstmt = conn.prepareStatement(sql);
    ...
    public int updateByTableId(
            String tableId, String tableName, String tablePath, String tableSchema) {
        if (NativeUtils.NATIVE_METADATA_UPDATE_ENABLED) {
            return NativeMetadataJavaClient.update(
        ...
        if (StringUtils.isBlank(tableName)
                && StringUtils.isBlank(tablePath)
                && StringUtils.isBlank(tableSchema)) {
            return result;
        }
        Connection conn = null;
        PreparedStatement pstmt = null;
        ...
        if (StringUtils.isNotBlank(tableSchema)) {
            sb.append(
                    String.format(
                            "table_schema = '%s', table_schema_arrow_ipc = NULL,"
                                    + " table_schema_arrow_ipc_json_hash = NULL, ",
                            tableSchema));
        }
