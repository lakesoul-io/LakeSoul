...
case class DataFileInfo(
  ...
  def expire(deleteTime: Long): DataFileInfo =
    this.copy(modification_time = deleteTime)
}
...
object DataOperation {
  ...
  private def filterFiles(
      file_arr_buf: ArrayBuffer[DataFileInfo]
  ): ArrayBuffer[DataFileInfo] = {
  ...
  private def fillFiles(
      file_arr_buf: ArrayBuffer[DataFileInfo],
      dataCommitInfoList: Array[DataCommitInfo]
  ): ArrayBuffer[DataFileInfo] = {
  ...
  def getSinglePartitionDataInfo(
      partition_info: PartitionInfoScala
  ): ArrayBuffer[DataFileInfo] = {
    ...
    metaPartitionInfoScala.addAllSnapshot(
      JavaConverters.bufferAsJavaList(
        partition_info.read_files.map(DBUtil.toProtoUuid).toBuffer
      )
    )
    val dataCommitInfoList = dbManager
      .getTableSinglePartitionDataInfo(metaPartitionInfoScala.build)
      .asScala
      .toArray
  ...
  private def getSinglePartitionDataInfo(
      table_id: String,
    ...
    val file_arr_buf = new ArrayBuffer[DataFileInfo]()
    val snapshotInfo =
      dbManager.getPartitionSnapshot(table_id, partition_desc, version)
  ...
  def getIncrementalPartitionDataInfo(
      table_id: String,
      partition_desc: String,
      startTimestamp: Long,
      endTimestamp: Long,
    ...
    if (readType.equals(LakeSoulOptions.ReadType.SNAPSHOT_READ)) {
      if (null == partition_desc || "".equals(partition_desc)) {
        ...
        partitions.forEach(partition => {
          val version = dbManager
            .getLastedVersionUptoTime(table_id, partition, endTimestamp)
          files_all_partitions_buf ++= getSinglePartitionDataInfo(
            table_id,
            partition,
            version
          )
        ...
        val version = dbManager.getLastedVersionUptoTime(
          table_id,
          partition_desc,
          endTimestamp
        )
      ...
      if (null == partition_desc || "".equals(partition_desc)) {
        val partitions = dbManager.getTableAllPartitionDesc(table_id)
        partitions.forEach(partition => {
          val preVersionTimestamp = dbManager.getLastedVersionTimestampUptoTime(
            table_id,
            partition,
            startTimestamp
          )
          if (preVersionTimestamp == 0) {
            ...
            files_all_partitions_buf ++= getSinglePartitionIncrementalDataInfos(
              table_id,
              partition,
              preVersionTimestamp,
              endTimestamp
        ...
        if (preVersionTimestamp == 0) {
          ...
          getSinglePartitionIncrementalDataInfos(
            table_id,
            partition_desc,
            preVersionTimestamp,
            endTimestamp
  ...
  def getSinglePartitionIncrementalDataInfos(
      table_id: String,
      partition_desc: String,
      startVersionTimestamp: Long,
      endVersionTimestamp: Long
  ): ArrayBuffer[DataFileInfo] = {
    ...
    val dataCommitInfoList = dbManager
      .getIncrementalPartitionsFromTimestamp(
        table_id,
        partition_desc,
        startVersionTimestamp,
        endVersionTimestamp
      )
      .asScala
      .toArray
    ...
    loop.breakable {
      for (dataItem <- dataCommitInfoList) {
        count += 1
        if (
          CommitOp.UpdateCommit.equals(
            dataItem.getCommitOp
          ) && startVersionTimestamp != dataItem.getTimestamp && count != 1
        ) {
          updated = true
          loop.break()
        }
        if (startVersionTimestamp == dataItem.getTimestamp) {
          preVersionUUIDs ++= dataItem.getSnapshotList.asScala.map(
            DBUtil.toJavaUUID
          )
        } else {
          if (CommitOp.CompactionCommit.equals(dataItem.getCommitOp)) {
            val compactShotList =
              dataItem.getSnapshotList.asScala.map(DBUtil.toJavaUUID).toArray
            compactionUUIDs += compactShotList(0)
            if (compactShotList.length > 1) {
              incrementalAllUUIDs ++= compactShotList.slice(
                1,
                compactShotList.length
              )
            }
          } else {
            incrementalAllUUIDs ++= dataItem.getSnapshotList.asScala.map(
              DBUtil.toJavaUUID
            )
          }
        }
    ...
    if (updated) {
      ...
      val file_arr_buf = new ArrayBuffer[DataFileInfo]()
      val dataCommitInfoList = dbManager
        .getDataCommitInfosFromUUIDs(
          table_id,
          partition_desc,
          resultUUID.map(DBUtil.toProtoUuid).toList.asJava
        )
        .asScala
        .toArray
