    ...
    if isinstance(arrow_type_json, str):
        ...
        if "Decimal128" in arrow_type_json:
            return pyarrow.decimal128(
                arrow_type_json["Decimal128"][0], arrow_type_json["Decimal128"][1]
            )
        elif "Decimal256" in arrow_type_json:
            return pyarrow.decimal256(
                arrow_type_json["Decimal256"][0], arrow_type_json["Decimal256"][1]
            )
            ...
            return pyarrow.list_(
                deserialize_from_rust_arrow_type(arrow_type_json["List"]["data_type"])
            )
        elif "FixedSizeList" in arrow_type_json:
            return pyarrow.list_(
                deserialize_from_rust_arrow_type(
                    arrow_type_json["FixedSizeList"][0]["data_type"]
                ),
                arrow_type_json["FixedSizeList"][1],
            )
        elif "Dictionary" in arrow_type_json:
            return pyarrow.dictionary(
                arrow_type_json["Dictionary"][0], arrow_type_json["Dictionary"][1]
            )
            ...
            return pyarrow.map_(
                deserialize_from_rust_arrow_type(
                    arrow_type_json["Map"][0]["data_type"]["Struct"][0]["data_type"]
                ),
                deserialize_from_rust_arrow_type(
                    arrow_type_json["Map"][0]["data_type"]["Struct"][1]["data_type"]
                ),
            ...
            return pyarrow.time32(
                "ms" if arrow_type_json["Time32"] == "Millisecond" else "s"
            )
        elif "Time64" in arrow_type_json:
            return pyarrow.time64(
                "us" if arrow_type_json["Time64"] == "Microsecond" else "ns"
            )
...
def deserialize_from_java_arrow_field(arrow_field_json):
    ...
    if isinstance(arrow_type_json, dict):
        ...
        if name == "list":
            ...
            arrow_type = pyarrow.list_(
                deserialize_from_java_arrow_field(child_field).type
            )
        if name == "largelist":
            child_field = arrow_field_json["children"][0]
            arrow_type = pyarrow.large_list(
                deserialize_from_java_arrow_field(child_field).type
            )
        if name == "fixedsizelist":
            ...
            arrow_type = pyarrow.list_(
                deserialize_from_java_arrow_field(child_field).type, list_size
            )
        if name == "union":
            pass
        if name == "map":
            ...
            pyarrow.map_(
                child_type.field[0].type, child_type.field[1].type, keys_sorted
            )
        if name == "int":
    ...
    if arrow_type is None:
        raise IOError(
            "Failed at deserialize_from_java_arrow_type: " + str(arrow_type_json)
        )
...
def to_arrow_field(arrow_field_json):
    if "data_type" in arrow_field_json:
        return pyarrow.field(
            arrow_field_json["name"],
            deserialize_from_rust_arrow_type(arrow_field_json["data_type"]),
