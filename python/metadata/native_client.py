...
from lib.lakesoul_metadata_c import *
...
class NativeMetadataClient:
    def __init__(self):
        ...
        self._runtime = create_tokio_runtime()
        ...
        def target():
            return create_tokio_postgres_client(CFUNCTYPE(c_void_p, c_bool, c_char_p)(callback),
                                                config.encode("utf-8"),
                                                self._runtime)
        ...
        self._prepared = create_prepared_statement()

    def __del__(self):
        free_tokio_runtime(self._runtime)
        free_tokio_postgres_client(self._client)
        free_prepared_statement(self._prepared)
