...
def preprocess_data(dataset):
    ...
    dataset = dataset.withColumn(
        "Title",
        F.split(dataset["Name"], ",")[1].substr(
            F.lit(0), F.instr(F.split(dataset["Name"], ",")[1], ".") - 1
        ),
    )
    ...
    dataset = dataset.fillna(
        {
            "Embarked": dataset.groupby("Embarked")
            .count()
...
def encode_categorical_features(dataset):
    ...
    for categorical_col in categorical_columns:
        string_indexer = StringIndexer(
            inputCol=categorical_col, outputCol=categorical_col + "Index"
        )
...
def tranform(dataset):
    ...
    train_dataset, val_dataset = train_dataset.randomSplit(
        [1 - args.split_ratio, args.split_ratio], seed=20230909
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Ratio for random train-validation split",
    )
    ...
    # Feature tranform
    dataset = (
        LakeSoulTable.forName(spark, table_raw)
        .toDF()
        .where(F.col("split").eqNullSafe("train"))
    )
    ...
    train_data.write.mode("append").format("lakesoul").option(
        "rangePartitions", "split"
    ).option("shortTableName", table_trans).save(tablePath)
    val_data.write.mode("append").format("lakesoul").option(
        "rangePartitions", "split"
    ).option("shortTableName", table_trans).save(tablePath)
