...
if __name__ == "__main__":
    ...
    trainDf.withColumn("split", lit("train")).write.mode("append").format(
        "lakesoul"
    ).option("rangePartitions", "split").option("shortTableName", dataset_table).save(
        tablePath
    )

    testDf.withColumn("split", lit("test")).write.mode("append").format(
        "lakesoul"
    ).option("rangePartitions", "split").option("shortTableName", dataset_table).save(
        tablePath
    )
    ...
    spark.sql("show tables").show()
    LakeSoulTable.forName(spark, dataset_table).toDF().where(
        col("split").eqNullSafe("train")
    ).show(20)
    LakeSoulTable.forName(spark, dataset_table).toDF().where(
        col("split").eqNullSafe("test")
    ).show(20)
