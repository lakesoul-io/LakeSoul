...
def test_data_values_vs_arrow(ray_session, part_arrow_table):
    ...
    for col_name in arrow_table.column_names:
        ...
        # pyarrow is old, so type checking is disabled
        ray_sorted = pc.sort_indices(ray_col)  # ty: ignore[unresolved-attribute]
        arrow_sorted = pc.sort_indices(arrow_col)  # ty: ignore[unresolved-attribute]
