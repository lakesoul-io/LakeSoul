...
def test_daft_write_partitioned_append_reads_filtered_partition_with_arrow(
    ...
    catalog = LakeSoulCatalog.from_env()
    table_name = _table_name("daft_part")
    schema = pa.schema(
        [
            pa.field("id", pa.int64(), nullable=False),
...
def test_daft_write_mixed_case_create_uses_datafusion_case_folding(
    tmp_path: Path,
) -> None:
    catalog = LakeSoulCatalog.from_env()
    table_name = _table_name("Daft_Mixed")
...
def test_daft_write_pk_upsert_reads_latest_rows_with_arrow(tmp_path: Path) -> None:
    catalog = LakeSoulCatalog.from_env()
    table_name = _table_name("daft_pk")
    schema = pa.schema(
        [
            pa.field("id", pa.int64(), nullable=False),
