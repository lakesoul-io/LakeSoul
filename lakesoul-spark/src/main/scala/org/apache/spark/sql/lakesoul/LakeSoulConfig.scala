...
case class LakeSoulConfig[T](
  ...
  /** Recover the saved value of this configuration from `TableInfo` or return
    * the default if this value hasn't been changed.
    */
  ...
  /** Validate this configuration and return the key - value pair to save into
    * the metadata.
    */
...
object LakeSoulConfig extends Logging {
  ...
  /** A global default value set as a SQLConf will overwrite the default value
    * of a LakeSoulConfig. For example, user can run: set
    * spark.databricks.delta.properties.defaults.randomPrefixLength = 5 This
    * setting will be populated to a LakeSoulTableRel during its creation time
    * and overwrites the default value of delta.randomPrefixLength.
    *
    * We accept these SQLConfs as strings and only perform validation in
    * LakeSoulConfig. All the LakeSoulConfigs set in SQLConf should adopt the
    * same prefix.
    */
  ...
  private def buildConfig[T](
    ...
    val lakeSoulConfig = LakeSoulConfig(
      s"lakesoul.$key",
      defaultValue,
      fromString,
      validationFunction,
  ...
  /** Validates specified configurations and returns the normalized key -> value
    * map.
    */
  def validateConfigurations(
    ...
    configurations.map {
      case (key, value)
          if key.toLowerCase(Locale.ROOT).startsWith("lakesoul.") =>
        Option(
          entries.get(key.toLowerCase(Locale.ROOT).stripPrefix("lakesoul."))
        )
          .map(_(value))
          .getOrElse {
  ...
  /** Whether this lakesoul table is append-only. Files can't be deleted, or
