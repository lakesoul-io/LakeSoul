...
case class LakeSoulPostHocAnalysis(spark: SparkSession)
    extends Rule[LogicalPlan] {

  override def apply(plan: LogicalPlan): LogicalPlan =
    plan.resolveOperatorsDown {

      /** Hash columns in LakeSoulTableRel are primary keys, they have no
        * duplicate data and shouldn't be null, so [[Intersect]]/[[Except]]
        * operator can be replaced with a left-semi/left-anti [[Join]] operator.
        * {{{
        ...
        * }}}
        *
        * Note: This rule is only applicable to INTERSECT/EXCEPT
        * LakeSoulTableRel hash columns.
        */
      case Intersect(left, right, _) =>
        ...
        val leftInfo = findLakeSoulRelation(
          transLeft,
          transLeft.references.map(_.name).toSet
        )
        val rightInfo = findLakeSoulRelation(
          transRight,
          transRight.references.map(_.name).toSet
        )
        val canOptimize =
          leftInfo._1 && rightInfo._1 && (leftInfo._2 == rightInfo._2)

        if (canOptimize) {
          val joinCond =
            left.output.zip(right.output).map { case (l, r) => EqualTo(l, r) }
          Join(
            left,
            right,
            LeftSemi,
  ...
  def findLakeSoulRelation(
      plan: LogicalPlan,
      outCols: Set[String]
  ): (Boolean, Int) = {
    plan match {
      case DataSourceV2Relation(
            LakeSoulTableV2(_, path, _, _, _, _),
            _,
            _,
            _,
            _
