...
/** Analysis rules for LakeSoul. Currently, these rules enable schema
  * enforcement / evolution with INSERT INTO.
  */
case class LakeSoulAnalysis(session: SparkSession, sqlConf: SQLConf)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan =
    plan.resolveOperatorsDown {
      ...
      case a @ AppendData(
            DataSourceV2Relation(d: LakeSoulTableV2, _, _, _, _),
            query,
            _,
            false,
            _,
          ...
          if query.resolved && needsSchemaAdjustment(
            d.name(),
            query,
            d.schema()
          ) =>
      ...
      case d @ DeleteFromTable(table, condition) if d.childrenResolved =>
        val newTarget = table.transformUp { case LakeSoulRelationV2(v2r) =>
          v2r
        }
        val indices = newTarget.collect { case LakeSoulTableRelationV2(tbl) =>
          tbl
        }
        if (indices.isEmpty) {
          ...
          indices.size == 1 && !indices.head.snapshotManagement.snapshot.isFirstCommit
        ) {
      ...
      case u @ UpdateTable(table, assignments, condition)
        ...
        val (cols, expressions) = assignments
          .map(a => {
            val key = a.key match {
  ...
  /** Performs the schema adjustment by adding UpCasts (which are safe) and
    * Aliases so that we can check if the by-ordinal schema of the insert query
    * matches our LakeSoul table.
    */
  private def normalizeQueryColumns(
      query: LogicalPlan,
      target: LakeSoulTableV2
  ): LogicalPlan = {
  ...
  /** With LakeSoul, we ACCEPT_ANY_SCHEMA, meaning that Spark doesn't
    * automatically adjust the schema of INSERT INTO. This allows us to perform
    * better schema enforcement/evolution. Since Spark skips this step, we see
    * if we need to perform any schema adjustment here.
    */
  ...
  /** Recursively casts structs in case it contains null types. TODO: Support
    * other complex types like MapType and ArrayType
    */
  private def addCastsToStructs(
    ...
    val fields = source.zipWithIndex.map {
      case (StructField(name, nested: StructType, _, metadata), i)
          if i < target.length =>
        target(i).dataType match {
          ...
          case o =>
            ...
            throw LakeSoulErrors.cannotInsertIntoColumn(
              tableName,
              field,
              targetName,
              o.simpleString
      ...
      case (other, i) if i < target.length =>
        ...
        Alias(
          ...
          targetAttr.name
        )(explicitMetadata = Option(targetAttr.metadata))

      case (other, i) =>
        // This is a new column, so leave to schema evolution as is. Do not lose it's name so
        // wrap with an alias
        Alias(GetStructField(parent, i, Option(other.name)), other.name)(
          explicitMetadata = Option(other.metadata)
        )
    }
...
object LakeSoulRelation {
  def unapply(plan: LogicalPlan): Option[LogicalRelation] = plan match {
    case dsv2 @ DataSourceV2Relation(d: LakeSoulTableV2, _, _, _, _) =>
