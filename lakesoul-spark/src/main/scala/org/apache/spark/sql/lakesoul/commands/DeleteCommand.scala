...
/** Performs a Delete based on the search condition
  *
  * Algorithm: 1) Scan all the files and determine which files have the rows
  * that need to be deleted. 2) Traverse the affected files and rebuild the
  * touched files. 3) Atomically write the remaining rows to new files and
      ...
      condition match {
        ...
        case Some(cond) =>
          ...
          if (otherPredicates.isEmpty) {
            ...
            val candidateFiles =
              tc.filterFiles(metadataPredicates ++ otherPredicates)
            ...
            val newTarget =
              LakeSoulUtils.replaceFileIndexV2(target, candidateFiles)
            ...
            if (filesToRewrite.isEmpty) {
              ...
              val rewriteFileInfo =
                filesToRewrite.map(f => getTouchedFile(f, nameToFileMap))

              // Keep everything from the resolved target except a new FileIndex
              // that only involves the affected files instead of all files.
              val newTarget =
                LakeSoulUtils.replaceFileIndexV2(target, rewriteFileInfo)

              val targetDF = Dataset.ofRows(sparkSession, newTarget)
              val filterCond = Not(
                EqualNullSafe(cond, Literal(true, BooleanType))
              )
              ...
              val expireFiles = removeFilesFromPaths(
                nameToFileMap,
                filesToRewrite,
                operationTimestamp
              )
