...
object SchemaUtils {
  ...
  /** Finds `StructField`s that match a given check `f`. Returns the path to the
    * column, and the field.
    *
    * @param checkComplexTypes
    *   While `StructType` is also a complex type, since we're returning
    *   StructFields, we definitely recurse into StructTypes. This flag defines
    *   whether we should recurse into ArrayType and MapType.
    */
  def filterRecursively(schema: StructType, checkComplexTypes: Boolean)(
    ...
    def recurseIntoComplexTypes(
        ...
        columnStack: Seq[String]
    ): Seq[(Seq[String], StructField)] = complexType match {
      case s: StructType =>
        s.fields.flatMap { sf =>
          val includeLevel = if (f(sf)) Seq((columnStack, sf)) else Nil
          includeLevel ++ recurseIntoComplexTypes(
            sf.dataType,
            columnStack :+ sf.name
          )
        }
  ...
  def typeExistsRecursively(dt: DataType)(f: DataType => Boolean): Boolean =
    dt match {
      case s: StructType =>
        f(s) || s.fields.exists(field =>
          typeExistsRecursively(field.dataType)(f)
        )
      case a: ArrayType =>
        f(a) || typeExistsRecursively(a.elementType)(f)
      case m: MapType =>
        f(m) || typeExistsRecursively(m.keyType)(f) || typeExistsRecursively(
          m.valueType
        )(f)
      case other =>
        f(other)
  ...
  def typeAsNullable(dt: DataType): DataType = dt match {
    ...
    case a @ ArrayType(s: StructType, _) =>
      a.copy(s.asNullable, containsNull = true)
  ...
  /** Drops null types from the DataFrame if they exist. We don't have easy ways
    * of generating types such as MapType and ArrayType, therefore if these
    * types contain NullType in their elements, we will throw an
    * AnalysisException.
    */
  def dropNullTypeColumns(df: DataFrame): DataFrame = {
    ...
    def generateSelectExpr(sf: StructField, nameStack: Seq[String]): Column =
      sf.dataType match {
        case st: StructType =>
        ...
        case a: ArrayType
            if typeExistsRecursively(a)(_.isInstanceOf[NullType]) =>
    ...
    val selectExprs = schema.flatMap { f =>
      if (f.dataType.isInstanceOf[NullType]) None
      else Some(generateSelectExpr(f, Nil))
    }
    ...
    def explode(schema: StructType): Seq[Seq[String]] = {
      ...
      schema.flatMap {
        ...
        case StructField(name, a: ArrayType, _, _) =>
          Seq(Seq(name)) ++ recurseIntoComplexTypes(a).map(nested =>
            Seq(name) ++ nested
          )
        case StructField(name, m: MapType, _, _) =>
          Seq(Seq(name)) ++ recurseIntoComplexTypes(m).map(nested =>
            Seq(name) ++ nested
          )
  ...
  /** Checks if input column names have duplicate identifiers. This throws an
    * exception if the duplication exists.
    *
    * @param schema
  ...
  /** Rewrite the query field names according to the table schema. This method
    * assumes that all schema validation checks have been made and this is the
    * last operation before writing into LakeSoulTableRel.
    */
  def normalizeColumnNames(
    ...
    if (dataFields.subsetOf(tableFields)) {
      ...
      val topLevelDataFields =
        dataFields.map(UnresolvedAttribute.parseAttributeName(_).head)
  ...
  /** As the LakeSoul snapshots update, the schema may change as well. This
    * method defines whether the new schema of a LakeSoul table can be used with
    * a previously analyzed LogicalPlan. Our rules are to return false if:
  ...
  def isReadCompatible(
    ...
    def isStructReadCompatible(
      ...
      val existingFieldNames =
        existingSchema.fieldNames.map(_.toLowerCase).toSet
      assert(
  ...
  /** Compare an existing schema to a specified new schema and return a message
    * describing the first difference found, if any:
  ...
  def reportDifferences(
    ...
    def structDifference(
      ...
      val fieldsDiffs =
        (existingFieldNames intersect specifiedFieldNames).flatMap(
          (name: String) =>
    ...
    def typeDifference(
      ...
      (existing, specified) match {
        case (e: StructType, s: StructType) =>
          structDifference(e, s, s"$field.")
    ...
    def arrayDifference(
      ...
      val nullabilityDiffs =
        if (existing.containsNull == specified.containsNull) Nil
          ...
          Seq(
            arrayNullabilityMessage(
              field,
              specified.containsNull,
              existing.containsNull
            )
          )
  ...
  /** Returns the given column's ordinal within the given `schema` and the size
    * of the last schema size. The length of the returned position will be as
    * long as how nested the column is.
    *
    * For ArrayType: accessing the array's element adds a position 0 to the
    * position list. e.g. accessing a.element.y would have the result ->
    * Seq(..., positionOfA, 0, positionOfY)
    *
    * For MapType: accessing the map's key adds a position 0 to the position
    * list. e.g. accessing m.key.y would have the result -> Seq(...,
    * positionOfM, 0, positionOfY)
    *
    * For MapType: accessing the map's value adds a position 1 to the position
    * list. e.g. accessing m.key.y would have the result -> Seq(...,
    * positionOfM, 1, positionOfY)
    *
    * @param column
    *   The column to search for in the given struct. If the length of `column`
    *   is greater than 1, we expect to enter a nested field.
    * @param schema
  ...
  def findColumnPosition(
      column: Seq[String],
      schema: StructType,
      resolver: Resolver = COL_RESOLVER
  ): (Seq[Int], Int) = {
    def find(
        column: Seq[String],
        schema: StructType,
  ...
  /** Add `column` to the specified `position` in `schema`.
    *
    * @param position
    *   A Seq of ordinals on where this column should go. It is a Seq to denote
    *   positions in nested columns (0-based). For example:
    *
    * tableSchema: <a:STRUCT<a1,a2,a3>, b,c:STRUCT<c1,c3>> column: c2 position:
    * Seq(2, 1) will return result: <a:STRUCT<a1,a2,a3>,
    * b,c:STRUCT<c1,**c2**,c3>>
    */
  def addColumn(
      schema: StructType,
    ...
    if (slicePosition < 0) {
      throw new AnalysisException(
        s"Index $slicePosition to add column $column is lower than 0"
      )
    }
    ...
    if (slicePosition == length) {
      if (position.length > 1) {
        throw new AnalysisException(
          s"Struct not found at position $slicePosition"
        )
      }
    ...
    if (position.length > 1) {
      ...
      val mid = schema(slicePosition) match {
        ...
        case StructField(
              name,
              ArrayType(f: StructType, containsNull),
              nullable,
              metadata
            ) =>
          if (!column.nullable && nullable) {
        ...
        case StructField(name, map @ MapType(_, _, _), nullable, metadata) =>
          ...
          val addedMap = (posTail.head, map) match {
            ...
            case (
                  MAP_VALUE_INDEX,
                  MapType(k, value: StructType, nullability)
                ) =>
  ...
  /** Drop from the specified `position` in `schema` and return with the
    * original column.
    *
    * @param position
    *   A Seq of ordinals on where this column should go. It is a Seq to denote
    *   positions in nested columns (0-based). For example:
    *
    * tableSchema: <a:STRUCT<a1,a2,a3>, b,c:STRUCT<c1,c2,c3>> position: Seq(2,
    * 1) will return result: <a:STRUCT<a1,a2,a3>, b,c:STRUCT<c1,c3>>
    */
  def dropColumn(
      schema: StructType,
      position: Seq[Int]
    ...
    if (slicePosition < 0) {
      throw new AnalysisException(
        s"Index $slicePosition to drop column is lower than 0"
      )
    }
    val length = schema.length
    if (slicePosition >= length) {
    ...
    if (position.length > 1) {
      val (mid, original) = schema(slicePosition) match {
        ...
        case o =>
          throw new AnalysisException(
            s"Can only drop nested columns from StructType. Found: $o"
          )
      }
      (
        StructType(pre ++ Seq(mid) ++ schema.slice(slicePosition + 1, length)),
      ...
      (
        StructType(pre ++ schema.slice(slicePosition + 1, length)),
        schema(slicePosition)
      )
    }
  }
  ...
  /** Check if the two data types can be changed.
    *
    * @return
    *   None if the data types can be changed, otherwise Some(err) containing
    *   the reason.
    */
  def canChangeDataType(
    ...
    def verifyNullability(
      ...
      verify(
        tn || !fn,
        s"tightening nullability of ${UnresolvedAttribute(columnPath).name}"
      )
    }
        ...
        case (StructType(fromFields), StructType(toFields)) =>
          ...
          verify(
            ...
            s"dropping column(s) [${remainingFields.map(_.name).mkString(", ")}]" +
              (if (columnPath.nonEmpty)
                 s" from ${UnresolvedAttribute(columnPath).name}"
               else "")
          )

        case (fromDataType, toDataType) =>
          verify(
            Cast.canCast(fromDataType, toDataType),
  ...
  /** Check whether we can write to the LakeSoul table, which has `tableSchema`,
    * using a query that has `dataSchema`. Our rules are that:
    *   - `dataSchema` may be missing columns or have additional columns
    *   - We don't trust the nullability in `dataSchema`. Assume fields are
    *     nullable.
    *   - We only allow nested StructType expansions. For all other complex
    *     types, we check for
    * strict equality
    *   - `dataSchema` can't have duplicate column names. Columns that only
    *     differ by case are also
    * not allowed. The following merging strategy is applied:
    ...
    *
    * Schema merging occurs in a case insensitive manner. Hence, column names
    * that only differ by case are not accepted in the `dataSchema`.
    */
  def mergeSchemas(
    ...
    def merge(current: DataType, update: DataType): DataType = {
      (current, update) match {
        case (StructType(currentFields), StructType(updateFields)) =>
          ...
          val newFields =
            updateFields.filterNot(f => nameToFieldMap.contains(f.name))
      ...
      val newDt = dt match {
        case StructType(fields) =>
          StructType(fields.map { field =>
            ...
            newField.copy(dataType =
              transform(path :+ newField.name, newField.dataType)
            )
          })
  ...
  /** Transform (nested) columns in a schema. Runs the transform function on all
    * nested StructTypes
    *
    * @param schema
    *   to transform.
  ...
  /** Transform (nested) columns in a schema using the given path and parameter
    * pairs. The transform function is only invoked when a field's path matches
    * one of the input paths.
    *
    * @param schema
    *   to transform
    * @param input
    *   paths and parameter pairs. The paths point to fields we want to
    *   transform. The parameters will be passed to the transform function for a
    *   matching field.
    * @param tf
    *   function to apply per matched field. This function takes the field path,
    *   the field itself and the input names and payload pairs that matched the
    *   field name. It should return a new field.
    * @tparam E
  ...
  /** Verifies that the column names are acceptable by Parquet and henceforth
    * LakeSoulTableRel. Parquet doesn't accept the characters ' ,;{}()\n\t'. We
    * ensure that neither the data columns nor the partition columns have these
    * characters.
    */
