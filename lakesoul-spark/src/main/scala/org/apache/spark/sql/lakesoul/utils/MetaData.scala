...
case class TableInfo(
  ...
  lazy val table_path: Path =
    SparkUtil.makeQualifiedTablePath(new Path(table_path_s.get))
  lazy val range_partition_columns: Seq[String] =
    range_partition_schema.fieldNames
  lazy val hash_partition_columns: Seq[String] =
    hash_partition_schema.fieldNames
  ...
  lazy val hash_partition_schema: StructType =
    if (hash_column.equalsIgnoreCase("")) {
      ...
      StructType(
        hash_column.split(LAKESOUL_HASH_PARTITION_SPLITTER).map(c => schema(c))
      )
    }

  // all partition columns
  lazy val partition_schema: StructType =
    range_partition_schema.merge(hash_partition_schema)

  // hash is belong to data_schema !!!
  private lazy val range_partition_set: Set[String] =
    range_column.split(LAKESOUL_RANGE_PARTITION_SPLITTER).toSet
  // all data schema except range partition columns
  @JsonIgnore
  lazy val data_schema: StructType = StructType(
    schema.filterNot(f => range_partition_set.contains(f.name))
  )
...
/** commit state info
  ...
  *   commit id
  * @param tag
  *   identifier to redo or rollback
  * @param timestamp
  *   timestamp of commit
  */
case class commitStateInfo(
...
case class RelationTable(
    tableName: String,
    tableId: String,
    partitionInfo: Seq[(String, String)]
) {
  override def toString: String = {
    tableName + "\001" + tableId + "\001" + partitionInfo
      .sortBy(_._1)
      .map(m => m._1 + "->" + m._2)
      .mkString("\002")
  }
