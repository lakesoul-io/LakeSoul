...
object PartitionFilter extends Logging {
  ...
  private def filterFromAllPartitionInfo(
      snapshot: Snapshot,
    ...
    val filteredParts = filterFileList(
      ...
      allPartitions,
      filters
    ).as[PartitionFilterInfo].collect()
    filteredParts.foreach(p => {
      snapshot.recordPartitionInfoRead(
        PartitionInfoScala(
          p.table_id,
          p.range_value,
          p.read_version,
    ...
    val partitionFilters = filters.flatMap { filter =>
      LakeSoulUtils
        .splitMetadataAndDataPredicates(
          filter,
          table_info.range_partition_columns,
          spark
        )
        ._1
    }
      ...
      case None       =>
        val infos = if (partitionFilters.isEmpty) {
          ...
          val newPartitionFilters = partitionFilters.filter({
            case IsNotNull(AttributeReference(name, _, _, _)) =>
              equalExprs.nonEmpty && !equalExprs.contains(name)
            case Literal(v, BooleanType) if v.asInstanceOf[Boolean] => false
            case _                                                  => true
          })

          if (
            table_info.range_partition_columns.nonEmpty && table_info.range_partition_columns
              .forall(p => {
                equalExprs.contains(p)
              })
          ) {
            // optimize for all partition equality filter
            val partDesc = table_info.range_partition_columns
              .map(p => {
                val colValue = equalExprs(p)
                s"$p=$colValue"
              })
              .mkString(DBConfig.LAKESOUL_RANGE_PARTITION_SPLITTER)
            val partInfo = SparkMetaVersion.getSinglePartitionInfo(
              table_info.table_id,
              partDesc,
              ""
            )
            logInfo(
              s"All equality partition filters $equalExprs for table ${table_info.table_path}, using" +
                s" desc $partDesc"
            )
            ...
            // using ts query
            val partQuery =
              equalExprs.map({ case (k, v) => s"$k=$v" }).mkString(" & ")
            logInfo(
              s"Partial equality partition filters $equalExprs for table ${table_info.table_path}, using" +
                s" query $partQuery"
            )
            SparkMetaVersion
              .getPartitionInfoByEqFilters(table_info.table_id, partQuery)
              .map(partInfo => {
                PartitionFilterInfo(
            ...
            logInfo(
              s"at least one non-equality filter exist $newPartitionFilters, read all partition info"
            )
            filterFromAllPartitionInfo(
              snapshot,
              table_info,
              newPartitionFilters
            )
  ...
  def filesForScan(
    ...
    if (filters.length < 1) {
      ...
      val ret =
        SparkMetaVersion.getTableDataInfoCached(partitionArray, snapshot)
      ...
      val partitionInfo = partitionFiltered
        .map(p => {
          PartitionInfoScala(
            p.table_id,
            p.range_value,
      ...
      logInfo(
        s"get table filtered partition's data info ${System.currentTimeMillis() - t0}ms"
      )
      ret
    }
  }
  ...
  def filterFileList(
    ...
    val spark = SparkSession.active
    import spark.implicits._
    val partitionsMatched = filterFileList(
      partitionSchema,
      files
        .map(f =>
          PartitionFilterInfo(
    ...
    files.filter(f =>
      partitionsMatched.exists(p => p.range_value == f.range_partitions)
    )
  }

  /** Filters the given [[Dataset]] by the given `partitionFilters`, returning
    * those that match.
    *
    * @param files
    *   The active files, which contains the partition value information
  ...
  def filterFileList(
    ...
    if (orComponents.length > 1) {
      ...
      logInfo(
        s"Processing ${orComponents.length} OR components separately for better performance"
      )
      ...
      if (orComponents.length > batchSize) {
        logInfo(
          s"Large number of OR components detected (${orComponents.length}), processing in batches of $batchSize"
        )
        ...
        val allRangeValues = batches.zipWithIndex.flatMap { case (batch, idx) =>
          logInfo(
            s"Processing batch ${idx + 1}/${batches.length} with ${batch.length} OR components"
          )
        ...
        logInfo(
          s"Found ${allRangeValues.length} unique partition values after filtering"
        )
  ...
  /** Rewrite the given `partitionFilters` to be used for filtering partition
    * values. We need to explicitly resolve the partitioning columns here
    * because the partition columns are stored as keys of a Map type instead of
    * attributes in the DataFileInfo schema (below) and thus cannot be resolved
    * automatically. e.g. (cast('range_partitions.zc as string) = ff)
    *
    * @param partitionFilters
    *   Filters on the partition columns
    ...
    partitionFilters.map(_.transformUp { case a: Attribute =>
      ...
      partitionCol match {
        case Some(StructField(name, dataType, _, _)) =>
          Cast(
            UnresolvedAttribute(
              partitionColumnPrefixes ++ Seq("range_partitions", name)
            ),
            dataType
          )
        case None =>
          ...
          UnresolvedAttribute(
            partitionColumnPrefixes ++ Seq("range_partitions", a.name)
          )
      }
    })
  }
