...
object KafkaStream {
  ...
  def topicValueToSchema(
      spark: SparkSession,
      topicAndMsg: util.Map[String, String]
      ...
      .forEach(topic => {
        ...
        schema.foreach(f =>
          f.dataType match {
            case _: StructType =>
              lakeSoulSchema =
                lakeSoulSchema.add(f.name, DataTypes.StringType, true)
            case _ =>
              lakeSoulSchema = lakeSoulSchema.add(f.name, f.dataType, true)
          }
        )
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (withSchemaRegistry) {
      ...
      spark.udf.register(
        "deserialize",
        (bytes: Array[Byte]) => kafkaAvroDeserializer.deserialize(bytes)
      )
    } else {
    ...
    val multiTopicData = if (withSchemaRegistry) {
      createStreamDF(spark).select(
        callUDF("deserialize", col("value")).as("value"),
        col("topic")
      )
    } else {
      createStreamDF(spark).selectExpr(
        "CAST(value AS STRING) as value",
        "topic"
      )
    }

    multiTopicData.writeStream
      .queryName("demo")
      .foreachBatch { (batchDF: DataFrame, _: Long) =>
        {
          ...
          if (topicList.size() > topicAndSchema.keySet.size) {
            topicAndSchema =
              topicValueToSchema(spark, getTopicMsg(topicPattern))
            createTableIfNoExists(topicAndSchema)
          }
          ...
          for (topic <- topicAndSchema.keySet) {
            ...
            if (!topicDF.rdd.isEmpty()) {
              val rows = topicDF
                .withColumn(
                  "payload",
                  from_json(col("value"), topicAndSchema.get(topic).get)
                )
                .selectExpr("payload.*")
              if (autoAddPartition) {
                val rowsWithDt =
                  rows.withColumn(LAKESOUL_PARTITION_COLUMN, lit(lakeSoulDt))
                rowsWithDt.write
                  .mode("append")
                  .format("lakesoul")
