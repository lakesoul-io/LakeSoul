...
case class PreprocessTableMergeInto(sqlConf: SQLConf)
  ...
  private def checkMergeConditionOnPrimaryKey(
      mergeCondition: Expression,
      tbl: LakeSoulTableV2
  ): Unit = {
    val hashColumnNameHit = scala.collection.mutable.Map(
      tbl.snapshotManagement.snapshot.getTableInfo.hash_partition_schema.fieldNames
        .map(k => k -> false): _*
    )
    ...
    mergeCondition foreachUp {
      case EqualTo(left: AttributeReference, right: AttributeReference)
        ...
        if (hashColumnNameHit.contains(left.name))
          hashColumnNameHit(left.name) = true
    ...
    if (notQualifiedCondition || !hashColumnNameHit.forall(_._2)) {
      throw LakeSoulErrors.operationNotSupportedException(
        s"Convert merge into to upsert with merge condition $mergeCondition",
        tbl.catalogTable.map(_.identifier)
      )
    }
  }

  private def checkLakeSoulTableHasHashPartition(
      table: LakeSoulTableV2
  ): Unit = {
    if (table.snapshotManagement.snapshot.getTableInfo.hash_column.isEmpty) {
      throw LakeSoulErrors.operationNotSupportedException(
        "Merge into none hash partitioned table",
  ...
  private def assignmentsIsAttributeOnly(
      assignments: Seq[Assignment]
  ): Boolean = {
    assignments.forall(a =>
      a.key.isInstanceOf[AttributeReference]
        && isAttribute(a.value)
    )
  }

  private def checkMatchedActionIsOneUpdateOnly(
      matchedAction: Seq[MergeAction],
      table: LakeSoulTableV2
  ): Unit = {
    matchedAction match {
      ...
      case _ =>
        throw LakeSoulErrors.operationNotSupportedException(
          s"Convert merge into to upsert with MatchedAction $matchedAction",
          table.catalogTable.map(_.identifier)
        )
    }
  }

  private def checkNotMatchedActionIsOneInsertOnly(
      notMatchedAction: Seq[MergeAction],
      table: LakeSoulTableV2
  ): Unit = {
    notMatchedAction match {
      ...
      case _ =>
        throw LakeSoulErrors.operationNotSupportedException(
          s"Convert merge into to upsert with NotMatchedAction $notMatchedAction",
          table.catalogTable.map(_.identifier)
        )
    }
  }
