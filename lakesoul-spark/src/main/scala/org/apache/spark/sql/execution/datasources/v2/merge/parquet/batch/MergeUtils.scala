...
object MergeUtils {
  ...
  /** to Buffer Iterator
    ...
    * @return
    */
  def toBufferedIterator(
      seq: Seq[(MergePartitionedFile, ColumnarBatch)]
  ): Seq[(Long, BufferedIterator[(InternalRow, Int)])] = {
    seq.map(tuple =>
      tuple._1.writeVersion -> tuple._2
        .rowIterator()
        .asScala
        .zipWithIndex
        .buffered
  ...
  def getNextBatch(
    ...
    filesInfo
      .filter(fileInfoReader => fileInfoReader._2.next())
      .map(fileInfoReader => fileInfoReader._1 -> fileInfoReader._2.get())
  }

  // initialize mergeBatchColumnIndex
  def initMergeBatchAndMergeIndex(
      fileSeq: Seq[(MergePartitionedFile, ColumnarBatch)],
      mergeColumnIndexMap: mutable.Map[Long, Array[Int]]
  ): Unit = {
    val versionWithColumnNumArr: Array[(Long, Int)] = fileSeq
      .sortWith((t1, t2) => t1._1.writeVersion < t2._1.writeVersion)
      .toArray
      .map(t => (t._1.writeVersion, t._2.numCols()))
    ...
    for (i <- versionWithColumnNumArr.indices) {
      ...
      mergeColumnIndexMap += versionWithColumnNumArr(i)._1 -> Range(
        lastLen,
        end
      ).toArray
    }
  ...
  def intBatchIndexMerge(
      resultIndex: Array[ArrayBuffer[MergeColumnIndex]]
  ): Unit = {
    for (i <- resultIndex.indices) {
