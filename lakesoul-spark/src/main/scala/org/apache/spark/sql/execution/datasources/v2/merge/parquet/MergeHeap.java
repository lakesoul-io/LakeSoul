...
/**
 * An unbounded priority {@linkplain Queue queue} based on a priority heap. The elements of the
 * priority queue are ordered according to their {@linkplain Comparable natural ordering}, or by a
 * {@link Comparator} provided at queue construction time, depending on which constructor is used. A
 * priority queue does not permit {@code null} elements. A priority queue relying on natural
 * ordering also does not permit insertion of non-comparable objects (doing so may result in {@code
 * ClassCastException}).
 *
 * <p>The <em>head</em> of this queue is the <em>least</em> element with respect to the specified
 * ordering. If multiple elements are tied for least value, the head is one of those elements --
 * ties are broken arbitrarily. The queue retrieval operations {@code poll}, {@code remove}, {@code
 * peek}, and {@code element} access the element at the head of the queue.
 *
 * <p>A priority queue is unbounded, but has an internal <i>capacity</i> governing the size of an
 * array used to store the elements on the queue. It is always at least as large as the queue size.
 * As elements are added to a priority queue, its capacity grows automatically. The details of the
 * growth policy are not specified.
 *
 ...
 * is <em>not</em> guaranteed to traverse the elements of the priority queue in any particular
 * order. If you need ordered traversal, consider using {@code Arrays.sort(pq.toArray())}.
 *
 * <p><strong>Note that this implementation is not synchronized.</strong> Multiple threads should
 * not access a {@code MergeHeap} instance concurrently if any of the threads modifies the queue.
 * Instead, use the thread-safe {@link java.util.concurrent.PriorityBlockingQueue} class.
 *
 * <p>Implementation note: this implementation provides O(log(n)) time for the enqueuing and
 * dequeuing methods ({@code offer}, {@code poll}, {@code remove()} and {@code add}); linear time
 * for the {@code remove(Object)} and {@code contains(Object)} methods; and constant time for the
 * retrieval methods ({@code peek}, {@code element}, and {@code size}).
 *
 ...
 * @since 1.5
 */
public class MergeHeap<E> extends AbstractQueue<E> implements java.io.Serializable {
    ...
    /**
     * The number of times this priority queue has been <i>structurally modified</i>. See
     * AbstractList for gory details.
     */
    transient int modCount = 0; // non-private to simplify nested class access

    /**
     * Creates a {@code MergeHeap} with the default initial capacity (11) that orders its elements
    ...
    /**
     * Creates a {@code MergeHeap} with the specified initial capacity that orders its elements
     * according to their {@linkplain Comparable natural ordering}.
     *
    ...
    /**
     * Creates a {@code MergeHeap} with the default initial capacity and whose elements are ordered
     ...
     * @throws IllegalArgumentException if {@code initialCapacity} is less than 1
     */
    public MergeHeap(int initialCapacity, Comparator<? super E> comparator) {
    ...
    /**
     * Creates a {@code MergeHeap} containing the elements in the specified collection. If the
     * specified collection is an instance of a {@link SortedSet} or is another {@code MergeHeap},
    ...
    /**
     * Creates a {@code MergeHeap} containing the elements in the specified priority queue. This
     * priority queue will be ordered according to the same ordering as the given priority queue.
     *
    ...
    /**
     * Creates a {@code MergeHeap} containing the elements in the specified sorted set. This
     * priority queue will be ordered according to the same ordering as the given sorted set.
     *
     ...
     *     currently in this priority queue according to the priority queue's ordering
     ...
     * queue. (In other words, this method must allocate a new array). The caller is thus free to
     ...
     * of this queue.
     *
     * <p>If the queue fits in the specified array with room to spare (i.e., the array has more
     * elements than the queue), the element in the array immediately following the end of the
     * collection is set to {@code null}.
     *
    ...
    private final class Itr implements Iterator<E> {
         ...
         *
         * <p>We expect that most iterations, even those involving removals, will not need to store
     ...
     *
     * <p>Normally this method leaves the elements at up to i-1, inclusive, untouched. Under these
    ...
    /**
     * Returns the comparator used to order the elements in this queue, or {@code null} if this
     * queue is sorted according to the {@linkplain Comparable natural ordering} of its elements.
     *
     ...
     * {@link Spliterator} over the elements in this queue.
     *
     * <p>The {@code Spliterator} reports {@link Spliterator#SIZED}, {@link Spliterator#SUBSIZED},
     * and {@link Spliterator#NONNULL}. Overriding implementations should document the reporting of
    ...
    static final class MergeHeapSpliterator<E> implements Spliterator<E> {
        ...
        MergeHeapSpliterator(MergeHeap<E> pq, int origin, int fence, int expectedModCount) {
            this.pq = pq;
            this.index = origin;
            this.fence = fence;
        ...
        public MergeHeapSpliterator<E> trySplit() {
            ...
            return (lo >= mid)
                    ? null
                    : new MergeHeapSpliterator<E>(pq, lo, index = mid, expectedModCount);
        }
