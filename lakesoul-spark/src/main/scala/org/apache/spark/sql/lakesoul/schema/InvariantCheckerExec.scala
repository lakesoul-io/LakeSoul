...
/** A physical operator that validates records, before they are written into
  * LakeSoulTableRel. Each row is left unchanged after validations.
  */
case class InvariantCheckerExec(child: SparkPlan, invariants: Seq[Invariant])
    extends UnaryExecNode {

  override def output: Seq[Attribute] = child.output

  private def isNullNotOkay(invariant: Invariant): Boolean =
    invariant.rule match {
      case NotNull => true
      case _       => false
    }
  ...
  private def buildExtractors(invariant: Invariant): Option[Expression] = {
    ...
    val topLevelRefOpt = output.collectFirst {
      case a: AttributeReference
          if SchemaUtils.COL_RESOLVER(a.name, topLevelColumn) =>
        a
    }
    ...
    if (invariant.column.length == 1) {
      ...
      topLevelRefOpt.flatMap { topLevelRef =>
        val boundTopLevel =
          BindReferences.bindReference[Expression](topLevelRef, output)
        try {
