...
/** A part (i.e. "block") of a single file that should be read, along with
  * partition column values that need to be prepended to each row.
  *
  ...
  */
case class MergePartitionedFile(
...
/** @param index
  *   hash key index in file result schema, such as fileResultScheme:[a,k,b],
  *   keyIndex is 1
  * @param keyType
...
/** A collection of file blocks that should be read as a single task (possibly
  * from multiple partitioned directories).
  */
case class MergeFilePartition(
    index: Int,
    files: Array[Array[MergePartitionedFile]],
    ...
    files.foreach(f =>
      f.foreach(file =>
        file.locations.filter(_ != "localhost").foreach { host =>
          hostToNumBytes(host) =
            hostToNumBytes.getOrElse(host, 0L) + file.length
        }
      )
    )
