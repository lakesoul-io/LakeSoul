...
class NativeLakeSoulColumnarOutputWriter(
    path: String,
    dataSchema: StructType,
    timeZoneId: String,
    context: TaskAttemptContext,
    physicalFormat: String = "vortex-compact"
) extends NativeLakeSoulOutputWriter(
      path,
      dataSchema,
      timeZoneId,
      context,
...
private[v2] trait NativeLakeSoulColumnarOutputWriterSupport
    extends OutputWriter {
  this: NativeLakeSoulOutputWriter =>
  ...
  private def toArrowRecordBatch(
      ...
      case _: Throwable =>
        vector match {
          ...
          case variable: BaseVariableWidthVector =>
            Array(
              variable.getValidityBuffer,
              variable.getOffsetBuffer,
              variable.getDataBuffer
            )
          case _ =>
