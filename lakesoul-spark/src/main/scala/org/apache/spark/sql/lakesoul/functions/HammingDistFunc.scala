...
class HammingDistFunc extends UnboundFunction {
  ...
  override def bind(structType: StructType): BoundFunction = {
    if (structType.fields.length != 2) {
      throw new UnsupportedOperationException(
        s"Cannot bind: ${this.name()} does not accept arguments type $structType"
      )
    }
  ...
  override def description(): String =
    "Returns hamming distance of two embedding vector"
  ...
  private class HammingDistFuncImpl extends ScalarFunction[Int] {
    ...
    override def inputTypes(): Array[DataType] =
      Array(ArrayType(LongType), ArrayType(LongType))
    ...
    override def produceResult(input: InternalRow): Int = {
      ...
      require(
        a0.length == a1.length,
        "The input sequences must have the same length"
      )
      a0.zip(a1)
        .map { zipped =>
