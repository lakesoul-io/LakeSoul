...
/** Perform UPDATE on a table
  ...
  * @param updateColumns
  *   : the to-be-updated target columns
  * @param updateExpressions
  *   : the corresponding update expression if the condition is matched
  * @param condition
  *   : Only rows that match the condition will be updated
  */
case class LakeSoulUpdate(
    child: LogicalPlan,
...
object LakeSoulUpdate {
  ...
  /** Extracts name parts from a resolved expression referring to a nested or
    * non-nested column
    *   - For non-nested column, the resolved expression will be like
    *     `AttributeReference(...)`.
    *   - For nested column, the resolved expression will be like
    *     `Alias(GetStructField(...))`.
    *
    * In the nested case, the function recursively traverses through the
    * expression to find the name parts. For example, a nested field of a.b.c
    * would be resolved to an expression
    *
    ...
    * `AttributeReference(a)` ++ Seq(b, c) -> [a, b, c]
    */
