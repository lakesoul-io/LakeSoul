...
/** List of invariants that can be defined on a LakeSoulTableRel that will allow
  * us to perform validation checks during changes to the table.
  */
object Invariants {
  ...
  case class PersistedExpression(expression: String)
      extends RulePersistedInMetadata {
  ...
  def getFromSchema(schema: StructType, spark: SparkSession): Seq[Invariant] = {
    val columns =
      SchemaUtils.filterRecursively(schema, checkComplexTypes = false) {
        field =>
    ...
    columns.map {
      ...
      case (parents, field) =>
        ...
        val invariant =
          Option(JsonUtils.mapper.readValue[PersistedRule](rule).unwrap) match {
