...
/** Preprocesses the [[LakeSoulUpdate]] logical plan before converting it to
  * [[UpdateCommand]].
  *   - Adjusts the column order, which could be out of order, based on the
  *     destination table
  *   - Generates expressions to compute the value of all target columns in
  *     LakeSoulTableRel, while taking into account that the specified SET
  *     clause may only update some columns or nested fields of columns.
  */
case class PreprocessTableUpdate(sqlConf: SQLConf)
  ...
  def toCommand(update: LakeSoulUpdate): UpdateCommand = {
    ...
    val targetColNameParts =
      update.updateColumns.map(LakeSoulUpdate.getTargetColNameParts(_))
