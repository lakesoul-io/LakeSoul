...
/** Writes out the files to `path` and returns a list of them in
  * `addedStatuses`.
  */
class DelayedCopyCommitProtocol(
    srcFiles: Seq[DataFileInfo],
    jobId: String,
    dstPath: String,
  ...
  private[lakesoul] def destinationRelativePath(
    ...
    if (srcBasePath.nonEmpty) {
      ...
      Option(srcFile.range_partitions)
        .filter(partition =>
          partition.nonEmpty && partition != LAKESOUL_NON_PARTITION_TABLE_PART_DESC
        )
  ...
  override def commitTask(
    ...
    if (srcFiles.nonEmpty) {
      val fs =
        new Path(srcFiles.head.path).getFileSystem(taskContext.getConfiguration)
      val statuses = srcFiles.map { srcFile =>
        ...
        DataFileInfo(
          ...
          fs.makeQualified(dstFile).toString,
          "add",
          status.getLen,
