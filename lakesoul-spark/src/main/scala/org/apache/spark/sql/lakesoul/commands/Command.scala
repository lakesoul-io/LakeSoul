...
trait Command {
  ...
  /** Converts string predicates into [[Expression]]s relative to a transaction.
    ...
    *   if a non-partition column is referenced.
    */
  ...
  /** Generates a map of file names to add file entries for operations where we
    * will need to rewrite files such as delete, merge, update. We expect file
    * names to be unique, because each file contains a UUID.
    */
  protected def generateCandidateFileMap(
      candidateFiles: Seq[DataFileInfo]
  ): Map[String, DataFileInfo] = {
    val nameToFileMap =
      candidateFiles.map(file => new Path(file.path).toString -> file).toMap
    assert(
      nameToFileMap.size == candidateFiles.length,
      s"File name collisions found among:\n${candidateFiles.map(_.path).mkString("\n")}"
    )
    nameToFileMap
  }

  /** Build a base relation of files that need to be rewritten as part of an
    * update/delete/merge operation.
    */
  ...
  /** Find the AddFile record corresponding to the file that was read as part of
    * a delete/update operation.
    *
    * @param filePath
  ...
  /** This method provides the RemoveFile actions that are necessary for files
    * that are touched and need to be rewritten in methods like Delete, Update.
    *
    * @param nameToFileMap
    *   A map generated using `generateCandidateFileMap`.
    * @param filesToRewrite
    *   Absolute paths of the files that were touched. We will search for these
