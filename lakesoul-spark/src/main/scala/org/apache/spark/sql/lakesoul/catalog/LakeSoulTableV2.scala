...
import org.apache.spark.sql.lakesoul.commands.{
...
case class LakeSoulTableV2(
  ...
  private lazy val (rootPath, partitionFilters) = {
    if (catalogTable.isDefined) {
      ...
      (
        SparkUtil.makeQualifiedTablePath(new Path(catalogTable.get.location)),
        Nil
      )
    } else {
  ...
  lazy val snapshotManagement: SnapshotManagement =
    SnapshotManagement(rootPath, namespace)

  override def name(): String = catalogTable
  ...
  override def schema(): StructType =
    StructType(
      snapshot.getTableInfo.data_schema ++ snapshot.getTableInfo.range_partition_schema
    )
  ...
  override def newScanBuilder(
    ...
    if (mergeOperatorInfo.getOrElse(Map.empty[String, String]).nonEmpty) {
      ...
      mergeOperatorInfo.get
        .map(_._1.replaceFirst(LakeSoulUtils.MERGE_OP_COL, ""))
        .foreach(info => {
  ...
  /** Creates a V1 BaseRelation from this Table to allow read APIs to go through
    * V1 DataSource code paths.
    */
  def toBaseRelation: BaseRelation = {
    val partitionPredicates =
      LakeSoulDataSource.verifyAndCreatePartitionFilters(
        path.toString,
  ...
  override def dropPartition(ident: InternalRow): Boolean = {
    if (mapTablePartitionSpec.containsKey(ident)) {
      ...
      val deleteFilePaths = SparkMetaVersion.dropPartitionInfoByRangeId(
        snapshot.getTableInfo.table_id,
        partitionDesc
      )
  ...
  override def listPartitionIdentifiers(
    ...
    val partitionInfoArray =
      SparkMetaVersion.getAllPartitionDesc(snapshot.getTableInfo.table_id)
...
private class WriteIntoTableBuilder(
  ...
  private val options =
    mutable.HashMap[String, String](
      writeOptions.asCaseSensitiveMap().asScala.toSeq: _*
    )
  ...
  override def overwrite(filters: Array[Filter]): WriteBuilder = {
    ...
    options.put(
      "replaceWhere",
      LakeSoulSourceUtils.translateFilters(filters).sql
    )
    forceOverwrite = true
    this
  }
    ...
    new V1Write {
      override def toInsertableRelation: InsertableRelation =
        (data: DataFrame, overwrite: Boolean) => {
          ...
          WriteIntoTable(
            snapshotManagement,
            if (forceOverwrite || overwrite) SaveMode.Overwrite
