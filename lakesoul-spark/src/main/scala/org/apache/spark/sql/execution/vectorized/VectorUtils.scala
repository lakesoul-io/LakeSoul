...
object VectorUtils {
  ...
  /** Populate a column vector with values from an internal row.
    *
    * @param col
    *   the column vector to populate
    * @param row
    *   the internal row to read from
    * @param fieldIdx
    *   the index of the field in the internal row
    */
  def populate(
    ...
    else if (t eq DataTypes.BooleanType)
      col.putBooleans(0, capacity, row.getBoolean(fieldIdx))
    else if (t eq DataTypes.BinaryType)
      ...
      t match {
        case dt: DecimalType =>
          ...
          if (dt.precision <= Decimal.MAX_INT_DIGITS)
            col.putInts(0, capacity, d.toUnscaledLong.toInt)
        ...
        case _ =>
          if (t.isInstanceOf[DateType] || t.isInstanceOf[YearMonthIntervalType])
            col
              .putInts(0, capacity, row.getInt(fieldIdx))
          else if (
            t.isInstanceOf[TimestampType] || t
              .isInstanceOf[TimestampNTZType] || t
              .isInstanceOf[DayTimeIntervalType]
          ) col.putLongs(0, capacity, row.getLong(fieldIdx))
          else
            throw new RuntimeException(
              String
                .format(
                  "DataType %s is not supported" + " in column vectorized reader.",
                  t.sql
                )
            )
      }
