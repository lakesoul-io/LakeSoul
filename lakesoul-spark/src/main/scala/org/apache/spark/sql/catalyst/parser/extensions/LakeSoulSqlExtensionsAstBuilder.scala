...
class LakeSoulSqlExtensionsAstBuilder(delegate: ParserInterface)
    extends LakeSoulSqlExtensionsBasicVisitor[AnyRef] {
  private def toSeq[T](list: java.util.List[T]): Seq[T] = toBuffer(list).toSeq
  private def toBuffer[T](
      list: java.util.List[T]
  ): scala.collection.mutable.Buffer[T] = list.asScala

  override def visitSingleStatement(
      ctx: org.apache.spark.sql.catalyst.parser.extensions.LakeSoulSqlExtensionsParser.SingleStatementContext
  ): LogicalPlan = {
    visit(ctx.statement).asInstanceOf[LogicalPlan]

  }
  override def visitCall(
      ctx: org.apache.spark.sql.catalyst.parser.extensions.LakeSoulSqlExtensionsParser.CallContext
  ): CallStatement = withOrigin(ctx) {
  ...
  override def visitPositionalArgument(
      ctx: org.apache.spark.sql.catalyst.parser.extensions.LakeSoulSqlExtensionsParser.PositionalArgumentContext
  ): CallArgument = withOrigin(ctx) {
    val expr = typedVisit[Expression](ctx.expression)
    PositionalArgument(expr)
  }
  override def visitNamedArgument(
      ctx: org.apache.spark.sql.catalyst.parser.extensions.LakeSoulSqlExtensionsParser.NamedArgumentContext
  ): NamedArgument = withOrigin(ctx) {
  ...
  override def visitExpression(
      ctx: org.apache.spark.sql.catalyst.parser.extensions.LakeSoulSqlExtensionsParser.ExpressionContext
  ): Expression = {
  ...
  override def visitMultipartIdentifier(
      ctx: org.apache.spark.sql.catalyst.parser.extensions.LakeSoulSqlExtensionsParser.MultipartIdentifierContext
