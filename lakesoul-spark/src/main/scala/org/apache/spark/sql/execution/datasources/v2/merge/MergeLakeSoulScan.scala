...
abstract class MergeDeltaLakeSoulScan(
  ...
  lazy val newFileIndex: LakeSoulFileIndexV2 = compactAndReturnNewFileIndex(
    fileIndex
  )

  val snapshotManagement: SnapshotManagement = fileIndex.snapshotManagement

  lazy val fileInfo: Seq[DataFileInfo] =
    if (SparkUtil.isPartitionVersionRead(newFileIndex.snapshotManagement)) {
  ...
  private def compactAndReturnNewFileIndex(
      oriFileIndex: LakeSoulFileIndexV2
  ): LakeSoulFileIndexV2 = {
    ...
    val minimumDeltaFiles =
      sessionConf.getConf(LakeSoulSQLConf.PART_MERGE_FILE_MINIMUM_NUM)
    val maxFiles =
      if (partitionGroupedFiles.isEmpty) 0
      else partitionGroupedFiles.map(m => m.map(_.length).max).max

    // if delta files num less equal than setting num, skip part merge and do nothing
    if (
      minimumDeltaFiles >= maxFiles || !sessionConf.getConf(
        LakeSoulSQLConf.PART_MERGE_ENABLE
      )
    ) {
      return oriFileIndex
    }

    val mergeOperatorStringInfo = options
      .keySet()
      .asScala
    ...
    val isCompactionCommand =
      options.getOrDefault("isCompaction", "false").toBoolean
  ...
  override def createReaderFactory(): PartitionReaderFactory = {
    ...
    val mergeOperatorInfo = options
      ...
      .map(k => {
        ...
        assert(
          allSchema.contains(realColName),
          s"merge column `$realColName` not found in [${allSchema.mkString(",")}]"
        )

        val mergeClass = Class
          .forName(options.get(k), true, Utils.getContextOrSparkClassLoader)
          .getConstructors()(0)
          .newInstance()
    ...
    val defaultMergeOpInfoString = sparkSession.sessionState.conf.getConfString(
      "defaultMergeOpInfo",
      "org.apache.spark.sql.execution.datasources.v2.merge.parquet.batch.merge_operator.DefaultMergeOp"
    )
    val defaultMergeOp = Class
      .forName(
        defaultMergeOpInfoString,
        true,
      ...
      .newInstance()
      .asInstanceOf[MergeOperator[Any]]
    NativeMergePartitionReaderFactory(
  ...
  protected def seqToString(seq: Seq[Any]): String =
    seq.mkString("[", ", ", "]")

  // Returns whether the two given arrays of [[Filter]]s are equivalent.
  protected def equivalentFilters(
      a: Array[Filter],
      b: Array[Filter]
  ): Boolean = {
  ...
  protected def partitions(isStreaming: Boolean): Seq[MergeFilePartition] = {
    ...
    val selectedPartitions =
      newFileIndex.listFiles(partitionFilters, dataFilters)
    logInfo(
      s"\tpartitions list files took ${System.currentTimeMillis() - t0}ms"
    )
    val partitionAttributes =
      DataTypeUtils.toAttributes(newFileIndex.partitionSchema)
    val attributeMap =
    ...
    val splitFiles = selectedPartitions.flatMap { partition =>
      ...
      val requestFilesSchemaMap = if (isStreaming) {
        val requestFilesSchema = newFileIndex
          .getFileInfoForPartitionVersion()
          .groupBy(_.range_version)
          .map(m => {
            val fileExistCols = m._2.head.file_exist_cols
              .split(LAKESOUL_FILE_EXISTS_COLUMN_SPLITTER)
            m._1 + "->" + StructType(
              requestedFields
                .filter(f =>
                  fileExistCols.contains(f) || tableInfo.hash_partition_columns
                    .contains(f)
                )
          ...
          .map(m => {
            ...
            (
              ...
              StructType(
                requestedFields
                  .filter(f =>
                    fileExistCols.contains(
                      f
                    ) || tableInfo.hash_partition_columns.contains(f)
                  )
                  .map(c => tableInfo.schema(c))
              )
            )
      ...
      val fileInfoSeq =
        if (isStreaming) newFileIndex.getFileInfoForPartitionVersion()
        else fileInfo
      ...
      val pathToFileInfoMap = fileInfoSeq
        .map(f => fs.makeQualified(new Path(f.path)).toString -> f)
        .toMap
      ...
      partition.files.flatMap { file =>
        ...
        val touchedFileInfo = pathToFileInfoMap.getOrElse(
          qualifiedPath,
          throw LakeSoulErrors.filePathNotFoundException(
            qualifiedPath,
            dataInfoPath
          )
        )
    ...
    if (splitFiles.length == 1) {
      ...
      if (
        !isSplittable(path) && splitFiles.head.length >
          sparkSession.sparkContext.getConf.get(IO_WARNING_LARGEFILETHRESHOLD)
      ) {
        logWarning(
          s"Loading one large unsplittable file ${path.toString} with only one " +
            s"partition, the reason is: ${getFileUnSplittableReason(path)}"
        )
      }
    }

    val ret = getFilePartitions(
      sparkSession.sessionState.conf,
      splitFiles,
...
case class OnePartitionMergeBucketScan(
    sparkSession: SparkSession,
    hadoopConf: Configuration,
    fileIndex: LakeSoulFileIndexV2,
    dataSchema: StructType,
  ...
  override def getFilePartitions(
    ...
    val fileNumLimit = options
      .getOrDefault(SCAN_FILE_NUMBER_LIMIT.key, Int.MaxValue.toString)
      .toInt
    val isCompactionTask = options
      .getOrDefault(COMPACTION_TASK.key, COMPACTION_TASK.defaultValueString)
      .toBoolean
    ...
    Seq.tabulate(bucketNum) { bucketId =>
      ...
      for (index <- groupedFiles.indices) {
        ...
        if (!isSingleFile) {
          val versionFiles =
            for (elem <- groupedFiles(index).indices)
              yield groupedFiles(index)(elem).copy(writeVersion = elem)
...
case class MultiPartitionMergeBucketScan(
    sparkSession: SparkSession,
    hadoopConf: Configuration,
    fileIndex: LakeSoulFileIndexV2,
    dataSchema: StructType,
  ...
  override def getFilePartitions(
    ...
    val fileWithBucketId: Map[Int, Map[String, Seq[MergePartitionedFile]]] =
      partitionedFiles
        .groupBy(_.fileBucketId)
        .map(f => (f._1, f._2.groupBy(_.rangeKey)))

    Seq.tabulate(bucketNum) { bucketId =>
      val files = fileWithBucketId
        .getOrElse(bucketId, Map.empty[String, Seq[MergePartitionedFile]])
        .map(_._2.toArray)
        .toArray
      ...
      for (index <- files.indices) {
        ...
        if (!isSingleFile) {
          val versionFiles =
            for (elem <- files(index).indices)
              yield files(index)(elem).copy(writeVersion = elem)
...
case class MultiPartitionMergeScan(
    sparkSession: SparkSession,
    hadoopConf: Configuration,
    fileIndex: LakeSoulFileIndexV2,
    dataSchema: StructType,
    ...
    groupByPartition.foreach(p => {
      p._2
        ...
        .foreach(g => {
          ...
          if (!isSingleFile) {
            val versionFiles =
              for (version <- files.indices)
                yield files(version).copy(writeVersion = version)
