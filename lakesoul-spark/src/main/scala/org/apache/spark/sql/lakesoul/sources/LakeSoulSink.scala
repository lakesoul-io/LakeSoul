...
class LakeSoulSink(
  ...
  def writeBatch(batchId: Long, data: DataFrame): Unit =
    snapshotManagement.withNewTransaction(tc => {
      ...
      if (
        SchemaUtils.typeExistsRecursively(data.schema)(_.isInstanceOf[NullType])
      ) {
      ...
      if (StreamingRecord.getBatchId(tableInfo.table_id, queryId) >= batchId) {
        logInfo(
          s"== Skipping already complete batch $batchId, in query $queryId"
        )
        return
      }
      ...
      // clean shuffle data
      val map = sqlContext.sparkContext.env.mapOutputTracker
        .asInstanceOf[MapOutputTrackerMaster]
        .shuffleStatuses
      map.keys.foreach(shuffleId => {
        sqlContext.sparkContext.cleaner.get
          .doCleanupShuffle(shuffleId, blocking = false)
      })
  ...
  override def toString: String =
    s"LakeSoulSink[${snapshotManagement.table_path}]"
}
