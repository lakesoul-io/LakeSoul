...
case class SetPartitionAndOrdering(session: SparkSession)
  ...
  override def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    ...
    case PhysicalOperation(
      ...
      val hashKeys = bucketScan.tableInfo.hash_partition_columns.flatMap(key =>
        output.find(_.name == key)
      )
      ...
      val child = withProjectAndFilter(
        project,
        filters,
        batchExec,
    ...
    case PhysicalOperation(
          ...
          relation @ DataSourceV2ScanRelation(
            DataSourceV2Relation(tbl: LakeSoulTableV2, _, _, _, _),
            mergeScan @ OnePartitionMergeBucketScan(
              _,
              _,
              _,
              _,
      ...
      val hashKeys = tableInfo.hash_partition_columns.flatMap(key =>
        output.find(_.name == key)
      )
      ...
      val isCompaction =
        options.getOrDefault("isCompaction", "false").equals("true")

      val outputOrdering = if (isCompaction) {
        val rangeKeys = tableInfo.range_partition_columns.flatMap(key =>
          output.find(_.name == key)
        )
        (rangeKeys ++
          Seq(
    ...
    case PhysicalOperation(
          ...
          relation @ DataSourceV2ScanRelation(
            DataSourceV2Relation(tbl: LakeSoulTableV2, _, _, _, _),
            mergeScan @ MultiPartitionMergeBucketScan(
              _,
              _,
              _,
              _,
...
case class withPartition(partition: Partitioning, child: SparkPlan)
