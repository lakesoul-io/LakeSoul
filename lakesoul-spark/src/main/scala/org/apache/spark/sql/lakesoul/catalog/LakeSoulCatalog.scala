...
/** A Catalog extension which can properly handle the interaction between the
  * HiveMetaStore and lakesoul tables. It delegates all operations DataSources
  * other than lakesoul to the SparkCatalog.
  */
class LakeSoulCatalog(val spark: SparkSession)
  ...
  /** Creates a lakesoul table
    ...
    * @param properties
    *   The table properties. Right now it also includes write options for
    *   backwards compatibility
    * @param sourceQuery
    *   A query if this CREATE request came from a CTAS or RTAS
    * @param operation
    *   The specific table creation mode, whether this is a
    *   Create/Replace/Create or Replace
    */
  private def createLakeSoulTable(
      ident: Identifier,
      schema: StructType,
    ...
    val (location, existingLocation) = if (isByPath) {
      (
        Option(ident.name()),
        if (LakeSoulSourceUtils.isLakeSoulTableExists(ident.name()))
          Some(ident.name())
        else None
      )
    } else {
    ...
    val storage = DataSource
      .buildStorageFormatFromOptions(tableProperties.toMap)
      .copy(locationUri = location.map(CatalogUtils.stringToURI))
    val tableType =
      if (location.isDefined) CatalogTableType.EXTERNAL
  ...
  override def loadTable(identifier: Identifier): Table = {
    val ident = identifier.namespace() match {
      case Array() =>
        Identifier.of(LakeSoulCatalog.showCurrentNamespace(), identifier.name())
      case _ => identifier
    }
      ...
      LakeSoulTableV2(
        ...
        Some(
          Identifier
            .of(
              ident.namespace(),
              tableInfo.short_table_name
                .getOrElse(tableInfo.table_path.toUri.toString)
            )
            .toString
        )
      )
    } else if (isNameIdentifier(ident)) {
      val tablePath = SparkMetaVersion.getTablePathFromShortTableName(
        ident.name,
        ident.namespace().mkString(".")
      )
      if (tablePath == null) {
  ...
  override def createTable(
    ...
    if (LakeSoulSourceUtils.isLakeSoulDataSourceName(getProvider(properties))) {
      ...
      throw LakeSoulErrors.analysisException(
        s"Not a lakesoul table: ${getProvider(properties)}",
        plan = None
      )
    }
  }
  ...
  override def stageCreateOrReplace(
    ...
    val iden = ident.namespace() match {
      case Array(_) => ident
      case Array()  =>
        Identifier.of(LakeSoulCatalog.showCurrentNamespace(), ident.name())
    }
    if (LakeSoulSourceUtils.isLakeSoulDataSourceName(getProvider(properties))) {
  ...
  /** Performs checks on the parameters provided for table creation for a
    * LakeSoul table.
    */
  private def verifyTableAndSolidify(
    ...
    if (tableDesc.bucketSpec.isDefined) {
      throw LakeSoulErrors.operationNotSupportedException(
        "Bucketing",
        Some(tableDesc.identifier)
      )
    }

    val ori_schema = query
      .map { plan =>
    ...
    val schema = StructType(
      ori_schema.map { case StructField(name, dataType, nullable, metadata) =>
    ...
    PartitioningUtils.validatePartitionColumn(
      ...
      caseSensitive = false
    ) // lakesoul is case insensitive

    val validatedConfigurations =
      LakeSoulConfig.validateConfigurations(tableDesc.properties)

    val db = tableDesc.identifier.database.getOrElse(
  ...
  private class StagedLakeSoulTableV2(
    ...
    override def newWriteBuilder(info: LogicalWriteInfo): WriteBuilder = {
      ...
      // them
      val combinedProps =
        info.options.asCaseSensitiveMap().asScala ++ properties.asScala
  ...
  override def alterTable(ident: Identifier, changes: TableChange*): Table = {
    ...
    val columnUpdates =
      new mutable.HashMap[Seq[String], (StructField, Option[ColumnPosition])]()

    grouped.foreach {
      case (t, newColumns) if t == classOf[AddColumn] =>
        AlterTableAddColumnsCommand(
          table,
          newColumns.map(_.asInstanceOf[AddColumn])
        ).run(spark)
      ...
      case (t, columnChanges) if classOf[ColumnChange].isAssignableFrom(t) =>
        ...
        ): (StructField, Option[ColumnPosition]) = {
          columnUpdates.getOrElseUpdate(
            fieldNames, {
        ...
        columnChanges.foreach {
          case comment: UpdateColumnComment =>
            ...
            columnUpdates(field) =
              oldField.withComment(comment.newComment()) -> pos

          case dataType: UpdateColumnType =>
            val field = dataType.fieldNames()
            val (oldField, pos) = getColumn(field)
            columnUpdates(field) =
              oldField.copy(dataType = dataType.newDataType()) -> pos
          ...
          case nullability: UpdateColumnNullability =>
            ...
            val (oldField, pos) = getColumn(field)
            columnUpdates(field) =
              oldField.copy(nullable = nullability.nullable()) -> pos
          ...
          case other =>
            throw LakeSoulErrors.operationNotSupportedException(
              "Unrecognized column change " + other.getClass
            )
        }

      case (t, _) if t == classOf[SetLocation] =>
        throw LakeSoulErrors.operationNotSupportedException(
          "ALTER TABLE xxx SET LOCATION '/xxx'"
        )
    }
  ...
  private case class BestEffortStagedTable(
    ...
    override def newWriteBuilder(info: LogicalWriteInfo): WriteBuilder =
      table match {
        ...
        case _                            =>
          throw new AnalysisException(
            s"Table implementation does not support writes: $name"
          )
      }
  }
  ...
  override def dropTable(ident: Identifier): Boolean = {
    if (isPathIdentifier(ident)) {
      ...
      LakeSoulTable
        .forName(ident.name(), ident.namespace().mkString("."))
        .dropTable()
    } else {
      false
    }
  ...
  override def loadNamespaceMetadata(
    ...
    namespace match {
      case Array(db) =>
        if (!LakeSoulCatalog.namespaceExists(namespace))
  ...
  override def alterNamespace(
    ...
    throw LakeSoulErrors.operationNotSupportedException(
      "LakeSoul currently doesn't support rename namespace"
    )
  }
...
/** A trait for handling table access through lakesoul.`/some/path`. This is a
  * stop-gap solution until PathIdentifiers are implemented in Apache Spark.
  */
trait SupportsPathIdentifier extends TableCatalog {
  ...
  private def hasLakeSoulNamespace(ident: Identifier): Boolean = {
    ident.namespace().length == 1 && LakeSoulCatalog.namespaceExists(
      ident.namespace()
    )
  }
  ...
  protected def isPathIdentifier(ident: Identifier): Boolean = {
    ...
    try {
      supportSQLOnFile && hasLakeSoulNamespace(ident) && new Path(
        ident.name()
      ).isAbsolute
    } catch {
  ...
  protected def isNameIdentifier(ident: Identifier): Boolean = {
    ident.namespace() match {
      ...
      case _ =>
        SparkMetaVersion
          .isShortTableNameExists(ident.name(), ident.namespace().mkString("."))
          ._1
    }
  }
  ...
  protected def isPathIdentifier(table: CatalogTable): Boolean = {
    isPathIdentifier(
      Identifier.of(table.identifier.database.toArray, table.identifier.table)
    )
  }
  ...
  override def tableExists(ident: Identifier): Boolean = {
    if (isPathIdentifier(ident)) {
      ...
      LakeSoulSourceUtils.isLakeSoulShortTableNameExists(
        ident.name(),
        ident.namespace().mkString(".")
      )
    } else {
...
object LakeSoulCatalog {
  ...
  def listTables(namespaces: Array[String]): Array[Identifier] = {
    SparkMetaVersion
      .listTables(namespaces)
      .asScala
      .flatMap { tablePath =>
        Option(SparkMetaVersion.getTableInfoByPath(tablePath)).map {
          tableInfo =>
            Identifier.of(
              namespaces,
              tableInfo.short_table_name
