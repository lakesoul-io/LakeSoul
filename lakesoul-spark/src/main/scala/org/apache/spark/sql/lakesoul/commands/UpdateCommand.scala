...
/** Performs an Update using `updateExpression` on the rows that match
  * `condition`
  *
  * Algorithm: 1) Identify the affected files, i.e., the files that may have the
  * rows to be updated. 2) Scan affected files, apply the updates, and generate
  * a new DF with updated rows. 3) Atomically write the new DF as new files and
  * remove the affected files that are identified in step 1.
  */
case class UpdateCommand(
  ...
  private def performUpdate(
    ...
    val (metadataPredicates, dataPredicates) =
      LakeSoulUtils.splitMetadataAndDataPredicates(
        updateCondition,
        tc.tableInfo.range_partition_columns,
        sparkSession
      )
    ...
    //
    val (addFiles, expireFiles): (Seq[DataFileInfo], Seq[DataFileInfo]) =
      if (candidateFiles.isEmpty) {
        ...
        if (filesToRewrite.isEmpty) {
          ...
          val operationTimestamp = System.currentTimeMillis()
          val deleteFiles =
            removeFilesFromPaths(nameToFile, filesToRewrite, operationTimestamp)
          val rewrittenFiles = rewriteFiles(
            sparkSession,
            tc,
            filesToRewrite,
  ...
  private def rewriteFiles(
    ...
    val rewriteFileInfo =
      inputLeafFiles.map(f => getTouchedFile(f, nameToFileMap))
  ...
  /** Build the new columns. If the condition matches, generate the new value
    * using the corresponding UPDATE EXPRESSION; otherwise, keep the original
    * column value
    */
