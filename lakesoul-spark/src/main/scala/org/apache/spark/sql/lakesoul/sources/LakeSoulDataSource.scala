...
class LakeSoulDataSource
  ...
  override def createSink(
    ...
    val snapshot = SnapshotManagement(
      SparkUtil.makeQualifiedTablePath(new Path(path)).toString,
    ...
    if (outputMode == OutputMode.Update()) {
      if (
        tableInfo.hash_column.isEmpty && parameters
          .getOrElse("hashpartitions", "")
          .isEmpty
      ) {
        throw LakeSoulErrors.outputModeNotSupportedException(
          getClass.getName,
          outputMode
        )
      }
    ...
    val options =
      new LakeSoulOptions(newParam, sqlContext.sparkSession.sessionState.conf)
    new LakeSoulSink(
      sqlContext,
      SparkUtil.makeQualifiedTablePath(new Path(path)),
      outputMode,
      options
    )
  }
  ...
  override def createRelation(
    ...
    val snapshot_manage = SnapshotManagement(
      SparkUtil.makeQualifiedTablePath(new Path(path)).toString,
      LakeSoulCatalog.showCurrentNamespace().mkString(".")
    )
  ...
  override def getTable(
    ...
    def getSnapshotOptions(
        options: CaseInsensitiveStringMap
    ): (String, Long, Long, String) = {
      val partitionDesc =
        if (options.containsKey(LakeSoulOptions.PARTITION_DESC)) {
      ...
      val readType =
        options.getOrDefault(LakeSoulOptions.READ_TYPE, ReadType.FULL_READ)
      ...
      val startVersion =
        if (readType.equals(ReadType.INCREMENTAL_READ))
          getSnapshotTimestamp(
            options.getOrDefault(LakeSoulOptions.READ_START_TIME, "")
          )
        else 0
      var endVersion = getSnapshotTimestamp(
        options.getOrDefault(LakeSoulOptions.READ_END_TIME, "")
      )
    ...
    if (options.containsKey(LakeSoulOptions.READ_TYPE)) {
      ...
      lakeSoulTable.snapshotManagement.updateSnapshotForVersion(
        snapshotOptions._1,
        snapshotOptions._2,
        snapshotOptions._3,
        snapshotOptions._4
...
object LakeSoulDataSource extends Logging {
  ...
  /** For LakeSoulTableRel, we allow certain magic to be performed through the
    * paths that are provided by users. Normally, a user specified path should
    * point to the root of a LakeSoulTableRel. However, some users are used to
    * providing specific partition values through the path, because of how
    * expensive it was to perform partition discovery before. We treat these
    * partition values as logical partition filters, if a table does not exist
    * at the provided path.
    *
    * In addition, we allow users to provide time travel specifications through
    * the path. This is provided after an `@` symbol after a path followed by a
    * time specification in `yyyyMMddHHmmssSSS` format, or a version number
    * preceded by a `v`.
    *
    * This method parses these specifications and returns these modifiers only
    * if a path does not really exist at the provided path. We first parse out
    * the time travel specification, and then the partition filters. For
    * example, a path specified as: /some/path/partition=1@v1234 will be parsed
    * into `/some/path` with filters `partition=1` and a time travel spec of
    * version 1234.
    *
    * @return
    *   A tuple of the root path of the LakeSoulTableRel, partition filters, and
    *   time travel options
    */
  def parsePathIdentifier(
    ...
    val rootPath =
      LakeSoulUtils.findTableRootPath(spark, hadoopPath).getOrElse {
    ...
    val partitionFilters = if (rootPath != hadoopPath) {
      ...
      val fragment =
        hadoopPath.toString.substring(rootPath.toString.length() + 1)
      try {
  ...
  /** Verifies that the provided partition filters are valid and returns the
  ...
  def verifyAndCreatePartitionFilters(
    ...
    if (partitionFilters.nonEmpty) {
      ...
      val badColumns = partitionFilters
        .map(_._1)
        .filterNot(table_info.range_partition_columns.contains)
      if (badColumns.nonEmpty) {
        val fragment =
          partitionFilters.map(f => s"${f._1}=${f._2}").mkString("/")
        throw LakeSoulErrors.partitionPathInvolvesNonPartitionColumnException(
          badColumns,
          fragment
        )
      }
