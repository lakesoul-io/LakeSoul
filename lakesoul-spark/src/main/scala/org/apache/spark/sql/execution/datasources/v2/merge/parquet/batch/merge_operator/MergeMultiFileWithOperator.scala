...
import org.apache.spark.sql.execution.datasources.v2.merge.parquet.batch.{
  MergeLogic,
...
/** @param filesInfo
  *   (file, columnar reader of file)
  */
class MergeMultiFileWithOperator(
  ...
  /** Get the key information of each merge file And build it into a Map.
    * Map[version->keyInfo] keyInfo:Array[(Int,DataType)]
    */
  var versionKeyInfoMap: Map[Long, Array[KeyIndex]] = filesInfo
    .map(info => {
      info._1.writeVersion -> info._1.keyInfo.toArray
    })
    .toMap

  /** The corresponding index of each field in mergeRow. Map:[fieldName->index]
    * , index: Indicates the position of the field after the merge
    */
  val fieldIndexMap: Map[String, Int] = resultSchema.zipWithIndex
    .map(m => {
      m._1.fieldName -> m._2
    })
    .toMap

  /** Replace the field name with the index. The order of the array elements
    * represents the order of the entire mergeRow elements。
    */
  val indexTypeArray: Seq[FieldIndex] =
    resultSchema.map(fieldInfo =>
      FieldIndex(fieldIndexMap(fieldInfo.fieldName), fieldInfo.fieldType)
    )

  /** Get the file information of each merge file And build it into a Map.
    * Map[writeVersion -> Array(FieldIndex)]
    */
  val versionFileInfoMap: Map[Long, Array[FieldIndex]] = filesInfo
    .map(t => {
      t._1.writeVersion ->
        t._1.fileInfo
          .map(fieldInfo =>
  ...
  private val resultIndex =
    new Array[ArrayBuffer[MergeColumnIndex]](resultSchema.length)
...
//  private val defaultMergeOp = if(defaultMergeOpInfo == null) new DefaultMergeOp[Any] else defaultMergeOpInfo
  ...
  var temporaryRow: Array[ArrayBuffer[Any]] =
    new Array[ArrayBuffer[Any]](resultSchema.length)
  MergeUtils.initTemporaryRow(temporaryRow)

  // get next batch
  val fileSeq: Seq[(MergePartitionedFile, ColumnarBatch)] =
  ...
  val mergeColumnarBatch: MergeColumnarBatchNew =
    MergeUtils.initMergeBatchNew(fileSeq, mergeOp, indexTypeArray)
  ...
  def merge(): Unit = {
    ...
    while (mergeHeap.nonEmpty) {
      ...
      if (StringUtils.isEmpty(lastKey)) {
        ...
        if (
          !combineKey(currentVersion, currentRowAndLineId._1).equals(lastKey)
        ) {
      ...
      if (currentFile._2.hasNext) {
        ...
        val fileInfo =
          filesInfo.filter(t => t._1.writeVersion.equals(currentVersion))
  ...
  def storeRowByMergeBatch(row: MergeOperatorColumnarBatchRow): Unit = {
    for (i <- resultIndex.indices) {
      if (resultIndex(i).nonEmpty) {
        ...
        getMergeValuesByType(row, i, fieldIndex.filedType).foreach(
          temporaryRow(i) += _
        )
      }
  ...
  def storeRow(version: Long, row: InternalRow): Unit = {
    for (i <- versionFileInfoMap(version).indices) {
      ...
      if (lastVersion == version) {
        ...
        temporaryRow(fieldIndex.index) = temporaryRow(
          fieldIndex.index
        ).init += getValueByType(row, i, fieldIndex.filedType)
      } else {
        temporaryRow(fieldIndex.index) += getValueByType(
          row,
          i,
          fieldIndex.filedType
        )
      }
    }
  }

  def fillMergeBatchIndex(
      rowAndId: (InternalRow, Int),
      writerVersion: Long
  ): Unit = {
    ...
    columns.indices.foreach(i => {
      if (lastVersion == writerVersion) {
        // it has duplicate data in one file, we just store the last one
        // seq.init() == list.removeLast()
        resultIndex(columns(i).index) = resultIndex(
          columns(i).index
        ).init += MergeColumnIndex(mergeBatchIndex(i), rowAndId._2)
      } else {
        resultIndex(columns(i).index) += MergeColumnIndex(
          mergeBatchIndex(i),
          rowAndId._2
        )
      }
  ...
  def getMergeValuesByType(
      row: MergeOperatorColumnarBatchRow,
      fieldIndex: Int,
      dataType: DataType
  ): Seq[Any] = {
    dataType match {
      ...
      case t: DecimalType           =>
        row.getMergeDecimal(fieldIndex, t.precision, t.scale)
  ...
  def getStoredMergeRowByType(
      row: ArrayBuffer[Any],
      mergeClass: MergeOperator[Any],
      dataType: DataType
  ): Any = {
    if (row.isEmpty) {
      return null
    ...
    dataType match {
      case StringType =>
        UTF8String.fromString(
          mergeClass
            .asInstanceOf[MergeOperator[String]]
            .mergeData(
              row
                .asInstanceOf[Seq[UTF8String]]
                .map(r => if (r == null) null else r.toString)
            )
        )
      case IntegerType | DateType =>
        mergeClass
          .asInstanceOf[MergeOperator[Int]]
          .mergeData(row.asInstanceOf[Seq[Int]])
      case BooleanType =>
        mergeClass
          .asInstanceOf[MergeOperator[Boolean]]
          .mergeData(row.asInstanceOf[Seq[Boolean]])
      case ByteType =>
        mergeClass
          .asInstanceOf[MergeOperator[Byte]]
          .mergeData(row.asInstanceOf[Seq[Byte]])
      case ShortType =>
        mergeClass
          .asInstanceOf[MergeOperator[Short]]
          .mergeData(row.asInstanceOf[Seq[Short]])
      case LongType | TimestampType =>
        mergeClass
          .asInstanceOf[MergeOperator[Long]]
          .mergeData(row.asInstanceOf[Seq[Long]])
      case FloatType =>
        mergeClass
          .asInstanceOf[MergeOperator[Float]]
          .mergeData(row.asInstanceOf[Seq[Float]])
      case DoubleType =>
        mergeClass
          .asInstanceOf[MergeOperator[Double]]
          .mergeData(row.asInstanceOf[Seq[Double]])
      case BinaryType =>
        mergeClass
          .asInstanceOf[MergeOperator[Array[Byte]]]
          .mergeData(row.asInstanceOf[Seq[Array[Byte]]])
      case CalendarIntervalType =>
        mergeClass
          .asInstanceOf[MergeOperator[CalendarInterval]]
          .mergeData(row.asInstanceOf[Seq[CalendarInterval]])
      case t: DecimalType =>
        mergeClass
          .asInstanceOf[MergeOperator[Decimal]]
          .mergeData(row.asInstanceOf[Seq[Decimal]])
      case t: StructType =>
        mergeClass
          .asInstanceOf[MergeOperator[InternalRow]]
          .mergeData(row.asInstanceOf[Seq[InternalRow]])
      case _: ArrayType =>
        mergeClass
          .asInstanceOf[MergeOperator[ArrayData]]
          .mergeData(row.asInstanceOf[Seq[ArrayData]])
      case _: MapType =>
        mergeClass
          .asInstanceOf[MergeOperator[MapData]]
          .mergeData(row.asInstanceOf[Seq[MapData]])
      case o =>
        throw new UnsupportedOperationException(
          s"LakeSoul MergeOperator don't support type ${o.typeName}"
        )
...
/** @param index
  *   filed index in result schema, such as ResultScheme:[a,k,b], field a is 0,
  *   k is 1,b is 2
  * @param filedType
  *   DataType of key field
  */
...
/** Construct an index to locate a value in column vector array of all files.
  *
  * @param columnVectorIndex
  *   index of column vector array of all files
  * @param rowIndex
  *   index of row in a column vector
  */
