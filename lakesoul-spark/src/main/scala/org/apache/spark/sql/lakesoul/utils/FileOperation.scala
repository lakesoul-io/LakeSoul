...
object FileOperation extends Logging {
  ...
  /** Create an absolute path from `child` using the `basePath` if the child is
    * a relative path. Return `child` if it is an absolute path.
    *
    * @param basePath
    *   Base path to prepend to `child` if child is a relative path. Note: It is
    *   assumed that the basePath do not have any escaped characters and is
    *   directly readable by Hadoop APIs.
    * @param child
    *   Child path to append to `basePath` if child is a relative path. Note: t
    *   is assumed that the child is escaped, that is, all special chars that
    *   need escaping by URI standards are already escaped.
    * @return
    *   Absolute path without escaped chars that is directly readable by Hadoop
    *   APIs.
    */
  ...
  /** Given a path `child`:
    ...
    * a) If the `child` doesn't live within the same base path, returns `child`
    * as is b) If `child` lives in a different FileSystem, throws an exception
    * Note that `child` may physically be pointing to a path within `basePath`,
    * but may logically belong to a different FileSystem, e.g. DBFS mount points
    * and direct S3 paths.
    */
  def tryRelativizePath(fs: FileSystem, basePath: Path, child: Path): Path = {
    ...
    if (child.isAbsolute) {
      try {
        new Path(
          fs.makeQualified(basePath)
            .toUri
            .relativize(fs.makeQualified(child).toUri)
        )
      } catch {
  ...
  private def isThrottlingError(t: Throwable): Boolean = {
    Option(t.getMessage).exists(
      _.toLowerCase(Locale.ROOT).contains("slow down")
    )
  }
  ...
  /** The default filter for hidden files. Files names beginning with _ or . are
    * considered hidden.
    *
    * @param fileName
    * @return
  ...
  /** Returns all the levels of sub directories that `path` has with respect to
    * `base`. For example: getAllSubDirectories("/base", "/base/a/b/c") =>
