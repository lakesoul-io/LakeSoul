...
case class ProcessCDCTableMergeOnRead(sqlConf: SQLConf)
    extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan =
    plan resolveOperatorsDown {
      case p: LogicalPlan
          if p.children.exists(_.isInstanceOf[DataSourceV2Relation]) && !p
            .isInstanceOf[Filter] =>
          ...
          case dsv2 @ DataSourceV2Relation(
            ...
            if (value.nonEmpty) {
              if (incremental.equals(ReadType.INCREMENTAL_READ)) {
                ...
                p.withNewChildren(
                  Filter(
                    Column(expr(s" ${value.get}!= 'delete'").expr).expr,
                    dsv2
                  ) :: Nil
                )
              }
      ...
      case p: LogicalPlan
        ...
        p.children.find(_.isInstanceOf[DataSourceV2Relation]).get match {
          case dsv2 @ DataSourceV2Relation(
            ...
            val incremental = isIncrementalRead(table)
            if (value.nonEmpty) {
              val isDeleteStatement = p.expressions.forall(s =>
                s.toString().contains(value.get) && s
                  .toString()
                  .contains("delete")
              )
              if (incremental.equals(ReadType.INCREMENTAL_READ)) {
  ...
  private def getLakeSoulTableCDCColumn(
      table: LakeSoulTableV2
  ): Option[String] = {
    table.snapshotManagement.snapshot.getTableInfo.configuration
      .get(LakeSoulTableProperties.lakeSoulCDCChangePropKey)
  }
