...
import org.apache.spark.sql.execution.datasources.v2.merge.parquet.batch.merge_operator.{
  MergeOperator,
  MergeParquetFileWithOperatorPartitionByBatchFile
}
import org.apache.spark.sql.execution.datasources.v2.merge.{
  MergeFilePartition,
  MergeFilePartitionReader,
  MergePartitionedFile,
  MergePartitionedFileReader
}
import org.apache.spark.sql.vectorized.ColumnarBatch

abstract class NativeMergeFilePartitionReaderFactory(
  ...
  override def createReader(
    ...
    val mergeReader =
      if (filePartition.isSingleFile) {
        ...
        new MergeParquetFileWithOperatorPartitionByBatchFile[InternalRow](
          iter,
          mergeOperatorInfo,
          defaultMergeOp
        )
    ...
    new MergeFilePartitionReader[InternalRow](
      Iterator(
        MergePartitionedFileReader( // filePartition.files.head,
          mergeReader
        )
      )
    )
  ...
  def buildReader(
      partitionedFile: MergePartitionedFile
  ): PartitionReader[InternalRow]

  def buildColumnarReader(
      partitionedFile: Seq[MergePartitionedFile]
  ): PartitionReader[ColumnarBatch]
