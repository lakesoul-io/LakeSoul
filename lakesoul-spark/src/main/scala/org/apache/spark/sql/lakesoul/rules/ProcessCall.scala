...
case class ProcessCall(spark: SparkSession) extends Rule[LogicalPlan] {
    ...
    case CallStatement(ident, args) =>
      if (!checkLakeSoulActions(ident)) {
        throw new AnalysisException(
          "Please check. Just support LakeSoulTable.rollback(xx) and LakeSoulTable.compaction(xx)"
        )
      }
  ...
  private def checkLakeSoulActions(ident: Seq[String]): Boolean = {
    ...
    if (
      ident(0).equalsIgnoreCase("lakesoultable") && (ident(1).equalsIgnoreCase(
        "rollback"
      ) || ident(1).equalsIgnoreCase("compaction"))
    ) {
      true
    } else {
      false
  ...
  private def normalizeArgs(args: Seq[CallArgument]): Seq[CallArgument] = {
    args.map {
      case a @ NamedArgument(name, _) =>
        a.copy(name = name.toLowerCase(Locale.ROOT))
      case other => other
    }
  }
}
