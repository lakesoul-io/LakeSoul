...
object CleanUtils {
  ...
  def createStructField(name: String, colType: String): StructField = {
    colType match {
      ...
      case "java.sql.Timestamp" =>
        StructField(name, TimestampType, nullable = true)
      case "java.math.BigDecimal" =>
        StructField(name, DecimalType(10, 0), nullable = true)
  ...
  /** Convert the detected ResultSet into a DataFrame
    */
  def createResultSetToDF(
      rs: ResultSet,
      sparkSession: SparkSession
  ): DataFrame = {
    println(
      System.currentTimeMillis() + "__________ get rs and begin createResultSetToDF __________"
    )
    ...
    for (i <- 1 to rsmd.getColumnCount) {
      ...
      rowSchemaList.add(
        createStructField(rsmd.getColumnName(i), rsmd.getColumnClassName(i))
      )
    }
    ...
    var totalDF =
      sparkSession.createDataFrame(new util.ArrayList[Row], rowSchema)

    val methods = new ArrayBuffer[Method]()
    for (i <- 0 until columnTypeList.size()) {
      val method =
        rsClass.getMethod("get" + columnTypeList.get(i), "aa".getClass)
      methods += method
    }
    ...
    println(
      System.currentTimeMillis() + "__________ operated rs in createResultSetToDF __________"
    )
    val tempDF = sparkSession.createDataFrame(resultList, rowSchema)
    totalDF = totalDF.union(tempDF)
    println(
      System.currentTimeMillis() + "__________ operated union in createResultSetToDF __________"
    )
    totalDF
  }
  ...
  def setTableOnlySaveOnceCompactionValue(
      tablePath: String,
      value: Boolean
  ): Unit = {
    val sql =
