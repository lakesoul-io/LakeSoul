...
object NewCompactionTask {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (parameter.has(NEW_COMPACT_PERCENTAGE_PARAMETER)) {
      newCompactTablePercentage = Some(
        parameter.getInt(NEW_COMPACT_PERCENTAGE_PARAMETER)
      )
    }
  ...
  class CompactionTableInfo(
    ...
    override def run(): Unit = {
      ...
      try {
        println(
          "------ " + threadName + " is compressing table path is: " + path + " ------"
        )
          ...
          newCompactTableSet match {
            case Some(tableSet) =>
              if (tableSet.contains(fullTableName)) {
                println(
                  "------- tableName: " + fullTableName + " use newCompact" + " ------"
                )
                table.newCompaction(
                ...
                println(
                  "------- tableName: " + fullTableName + " use oldCompact" + " ------"
                )
                table.compaction(
            ...
            case None =>
              ...
              println(
                s"DEBUG: table=$fullTableName, hash=$hash, absHash=${Math.abs(hash)}, bucketNum=$bucketNum, percentage=${newCompactTablePercentage.get}, condition=${bucketNum < newCompactTablePercentage.get}"
              )
        ...
        case e: Exception => {
          println(
            "****** " + dateFormat.format(
              new Date()
            ) + " threadName is: " + threadName + " throw exception, table path is: " + path + " ******"
          )
          println(e.toString)
        }
      } finally {
        ...
        println(
          "========== " + dateFormat.format(
            new Date()
          ) + " " + threadName + " processed notification: " + setValue + " ========== "
        )
      }
    }
  }
