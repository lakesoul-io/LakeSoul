...
/** An extension for Spark SQL.
  ...
  * }}}
  *
  * Python example to create a `SparkSession`(PySpark doesn't pick up the SQL
  * conf "spark.sql.extensions" in Apache Spark 2.4.x, hence we need to activate
  * it manually in 2.4.x. However, because `SparkSession` has been created and
  * everything has been materialized, we need to clone a new session to trigger
  * the initialization. See SPARK-25003):
  * {{{
...
class LakeSoulSparkSessionExtension extends (SparkSessionExtensions => Unit) {
  ...
  override def apply(extensions: SparkSessionExtensions): Unit = {
    ...
    extensions.injectParser { case (_, parser) =>
      new LakeSoulSparkSqlExtensionsParser(parser)
    }
