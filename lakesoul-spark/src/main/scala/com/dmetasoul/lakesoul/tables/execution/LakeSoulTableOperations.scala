...
trait LakeSoulTableOperations extends AnalysisHelper {
  ...
  protected def executeJoinRightAndUpsert(deltaLeftDF: DataFrame,
  ...
  protected def executeJoinLeftAndUpsert(deltaRightDF: DataFrame,
                                         leftTablePath: String,
                                         joinKey: Seq[String],
                                         currentVersion: String,
                                         condition: String = ""): Unit = {
    val leftTable = if (currentVersion == "") LakeSoulTable.forPath(leftTablePath) else LakeSoulTable.forPathSnapshot(leftTablePath, "", currentVersion)
    val deltaJoin = leftTable.toDF.join(broadcast(deltaRightDF), joinKey, "inner")
    executeUpsert(this, deltaJoin, condition)
  }
