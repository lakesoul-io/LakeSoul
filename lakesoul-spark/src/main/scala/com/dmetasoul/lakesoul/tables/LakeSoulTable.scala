...
class LakeSoulTable(df: => Dataset[Row], snapshotManagement: SnapshotManagement)
   ...
   * @param condition     you can define a condition to filter LakeSoul data
   */
  def upsertOnJoinKey(deltaDF: DataFrame, joinKey: Seq[String], partitionDesc: Seq[String] = Seq.empty[String],
   ...
   * @param condition          you can define a condition to filter LakeSoul data
   */
  def joinWithTablePathsAndUpsert(deltaDF: DataFrame, tablePaths: Seq[String],
   ...
   * @param condition          you can define a condition to filter LakeSoul data
   */
  def joinWithTableNamesAndUpsert(deltaDF: DataFrame, tableNames: Seq[String],
                                  tablePartitionDesc: Seq[Seq[String]] = Seq.empty[Seq[String]],
                                  condition: String = ""): Unit = {
  ...
  def compaction(condition: String = "",
    ...
    executeCompaction(df, snapshotManagement, condition, force, newMergeOpInfo, hiveTableName, hivePartitionName,
      cleanOldCompaction, fileNumLimit, newBucketNum, parsedFileSizeLimit)
  }
  ...
  def newCompaction(conditionStr: String = "",
    ...
    def dropPartition(condition: Expression): Unit = {
      assert(snapshotManagement.snapshot.getTableInfo.range_partition_columns.nonEmpty,
        s"Table `${
          snapshotManagement.table_path
        }` is not a range partitioned table, dropTable command can't use on it.")
      executeDropPartition(snapshotManagement, condition)
    }

    def rollbackPartition(partitionValue: String, toVersionNum: Int): Unit = {
      SparkMetaVersion
        .rollbackPartitionInfoByVersion(snapshotManagement.getTableInfoOnly.table_id, partitionValue, toVersionNum)
    }
    ...
    def rollbackPartition(partitionValue: String, toTime: String, timeZoneID: String = ""): Unit = {
      ...
      val version = SparkMetaVersion
        .getLastedVersionUptoTime(snapshotManagement.getTableInfoOnly.table_id, partitionValue, endTime)
      if (version < 0) {
  ...
  object LakeSoulTable {
    ...
    def forPathSnapshot(path: String, partitionDesc: String, endTime: String, timeZone: String = ""): LakeSoulTable = {
      ...
      forPath(sparkSession, path, partitionDesc, "1970-01-01 00:00:00", endTime, timeZone,
        LakeSoulOptions.ReadType.SNAPSHOT_READ)
    }

    /** Incremental Query from startTime to now
     */
    def forPathIncremental(path: String, partitionDesc: String, startTime: String, endTime: String,
                           timeZone: String = ""): LakeSoulTable = {
      val sparkSession = SparkSession.getActiveSession.getOrElse {
        throw new IllegalArgumentException("Could not find active SparkSession")
      }

      forPath(sparkSession, path, partitionDesc, startTime, endTime, timeZone,
    ...
    def forPath(sparkSession: SparkSession, path: String, partitionDesc: String,
                partitionVersion: Int): LakeSoulTable = {
      val p = SparkUtil.makeQualifiedTablePath(new Path(path)).toUri.toString
      if (LakeSoulUtils.isLakeSoulTable(sparkSession, new Path(p))) {
        new LakeSoulTable(sparkSession.read.format(LakeSoulSourceUtils.SOURCENAME).load(p),
    ...
    * */
    def forPath(sparkSession: SparkSession, path: String, partitionDesc: String, startTimeStamp: String,
                endTimeStamp: String, timeZone: String, readType: String): LakeSoulTable = {
      val timeZoneID = if (timeZone.equals("") || !TimeZone.getAvailableIDs.contains(timeZone)) TimeZone.getDefault
        .getID else timeZone
