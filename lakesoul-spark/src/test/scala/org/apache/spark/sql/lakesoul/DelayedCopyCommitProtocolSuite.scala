...
class DelayedCopyCommitProtocolSuite extends AnyFunSuite {
  ...
  test(
    "destinationRelativePath keeps the compact-relative path for compacted files"
  ) {
    ...
    assert(
      protocol.destinationRelativePath(srcFile) == "range=1/part-0000.parquet"
    )
  }

  test(
    "destinationRelativePath rebuilds a partition-relative path for uncompacted files"
  ) {
    val protocol = new DelayedCopyCommitProtocol(Nil, "job", "/tmp/dst", None)
    val srcFile = DataFileInfo(
    ...
    assert(
      protocol.destinationRelativePath(
        srcFile
      ) == "range=1/date=2023-01-01/part-0000.parquet"
    )
  }

  test(
    "destinationRelativePath keeps only the file name for non-partitioned uncompacted files"
  ) {
    val protocol = new DelayedCopyCommitProtocol(Nil, "job", "/tmp/dst", None)
    val srcFile = DataFileInfo(
