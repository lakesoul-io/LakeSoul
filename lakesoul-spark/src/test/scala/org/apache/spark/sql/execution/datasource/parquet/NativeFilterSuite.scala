...
class ParquetNativeFilterSuite
  ...
  override def withNestedParquetDataFrame(
      inputDF: DataFrame
    ...
    withNestedDataFrame(inputDF).foreach { case (newDF, colName, resultFun) =>
      withTempDir { file =>
        ...
        loadLakeSoulTable(file.getCanonicalPath) { df =>
    ...
    withSQLConf(
      ...
      query.queryExecution.optimizedPlan.collectFirst {
        case PhysicalOperation(
          ...
          val sourceFilters =
            filters.flatMap(DataSourceStrategy.translateFilter(_, true)).toArray
          ...
          assert(
            parquetFilters.convertibleFilters(sourceFilters) === pushedFilters
          )
        ...
        case PhysicalOperation(
          ...
          val pushedParquetFilters = pushedFilters.map { pred =>
            val maybeFilter = parquetFilters.createFilter(pred)
            println(
              "pred:" + pred.toString + ", maybeFilter:" + maybeFilter.toString
            )
            assert(
              maybeFilter.isDefined,
              s"Couldn't generate filter predicate for $pred"
...
/** A test suite that tests Parquet filter2 API based filter pushdown
  ...
  *      the `BooleanSimplification` optimization rule whenever possible. As a
  *      result, predicate `!(a < 1)` results in a `GtEq` filter predicate
  *      rather than a `Not`.
  *   2. `Tuple1(Option(x))` is used together with `AnyVal` types like `Int` to
  *      ensure the inferred data type is nullable.
  *
  * NOTE:
  *
  * This file intendedly enables record-level filtering explicitly. If new test
  * cases are dependent on this configuration, don't forget you better
  * explicitly set this configuration within the test.
  */
abstract class ParquetFilterSuite
  ...
  protected def createParquetFilters(
      ...
      datetimeRebaseSpec: RebaseSpec = RebaseSpec(
  ...
  /** Takes a sequence of products `data` to generate multi-level nested
    ...
    * dataframes which are written and read back with Parquet datasource.
    *
    * This is different from [[ParquetTest.withParquetDataFrame]] which does not
    * test nested cases.
    */
  private def withNestedParquetDataFrame[T <: Product: ClassTag: TypeTag](
      data: Seq[T]
  )(runTest: (DataFrame, String, Any => Any) => Unit): Unit =
    withNestedParquetDataFrame(spark.createDataFrame(data))(runTest)

  protected def withNestedParquetDataFrame(
      inputDF: DataFrame
  )(runTest: (DataFrame, String, Any => Any) => Unit): Unit = {
    withNestedDataFrame(inputDF).foreach { case (newDF, colName, resultFun) =>
      withTempPath { file =>
  ...
  /** Takes single level `inputDF` dataframe to generate multi-level nested
    * dataframes as new test data. It tests both non-nested and nested
    * dataframes which are written and read back with specified datasource.
    */
  override protected def withNestedDataFrame(
      inputDF: DataFrame
  ): Seq[(DataFrame, String, Any => Any)] = {
    ...
    Seq(
      ...
      (
        df.withColumn("a", struct(struct(df("temp") as "c") as "b"))
          .drop("temp"),
  ...
  private def testTimestampPushdown(
      data: Seq[String],
      java8Api: Boolean
  ): Unit = {
    ...
    withNestedParquetDataFrame(df) { case (parquetDF, colName, fun) =>
      ...
      checkFilterPredicate(
        Literal(ts2.ts) > tsAttr,
        classOf[Lt[_]],
        resultFun(ts1)
      )
      checkFilterPredicate(
        Literal(ts3.ts) < tsAttr,
        classOf[Gt[_]],
        resultFun(ts4)
      )
      checkFilterPredicate(
  ...
  private def testStringStartsWith(
      dataFrame: DataFrame,
      filter: String
  ): Unit = {
    withTempPath { dir =>
      ...
      Seq(true, false).foreach { pushDown =>
        withSQLConf(
          ...
          df.foreachPartition((it: Iterator[Row]) =>
            it.foreach(v => accu.add(0))
          )
          if (pushDown) {
            assert(accu.value == 0)
          } else {
  ...
  test("filter pushdown - tinyint") {
    ...
    withNestedParquetDataFrame(data) { case (inputDF, colName, resultFun) =>
      ...
      checkFilterPredicate(
        tinyIntAttr.isNotNull,
        classOf[NotEq[_]],
        (1 to 4).map(i => Row.apply(resultFun(i)))
      )

      checkFilterPredicate(
        tinyIntAttr === 1.toByte,
        classOf[Eq[_]],
        resultFun(1)
      )
      checkFilterPredicate(
      ...
      checkFilterPredicate(
        tinyIntAttr >= 4.toByte,
        classOf[GtEq[_]],
        resultFun(4)
      )

      checkFilterPredicate(
        Literal(1.toByte) === tinyIntAttr,
        classOf[Eq[_]],
        resultFun(1)
      )
      checkFilterPredicate(
      ...
      checkFilterPredicate(
        Literal(2.toByte) > tinyIntAttr,
        classOf[Lt[_]],
        resultFun(1)
      )
      checkFilterPredicate(
        Literal(3.toByte) < tinyIntAttr,
        classOf[Gt[_]],
        resultFun(4)
      )
      checkFilterPredicate(
      ...
      checkFilterPredicate(
        tinyIntAttr < 2.toByte || tinyIntAttr > 3.toByte,
        classOf[Operators.Or],
  ...
  test("filter pushdown - smallint") {
    ...
    withNestedParquetDataFrame(data) { case (inputDF, colName, resultFun) =>
      ...
      checkFilterPredicate(
        smallIntAttr.isNotNull,
        classOf[NotEq[_]],
        (1 to 4).map(i => Row.apply(resultFun(i)))
      )

      checkFilterPredicate(
        smallIntAttr === 1.toShort,
        classOf[Eq[_]],
        resultFun(1)
      )
      checkFilterPredicate(
      ...
      checkFilterPredicate(
        smallIntAttr < 2.toShort,
        classOf[Lt[_]],
        resultFun(1)
      )
      checkFilterPredicate(
        smallIntAttr > 3.toShort,
        classOf[Gt[_]],
        resultFun(4)
      )
      checkFilterPredicate(
      ...
      checkFilterPredicate(
        smallIntAttr >= 4.toShort,
        classOf[GtEq[_]],
        resultFun(4)
      )

      checkFilterPredicate(
        Literal(1.toShort) === smallIntAttr,
        classOf[Eq[_]],
        resultFun(1)
      )
      checkFilterPredicate(
      ...
      checkFilterPredicate(
        Literal(2.toShort) > smallIntAttr,
        classOf[Lt[_]],
        resultFun(1)
      )
      checkFilterPredicate(
        Literal(3.toShort) < smallIntAttr,
        classOf[Gt[_]],
        resultFun(4)
      )
      checkFilterPredicate(
      ...
      checkFilterPredicate(
        Literal(4.toShort) <= smallIntAttr,
        classOf[GtEq[_]],
        resultFun(4)
      )
      ...
      checkFilterPredicate(
        smallIntAttr < 2.toShort || smallIntAttr > 3.toShort,
        classOf[Operators.Or],
        Seq(Row(resultFun(1)), Row(resultFun(4)))
      )
    }
  ...
  test("filter pushdown - double") {
    ...
    withNestedParquetDataFrame(data) { case (inputDF, colName, resultFun) =>
      ...
      checkFilterPredicate(
        Literal(2) > doubleAttr,
        classOf[Lt[_]],
        resultFun(1)
      )
      checkFilterPredicate(
        Literal(3) < doubleAttr,
        classOf[Gt[_]],
        resultFun(4)
      )
      checkFilterPredicate(
  ...
  test("filter pushdown - string") {
    ...
    withNestedParquetDataFrame(data) { case (inputDF, colName, resultFun) =>
      ...
      checkFilterPredicate(
        Literal("2") > stringAttr,
        classOf[Lt[_]],
        resultFun("1")
      )
      checkFilterPredicate(
        Literal("3") < stringAttr,
        classOf[Gt[_]],
        resultFun("4")
      )
      checkFilterPredicate(
  ...
  test("filter pushdown - binary") {
    ...
    withNestedParquetDataFrame(data) { case (inputDF, colName, resultFun) =>
      ...
      checkFilterPredicate(
        Literal(2.b) > binaryAttr,
        classOf[Lt[_]],
        resultFun(1.b)
      )
      checkFilterPredicate(
        Literal(3.b) < binaryAttr,
        classOf[Gt[_]],
        resultFun(4.b)
      )
      checkFilterPredicate(
      ...
      withSQLConf(SQLConf.DATETIME_JAVA8API_ENABLED.key -> java8Api.toString) {
        ...
        withNestedParquetDataFrame(dates) { case (inputDF, colName, fun) =>
          ...
          def resultFun(dateStr: String): Any = {
            val parsed =
              if (java8Api) LocalDate.parse(dateStr) else Date.valueOf(dateStr)
            fun(parsed)
          }
          ...
          checkFilterPredicate(
            Literal("2018-03-18".date) === dateAttr,
            classOf[Eq[_]],
            resultFun("2018-03-18")
          )
          checkFilterPredicate(
  ...
  test("filter pushdown - decimal") {
    Seq(
      ...
      withSQLConf(
        SQLConf.PARQUET_WRITE_LEGACY_FORMAT.key -> legacyFormat.toString
      ) {
        val rdd =
          spark.sparkContext.parallelize(
            (1 to 4).map(i => Row(new java.math.BigDecimal(i)))
          )
        val dataFrame = spark.createDataFrame(
          rdd,
          StructType.fromDDL(s"a decimal($precision, 2)")
        )
        withNestedParquetDataFrame(dataFrame) {
          case (inputDF, colName, resultFun) =>
            implicit val df: DataFrame = inputDF
            ...
            checkFilterPredicate(
              Literal(2) > decimalAttr,
              classOf[Lt[_]],
              resultFun(1)
            )
            checkFilterPredicate(
              Literal(3) < decimalAttr,
              classOf[Gt[_]],
              resultFun(4)
            )
            checkFilterPredicate(
  ...
  test("Ensure that filter value matched the parquet file schema") {
    ...
    assert(
      parquetFilters
        .createFilter(sources.LessThan("cdecimal3", decimal))
        .isDefined
    )
    assertResult(None) {
