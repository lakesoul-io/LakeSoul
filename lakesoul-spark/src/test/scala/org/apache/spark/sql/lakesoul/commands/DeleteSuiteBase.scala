...
abstract class DeleteSuiteBase
  ...
  protected def checkDelete(
    ...
    executeDelete(
      target = tableName.getOrElse(s"lakesoul.default.`$tempPath`"),
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(
      s"basic case - delete from a LakeSoul table by path - Partition=$isPartitioned"
    ) {
      withTable("starTable") {
        val partitions = if (isPartitioned) "key" :: Nil else Nil
        ...
        checkDelete(Some("key = 0 or value = 99"), Nil, Seq("key", "value"))
      }
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(
      s"basic case with hash partition - delete from a LakeSoul table by path - rangePartition=$isPartitioned"
    ) {
      withTable("starTable") {
        val partitions = if (isPartitioned) "range" :: Nil else Nil
  ...
  test(
    s"basic case with hash partition - delete from a LakeSoul table by path"
  ) {
    withTable("starTable") {
      ...
      checkDelete(
        Some("value = 2 or hash = 1"),
        Row(0, 3, 3) :: Nil,
        Seq("range", "hash", "value")
      )
      checkDelete(
        Some("hash = 3 or value = 99"),
        Nil,
        Seq("range", "hash", "value")
      )
    }
  ...
  test("upsert before delete with hash partition") {
    withTable("starTable") {
      ...
      checkDelete(
        Some("value = 4 or range = 1"),
        Row(0, 3, 1) :: Nil,
        Seq("range", "hash", "value")
      )
      checkDelete(Some("hash = 3"), Nil, Seq("range", "hash", "value"))
    }
  }

  Seq(true, false).foreach { isPartitioned =>
    test(
      s"basic case - delete from a LakeSoul table by name - Partition=$isPartitioned"
    ) {
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"basic key columns - Partition=$isPartitioned") {
      ...
      checkDelete(Some("key < 2"), Row(2, 2) :: Nil, Seq("key", "value"))
      checkDelete(Some("key = 2"), Nil, Seq("key", "value"))
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"basic key columns with hash partition - Partition=$isPartitioned") {
      ...
      checkDelete(Some("range = 2"), Nil, Seq("range", "hash", "value"))
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"where key columns - Partition=$isPartitioned") {
      ...
      checkDelete(Some("key = 2"), Row(0, 3) :: Nil, Seq("key", "value"))
      checkDelete(Some("key = 0"), Nil, Seq("key", "value"))
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"where data columns - Partition=$isPartitioned") {
      ...
      checkDelete(Some("value = 3"), Row(1, 4) :: Nil, Seq("key", "value"))
      checkDelete(Some("value != 0"), Nil, Seq("key", "value"))
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"where data columns with hash partition - Partition=$isPartitioned") {
      ...
      checkDelete(Some("value != 0"), Nil, Seq("range", "hash", "value"))
    }
  }
  ...
  test("Negative case - non-deterministic condition") {
    ...
    val e = intercept[AnalysisException] {
      executeDelete(
        target = s"lakesoul.default.`$tempPath`",
        where = "rand() > 0.5"
      )
    }.getMessage
    assert(
  ...
  test("delete cached table by name") {
    withTable("cached_lakesoul_table") {
      Seq((2, 2), (1, 4))
        ...
        .write
        .format("lakesoul")
        .saveAsTable("cached_lakesoul_table")
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"condition having current_date - Partition=$isPartitioned") {
      ...
      checkDelete(Some("CURRENT_DATE <= key"), Nil, Seq("key", "value"))
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"foldable condition - Partition=$isPartitioned") {
      ...
      checkDelete(Some("false"), allRows, Seq("key", "value"))
      checkDelete(Some("1 <> 1"), allRows, Seq("key", "value"))
      checkDelete(Some("1 > null"), allRows, Seq("key", "value"))
      checkDelete(Some("true"), Nil, Seq("key", "value"))
      checkDelete(Some("1 = 1"), Nil, Seq("key", "value"))
    }
  }

  test("should not delete the rows where condition evaluates to null") {
    append(
      Seq(("a", null), ("b", null), ("c", "v"), ("d", "vv"))
        .toDF("key", "value")
        .coalesce(1)
    )
    ...
    checkDelete(
      Some("value = null"),
      Row("a", null) :: Row("b", null) :: Row("c", "v") :: Row(
        "d",
        "vv"
      ) :: Nil,
      Seq("key", "value")
  ...
  test("do not support subquery test") {
    ...
    val e1 = intercept[AnalysisException] {
      executeDelete(
        target = s"lakesoul.default.`$tempPath`",
        "EXISTS (SELECT max(c) FROM source)"
      )
    }.getMessage
    ...
    val e2 = intercept[AnalysisException] {
      executeDelete(
        target = s"lakesoul.default.`$tempPath`",
        "NOT EXISTS (SELECT max(c) FROM source)"
      )
    }.getMessage
    ...
    val e3 = intercept[AnalysisException] {
      executeDelete(
        target = s"lakesoul.default.`$tempPath`",
        "key IN (SELECT max(c) FROM source)"
      )
    }.getMessage
    ...
    val e4 = intercept[AnalysisException] {
      executeDelete(
        target = s"lakesoul.default.`$tempPath`",
        "key NOT IN (SELECT max(c) FROM source)"
      )
    }.getMessage
