    ...
    withTempDir(dir1 => {
      withTempDir(dir2 => {
        withTempDir(dir3 => {
          ...
          val df = leftTable.toDF
            .join(rightTable.toDF, Seq("hash_right"), "left_outer")
          df.write
            .mode("overwrite")
            .format("lakesoul")
            .option("hashPartitions", "hash_left")
          ...
          val deltaLeft =
            Seq((3, "c", "v1-31"), (4, "d", "v1-4"), (5, "e", "v1-5"))
              .toDF("hash_left", "hash_right", "v1")
    ...
    withTable(leftTableName) {
      withTable(rightTableName) {
        withTable(joinTableName) {
          ...
          val dfJoinInit1 = LakeSoulTable
            .forName(leftTableName)
            .toDF
            .filter("rangeL1=1 and rangeL2=1")
            .join(
              LakeSoulTable
                .forName(rightTableName)
                .toDF
                .filter("rangeR1=1 and rangeR2=1"),
              Seq("hashRight"),
              "left_outer"
            )
          val dfJoinInit2 = LakeSoulTable
            .forName(leftTableName)
            .toDF
            .filter("rangeL1=1 and rangeL2=2")
            .join(
              LakeSoulTable
                .forName(rightTableName)
                .toDF
                .filter("rangeR1=1 and rangeR2=2"),
              Seq("hashRight"),
              "left_outer"
            )
          val dfJoinInit3 = LakeSoulTable
            .forName(leftTableName)
            .toDF
            .filter("rangeL1=2 and rangeL2=1")
            .join(
              LakeSoulTable
                .forName(rightTableName)
                .toDF
                .filter("rangeR1=2 and rangeR2=1"),
              Seq("hashRight"),
              "left_outer"
            )
          val dfJoinInit4 = LakeSoulTable
            .forName(leftTableName)
            .toDF
            .filter("rangeL1=2 and rangeL2=2")
            .join(
              LakeSoulTable
                .forName(rightTableName)
                .toDF
                .filter("rangeR1=2 and rangeR2=2"),
              Seq("hashRight"),
              "left_outer"
            )
          val dfJoinInit = dfJoinInit1
            .union(dfJoinInit2)
            .union(dfJoinInit3)
            .union(dfJoinInit4)
            .repartition(1)
          ...
          LakeSoulTable
            .forName(joinTableName)
            .joinWithTableNamesAndUpsert(
              dfLeft1,
              Seq(rightTableName),
              Seq(Seq("rangeR1=1", "rangeR2=1"))
            )
          ...
          LakeSoulTable
            ...
            .joinWithTableNamesAndUpsert(
              dfLeft3,
              Seq(rightTableName),
              Seq(Seq("rangeR1=1", "rangeR2=3"))
            )

          val dfRight1 =
            Seq((1, "b1-2", "1", 1), (2, "b2-1", "1", 1), (6, "b6-1", "1", 1))
              .toDF("hashRight", "v2", "rangeR1", "rangeR2")
          ...
          LakeSoulTable
            ...
            .upsertOnJoinKey(
              dfRight2,
              Seq("hashRight"),
              Seq("rangeL1=2", "rangeL2=2")
            )

          val gtResult1_1 = LakeSoulTable
            .forName(leftTableName)
            .toDF
            .filter("rangeL1=1 and rangeL2=1")
            .join(
          ...
          val gtResult1_3 = LakeSoulTable
            .forName(leftTableName)
            .toDF
            .filter("rangeL1=1 and rangeL2=3")
            .join(
              LakeSoulTable
                .forName(rightTableName)
                .toDF
                .filter("rangeR1=1 and rangeR2=3"),
              Seq("hashRight"),
              "left_outer"
            )
          val gtResult2_1 = LakeSoulTable
            .forName(leftTableName)
            .toDF
            .filter("rangeL1=2 and rangeL2=1")
            .join(
    ...
    withTable(leftTableName) {
      withTable(rightTableName) {
        withTable(joinTableName) {
          ...
          val dfRightInit =
            Seq((1, "b1-1", 1), (2, "b2-1", 1), (3, "b3-1", 1), (4, "b4-1", 2))
          ...
          val ex = intercept[AnalysisException] {
            LakeSoulTable
              .forName(joinTableName)
              .joinWithTableNamesAndUpsert(dfLeft1, Seq(rightTableName))
          }
          assert(
            ex.getMessage.contains(
              s"join key $missingKey is missing in join table"
            )
          )
          ...
          val ex1 = intercept[AnalysisException] {
            LakeSoulTable
              .forName(joinTableName)
              .upsertOnJoinKey(dfRight1, Seq("hashRight"))
          }
          assert(
