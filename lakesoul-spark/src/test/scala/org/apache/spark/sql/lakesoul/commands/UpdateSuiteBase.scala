...
abstract class UpdateSuiteBase
  ...
  protected def appendHashPartition(
      df: DataFrame,
  ...
  protected def executeUpsert(df: DataFrame): Unit = {
    LakeSoulTable
      .forPath(snapshotManagement.table_path)
      .upsert(df)
  }
  ...
  protected def checkUpdate(
    ...
    executeUpdate(
      tableName.getOrElse(s"lakesoul.default.`$tempPath`"),
      setClauses,
      where = condition.orNull
    )
    ...
    test(s"basic update - LakeSoul table by path - Partition=$isPartitioned") {
      withTable("lakeSoulTable") {
        ...
        checkUpdate(
          ...
          expectedResults =
            Row(0, 3) :: Row(2, 5) :: Row(2, 2) :: Row(3, 4) :: Nil,
          Seq("key", "value"),
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(
      s"basic update - LakeSoul table by path with hash partition - Partition=$isPartitioned"
    ) {
      withTable("lakeSoulTable") {
        val partitions = if (isPartitioned) "key" :: Nil else Nil
        appendHashPartition(
          Seq((2, 2, 2), (1, 4, 4), (1, 1, 1), (0, 3, 3))
            .toDF("key", "hash", "value"),
          partitions
        )
        ...
        checkUpdate(
          ...
          setClauses = "value = key + value, key = key + 1",
          expectedResults =
            Row(0, 3, 3) :: Row(2, 4, 5) :: Row(2, 1, 2) :: Row(3, 2, 4) :: Nil,
          Seq("key", "hash", "value"),
          tableName = Some(s"lakesoul.default.`$tempPath`")
        )
      }
  ...
  Seq(true).foreach { isPartitioned =>
    test(s"basic update - with hash partition - Partition=$isPartitioned") {
      val partitions = if (isPartitioned) "key" :: Nil else Nil
      appendHashPartition(
        Seq((2, 2, 2), (1, 1, 4), (1, 2, 1), (0, 0, 3))
          .toDF("key", "hash", "value"),
        partitions
      )

      checkUpdate(
        condition = Some("value > 2"),
        setClauses = "value = 1",
        expectedResults =
          Row(2, 2, 2) :: Row(1, 1, 1) :: Row(1, 2, 1) :: Row(0, 0, 1) :: Nil,
        Seq("key", "hash", "value")
      )
    }
  ...
  Seq(true).foreach { isPartitioned =>
    test(
      s"upsert before update - with hash partition - Partition=$isPartitioned"
    ) {
      val partitions = if (isPartitioned) "key" :: Nil else Nil
      appendHashPartition(
        Seq((2, 2, 1), (1, 2, 1), (0, 0, 1))
          .toDF("key", "hash", "value"),
        partitions
      )

      executeUpsert(
        Seq((2, 2, 2), (1, 3, 2), (0, 0, 2))
          .toDF("key", "hash", "value")
      )

      checkUpdate(
        condition = Some("hash = 2"),
        setClauses = "value = 3",
        expectedResults =
          Row(2, 2, 3) :: Row(1, 2, 3) :: Row(1, 3, 2) :: Row(0, 0, 2) :: Nil,
        Seq("key", "hash", "value")
      )
    }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"basic update - LakeSoul table by name - Partition=$isPartitioned") {
      withTable("lakesoul_table") {
        val partitionByClause =
          if (isPartitioned) "PARTITIONED BY (key)" else ""
        sql(s"""
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"table has null values - partitioned=$isPartitioned") {
      val partitions = if (isPartitioned) "key" :: Nil else Nil
      append(
        Seq(("a", 1), (null, 2), (null, 3), ("d", 4)).toDF("value", "key"),
        partitions
      )

      // predicate evaluates to null; no-op
      checkUpdate(
        condition = Some("value = null"),
        setClauses = "value = -1",
        expectedResults =
      ...
      checkUpdate(
        condition = Some("value is null"),
        setClauses = "key = -2",
        expectedResults =
          Row("a", -1) :: Row(null, -2) :: Row(null, -2) :: Row("d", 4) :: Nil,
        Seq("value", "key")
      )

      checkUpdate(
        condition = Some("value is not null"),
        setClauses = "key = -3",
        expectedResults =
          Row("a", -3) :: Row(null, -2) :: Row(null, -2) :: Row("d", -3) :: Nil,
        Seq("value", "key")
      )
      ...
      checkUpdate(
        ...
        setClauses = "key = -4",
        expectedResults =
          Row("a", -3) :: Row(null, -4) :: Row(null, -4) :: Row("d", -3) :: Nil,
        Seq("value", "key")
      )
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(
      s"basic update - without where and partial columns - Partition=$isPartitioned"
    ) {
      val partitions = if (isPartitioned) "key" :: Nil else Nil
      append(
        Seq((2, 2), (1, 4), (1, 1), (0, 3)).toDF("key", "value"),
      ...
      checkUpdate(
        ...
        setClauses = "key = 1",
        expectedResults =
          Row(1, 1) :: Row(1, 2) :: Row(1, 3) :: Row(1, 4) :: Nil,
        Seq("key", "value")
      )
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(
      s"basic update - without where and out-of-order columns - Partition=$isPartitioned"
    ) {
      val partitions = if (isPartitioned) "key" :: Nil else Nil
      append(
        Seq((2, 2), (1, 4), (1, 1), (0, 3)).toDF("key", "value"),
      ...
      checkUpdate(
        ...
        expectedResults =
          Row(1, 3) :: Row(1, 3) :: Row(1, 3) :: Row(1, 3) :: Nil,
        Seq("key", "value")
      )
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(
      s"basic update - without where and complex input - Partition=$isPartitioned"
    ) {
      val partitions = if (isPartitioned) "key" :: Nil else Nil
      append(
        Seq((2, 2), (1, 4), (1, 1), (0, 3)).toDF("key", "value"),
      ...
      checkUpdate(
        ...
        expectedResults =
          Row(1, 3) :: Row(2, 4) :: Row(2, 4) :: Row(3, 5) :: Nil,
        Seq("key", "value")
      )
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"basic update - with where - Partition=$isPartitioned") {
      ...
      checkUpdate(
        ...
        setClauses = "value = 3, key = 1",
        expectedResults =
          Row(1, 3) :: Row(2, 2) :: Row(0, 3) :: Row(1, 3) :: Nil,
        Seq("key", "value")
      )
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(
      s"basic update with hash partition - with where - Partition=$isPartitioned"
    ) {
      val partitions = if (isPartitioned) "key" :: Nil else Nil
      appendHashPartition(
        Seq((2, 2, 2), (1, 4, 4), (1, 1, 1), (0, 3, 3))
      ...
      checkUpdate(
        condition = Some("hash = 1"),
        setClauses = "value = 3, key = 1",
        expectedResults =
          Row(2, 2, 2) :: Row(1, 4, 4) :: Row(1, 1, 3) :: Row(0, 3, 3) :: Nil,
        Seq("key", "hash", "value")
      )
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(
      s"basic update - with where and complex input - Partition=$isPartitioned"
    ) {
      val partitions = if (isPartitioned) "key" :: Nil else Nil
      append(
        Seq((2, 2), (1, 4), (1, 1), (0, 3)).toDF("key", "value"),
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(
      s"basic update - with where and no row matched - Partition=$isPartitioned"
    ) {
      val partitions = if (isPartitioned) "key" :: Nil else Nil
      append(
        Seq((2, 2), (1, 4), (1, 1), (0, 3)).toDF("key", "value"),
      ...
      checkUpdate(
        ...
        setClauses = "value = key + value, key = key + 1",
        expectedResults =
          Row(0, 3) :: Row(1, 1) :: Row(1, 4) :: Row(2, 2) :: Nil,
        Seq("key", "value")
      )
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"type mismatch - Partition=$isPartitioned") {
      ...
      checkUpdate(
        ...
        expectedResults =
          Row(0, 3) :: Row(2, 5) :: Row(3, 4) :: Row(2, 2) :: Nil,
        Seq("key", "value")
      )
    }
  }
  ...
  Seq(true, false).foreach { isPartitioned =>
    test(s"basic update - TypeCoercion twice - Partition=$isPartitioned") {
      ...
      checkUpdate(
        condition =
          Some("cast(key as long) * cast('1.0' as decimal(38, 18)) > 100"),
        setClauses = "value = -3",
  ...
  test("different variations of column references") {
    ...
    spark.read
      .format("lakesoul")
      .load(tempPath)
      .createOrReplaceTempView("tblName")
  ...
  test("LakeSoul Table columns can have db and table qualifiers") {
    withTable("lakeSoulTable") {
      spark.read
        ...
        .write
        .format("lakesoul")
        .saveAsTable("`lakeSoulTable`")

      executeUpdate(
        lakeSoulTable = "lakeSoulTable",
        set =
          "`default`.`lakeSoulTable`.a.`b.1` = -1, lakeSoulTable.a.`c.e` = 'RANDOM'",
        where = "d = 1"
      )

      checkAnswer(
        spark.table("lakeSoulTable"),
        spark.read.json(
          """
            {"a": {"b.1": -1, "c.e": 'RANDOM'}, "d": 1}
            {"a": {"b.1": 3, "c.e": 'string'}, "d": 2}"""
  ...
  test("Negative case - non-lakesoul lakeSoulTable") {
    ...
    val e = intercept[AnalysisException] {
      executeUpdate(
        lakeSoulTable = s"lakesoul.default.`$tempPath`",
        set = "key1 = 3"
      )
    }.getMessage
  ...
  test("Negative case - check lakeSoulTable columns during analysis") {
    ...
    withTable("table") {
      sql(
        s"CREATE TABLE table (s int, t string) USING lakesoul PARTITIONED BY (s) LOCATION '${tempDir.getCanonicalPath}'"
      )
      var ae = intercept[AnalysisException] {
        executeUpdate(
          "table",
          set = "column_doesnt_exist = 'San Francisco'",
          where = "t = 'a'"
        )
      }
      ...
      withSQLConf(SQLConf.CASE_SENSITIVE.key -> "false") {
        ...
        assert(
          ae.message.contains("There is a conflict from these SET columns")
        )
      }
  ...
  test("nested data support") {
    ...
    checkUpdateJson(
      ...
      updateWhere = "z = 20",
      set =
        "a.c.d = 'RANDOM2'" :: "a.c.e = 'STR2'" :: "a.g = -2" :: "z = -20" :: Nil,
      expected = """
          {"a": {"c": {"d": 'RANDOM', "e": 'str'}, "g": 1}, "z": 10}
          {"a": {"c": {"d": 'RANDOM2', "e": 'STR2'}, "g": -2}, "z": -20}"""
    )
  ...
  test("nested data - negative case") {
    withSQLConf(NATIVE_IO_PHYSICAL_FORMAT.key -> "parquet") {
      ...
      testAnalysisException(
        targetDF,
        set =
          "a.c = named_struct('d', 'rand', 'e', 'str')" :: "a.c.d = 'RANDOM2'" :: Nil,
      ...
      testAnalysisException(
        targetDF,
        set = Seq(
          "a = named_struct('c', named_struct('d', 'rand', 'e', 'str'))",
          "a.c.d = 'RANDOM2'"
        ),
        errMsgs = "There is a conflict from these SET columns" :: Nil
      )
      ...
      val mapData =
        spark.read.schema(schema).json(Seq("""{"a": {"b": 1}}""").toDS())
      testAnalysisException(
        mapData,
        set = "a.b = -1" :: Nil,
        errMsgs =
          "Updating nested fields is only supported for StructType" :: Nil
      )

      // Updating an ArrayStruct is not supported
      val arrayStructData =
        spark.read.json(Seq("""{"a": [{"b": 1}, {"b": 2}]}""").toDS())
      testAnalysisException(
        arrayStructData,
        set = "a.b = -1" :: Nil,
        errMsgs = "data type mismatch" :: Nil
