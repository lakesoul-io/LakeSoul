  ...
  test("test snapshot read with OnePartition") {
    withTable("tt") {
      withTempDir(dir => {
        ...
        val tableForUpsert = Seq(
          ("range1", "hash1-1", "delete"),
          ("range1", "hash1-5", "insert"),
          ("range2", "hash2-1", "delete"),
          ("range2", "hash2-5", "insert")
        )
          .toDF("range", "hash", "op")
        Thread.sleep(2000)
        lake.upsert(tableForUpsert)
        val tableForUpsert1 =
          Seq(("range1", "hash1-2", "update"), ("range2", "hash2-2", "update"))
            .toDF("range", "hash", "op")
        Thread.sleep(2000)
        ...
        Thread.sleep(1000)
        val tableForUpsert2 =
          Seq(("range1", "hash1-3", "insert"), ("range2", "hash2-3", "insert"))
            .toDF("range", "hash", "op")
        lake.upsert(tableForUpsert2)
        val versionA: String =
          new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(timeA)
        ...
        checkAnswer(
          data3,
          Seq(("range1", "hash1-1", "delete")).toDF("range", "hash", "op")
        )
      })
    }
  }
    ...
    withTable("tt") {
      withTempDir(dir => {
        val tablePath = SparkUtil
          ...
          .toString
        Seq(
          (1, "range1", "hash1-1", "insert"),
          (2, "range2", "hash2-1", "insert")
        )
        ...
        val lake = LakeSoulTable.forPath(tablePath)
        val tableForUpsert = Seq(
          (3, "range1", "hash1-2", "update"),
          (4, "range1", "hash1-5", "insert"),
          (5, "range2", "hash2-2", "insert"),
          (6, "range2", "hash2-5", "insert")
        )
          .toDF("id", "range", "hash", "op")
        Thread.sleep(2000)
        lake.upsert(tableForUpsert)
        val tableForUpsert1 = Seq(
          (7, "range1", "hash1-1", "delete"),
          (8, "range2", "hash2-10", "delete")
        )
          .toDF("id", "range", "hash", "op")
        Thread.sleep(2000)
        val timeA = System.currentTimeMillis()
        lake.upsert(tableForUpsert1)
        val tableForUpsert2 = Seq(
          (9, "range1", "hash1-13", "insert"),
          (10, "range2", "hash2-13", "update")
        )
          .toDF("id", "range", "hash", "op")
        Thread.sleep(2000)
        lake.upsert(tableForUpsert2)
        val tableForUpsert3 = Seq(
          (11, "range1", "hash1-15", "insert"),
          (12, "range2", "hash2-15", "update")
        )
          .toDF("id", "range", "hash", "op")
        Thread.sleep(2000)
        ...
        lake2.createOrReplaceTempView("testView")
        val data3 =
          sql(s"select id,range,hash,op from testView where hash='hash1-5'")
        checkAnswer(
          data2,
          Seq(
            (1, "range1", "hash1-1", "insert"),
    ...
    withTable("tt") {
      withTempDir(dir => {
        ...
        withSQLConf(
          ...
          lake.upsert(tableForUpsert1)
          val tableForUpsert2 = Seq(
            ("range1", "hash1-13", "insert"),
            ("range2", "hash2-13", "update")
          )
            .toDF("range", "hash", "op")
          Thread.sleep(3000)
          lake.upsert(tableForUpsert2)
          val tableForUpsert3 = Seq(
            ("range1", "hash1-15", "insert"),
            ("range2", "hash2-15", "update")
          )
            .toDF("range", "hash", "op")
          lake.upsert(tableForUpsert3)
          Thread.sleep(1000)
          ...
          val versionB: String =
            new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(timeB)
          val versionC: String =
            new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(timeC)
          val parDesc = "range=range1"
          val lake1 = LakeSoulTable.forPathIncremental(
            tablePath,
            parDesc,
            versionB,
            versionC
          ...
          lake2.createOrReplaceTempView("testView")
          val data3 =
            sql(s"select range,hash,op from testView where op='delete'")
          checkAnswer(
            data1,
            Seq(
              ("range1", "hash1-1", "delete"),
    ...
    override def run(): Unit = {
      withTable("tt") {
        withTempDir(dir => {
          ...
          val tableForUpsert1 = Seq(
            (5, "range1", "hash1-3", "insert"),
            (6, "range2", "hash2-5", "insert"),
            (7, "range1", "hash1-1", "delete"),
            (8, "range2", "hash2-5", "delete")
          )
            .toDF("id", "range", "hash", "op")
          Thread.sleep(2000)
          lake.upsert(tableForUpsert1)
    ...
    override def run(): Unit = {
      withTable("tt") {
        withTempDir(dir => {
          ...
          lake.upsert(tableForUpsert)
          val testStreamRead = new TestStreamRead
          testStreamRead.setTablePath(
            tablePath,
            time - 4000,
            "range=range1,op=insert"
          )
          testStreamRead.start()
          val tableForUpsert1 = Seq(
            (7, "range1", "hash1-1", "delete"),
      ...
      withTable("tt") {
        withTempDir(dir => {
          ...
          val tableForUpsert1 = Seq(
            (3, "range1", "hash1-2", "update"),
            (4, "range2", "hash2-2", "insert"),
            (5, "range1", "hash1-5", "insert"),
            (6, "range1", "hash1-1", "delete")
          )
            .toDF("id", "range", "hash", "op")
          Thread.sleep(2000)
          lake.upsert(tableForUpsert1)
          val tableForUpsert2 = Seq(
            (7, "range2", "hash2-5", "insert"),
            (8, "range2", "hash2-10", "delete")
          )
            .toDF("id", "range", "hash", "op")
          Thread.sleep(2000)
