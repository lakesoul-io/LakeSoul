...
class NativeLakeSoulFileFormatSuite extends AnyFunSuite {
  ...
  test("file_format option selects vortex extension case-insensitively") {
    ...
    assert(
      fileExtension(job, Map(LakeSoulOptions.FILE_FORMAT -> "VORTEX"))
        .endsWith(".vortex")
    )
  }
  ...
  test("file_format option selects vortex compact extension") {
    ...
    assert(
      fileExtension(job, Map(LakeSoulOptions.FILE_FORMAT -> "vortex-compact"))
        .endsWith(".vortex")
    )
  }
  ...
  test("file_format option keeps parquet extension") {
    ...
    assert(
      fileExtension(job, Map(LakeSoulOptions.FILE_FORMAT -> "parquet"))
        .endsWith(".parquet")
    )
  }
