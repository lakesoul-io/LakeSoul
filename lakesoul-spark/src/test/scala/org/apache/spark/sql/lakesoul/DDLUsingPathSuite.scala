...
trait DDLUsingPathTests
  ...
  protected def testUsingPath(command: String, tags: Tag*)(
      f: (String, String, String) => Unit
  ): Unit = {
  ...
  testUsingPath("SELECT") { (ns, table, path) =>
    Seq(s"`lakesoul`.`$ns`.`$table`", s"`lakesoul`.`$ns`.`$path`").foreach {
      tableOrPath =>
    ...
    assert(
      ex.getMessage.contains(
        s"The table or view `lakesoul`.`$ns`.`/path/to/lakesoul` cannot be found"
      )
    )
    if (ns == "default") {
  ...
  testUsingPath("DESCRIBE TABLE") { (ns, table, path) =>
    ...
    Seq(s"`lakesoul`.`$ns`.`$table`", s"`lakesoul`.`$ns`.`$path`").foreach {
      tableOrPath =>
        ...
        checkDescribe(
          s"DESCRIBE EXTENDED $tableOrPath",
          "v1" -> "int",
          "v2" -> "string",
          "struct" -> "struct<x:int,y:string>",
          "Provider" -> "lakesoul",
      ...
      withTable("lakesoul_test") {
        ...
        checkAnswer(
          spark.read
            .format("lakesoul")
            .load(tblPath)
            .filter("id = 2 and data = 'c'")
            .select("id", "data"),
          Seq(Row(2, "c"))
        )
        checkAnswer(
          spark.read.format("lakesoul").load(tblPath).filter("id = 3"),
          Seq.empty
        )
        checkAnswer(
          spark.read.format("lakesoul").load(tblPath).filter("data = 'd'"),
          Seq.empty
        )
      }
    }
