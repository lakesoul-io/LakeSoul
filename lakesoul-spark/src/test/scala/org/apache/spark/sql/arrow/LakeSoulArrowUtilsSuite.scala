...
class LakeSoulArrowUtilsSuite extends AnyFunSuite {
  ...
  test("large offset types are lossless in metadata and regular in execution") {
    ...
    val largeList =
      field("items", ArrowType.LargeList.INSTANCE, children = Seq(listElement))
    ...
    val original =
      new Schema(Seq(largeText, largeBinary, largeList, nestedLarge).asJava)
    ...
    assert(
      sparkSchema("items").dataType == ArrayType(
        StringType,
        containsNull = false
      )
    )
    ...
    assert(
      execution
        ...
        .getChildren
        .get(0)
        .getType == ArrowType.Utf8.INSTANCE
    )
    assert(
      execution
        .findField("nested")
        .getChildren
        .get(0)
        .getType == ArrowType.Binary.INSTANCE
    )
  ...
  test("metadata restoration follows current nested Spark schema") {
    ...
    val restored = ArrowUtils
      .toMetadataArrowSchema(StructType(Seq(evolvedField)))
      .findField("items")
    ...
    assert(
      restored.getChildren
        .get(0)
        .getChildren
        .get(0)
        .getType == ArrowType.LargeUtf8.INSTANCE
    )
    assert(
      restored.getChildren
        .get(0)
        .getChildren
        .get(1)
        .getType == new ArrowType.Int(64, true)
    )
  }

  test("incompatible Spark type change discards the old Arrow type hint") {
    val original =
      new Schema(Seq(field("value", ArrowType.LargeUtf8.INSTANCE)).asJava)
    val loaded = ArrowUtils.fromArrowSchema(original)
    val changed = StructType(Seq(loaded("value").copy(dataType = BinaryType)))

    assert(
      ArrowUtils.toMetadataArrowSchema(changed).findField("value").getType ==
  ...
  test("existing time and timestamp hints use structured restoration") {
    val original = new Schema(
      Seq(
        ...
        field("timestamp", new ArrowType.Timestamp(TimeUnit.MICROSECOND, null))
      ).asJava
    )
    val loaded = ArrowUtils.fromArrowSchema(original)
  ...
  test("invalid Arrow field marker reports the field name") {
    ...
    val schema =
      StructType(Seq(StructField("broken", StringType, metadata = metadata)))
  ...
  private def field(
    ...
    new Field(
      ...
      new FieldType(
        ...
        dataType,
        null,
        new util.HashMap(metadata.asJava)
      ),
      children.asJava
    )
  }
