...
class UpsertWithDuplicateDataByDifferent {
  ...
  private def execute(onlyOnePartition: Boolean): Unit = {
    ...
    try {
      val nameData = TestUtils
        .getData1(20000, onlyOnePartition)
        .toDF("hash", "name", "range")
        .persist()

      val nameDataDistinct = nameData
        .groupBy("range", "hash")
        .agg(last("name").as("n"))
        .select(col("range"), col("hash"), col("n").as("name"))
        .persist()

      lazy val ageData = TestUtils
        .getData1(18000, onlyOnePartition)
        .toDF("hash", "age", "range")
        .persist()
      lazy val ageDataDistinct = ageData
        .groupBy("range", "hash")
        .agg(last("age").as("n"))
        .select(col("range"), col("hash"), col("n").as("age"))
        .persist()

      lazy val stuData = TestUtils
        .getData1(23000, onlyOnePartition)
        .toDF("hash", "stu", "range")
        .persist()
      lazy val stuDataDistinct = stuData
        .groupBy("range", "hash")
        .agg(last("stu").as("n"))
        .select(col("range"), col("hash"), col("n").as("stu"))
        .persist()

      lazy val gradeData = TestUtils
        .getData1(15000, onlyOnePartition)
        .toDF("hash", "grade", "range")
        .persist()
      lazy val gradeDataDistinct = gradeData
        .groupBy("range", "hash")
        .agg(last("grade").as("n"))
        .select(col("range"), col("hash"), col("n").as("grade"))
        .persist()
