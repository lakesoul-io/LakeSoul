...
trait AlterTableLakeSoulTestBase
  ...
  final protected def withLakeSoulTable(schema: String, ns: String)(
      f: String => Unit
  ): Unit = {
  ...
  final protected def withLakeSoulTable(df: DataFrame, ns: String)(
      f: String => Unit
  ): Unit = {
  ...
  protected def assertNotSupported(command: String, messages: String*): Unit = {
    ...
    assert(
      ex.contains("not supported") || ex.contains("Unsupported") || ex.contains(
        "Cannot"
      )
    )
...
trait AlterTableTests extends AlterTableLakeSoulTestBase {
  ...
  ddlTest("ADD COLUMNS - simple") { ns =>
    withLakeSoulTable(Seq((1, "a"), (2, "b")).toDF("v1", "v2"), ns) {
      tableName =>
        ...
        checkDatasetUnorderly(
          spark
            .table(tableName)
            .as[(Int, String, Option[Long], Option[Double])],
          (1, "a", None, None),
          (2, "b", None, None)
        )
    }
  ...
  ddlTest(
    "ADD COLUMNS should not be able to add column to basic type key/value of " +
      "MapType"
  ) { ns =>
    withLakeSoulTable(
      Seq((1, "a"), (2, "b"))
        .toDF("v1", "v2")
        .withColumn("m", map('v1, 'v2)),
      ns
    ) { tableName =>
  ...
  ddlTest("ADD COLUMNS into complex types - Map") { ns =>
    withLakeSoulTable(
      Seq((1, "a"), (2, "b"))
        .toDF("v1", "v2")
        .withColumn("m", map(struct("v1"), struct("v2"))),
      ns
    ) { tableName =>
      sql(s"""
  ...
  ddlTest("ADD COLUMNS - a partitioned table") { ns =>
    withLakeSoulTable(Seq((1, "a"), (2, "b")).toDF("v1", "v2"), ns, Seq("v2")) {
      tableName =>
        checkDatasetUnorderly(
          spark.table(tableName).as[(Int, String)],
          (1, "a"),
  ...
  ddlTest("ADD COLUMNS - with a comment") { ns =>
    withLakeSoulTable(Seq((1, "a"), (2, "b")).toDF("v1", "v2"), ns) {
      tableName =>
        ...
        sql(
          s"ALTER TABLE $tableName ADD COLUMNS (v3 long COMMENT 'new column')"
        )
  ...
  test("ADD COLUMNS - with positions") {
    ...
    withLakeSoulTable(df, "default") { tableName =>
      ...
      sql(
        s"ALTER TABLE $tableName ADD COLUMNS (v3 long FIRST, v4 long AFTER v1, v5 long)"
      )
      ...
      checkDatasetUnorderly(
        spark
          .table(tableName)
          .as[(Option[Long], Int, Option[Long], String, Option[Long])],
        (None, 1, None, "a", None),
        (None, 2, None, "b", None)
      )
    }
  ...
  test("ADD COLUMNS - with positions using an added column") {
    ...
    withLakeSoulTable(df, "default") { tableName =>
      ...
      sql(
        "ALTER TABLE lakesoul_test ADD COLUMNS (v3 long FIRST, v4 long AFTER v3, v5 long AFTER v4)"
      )
      ...
      checkDatasetUnorderly(
        spark
          .table("lakesoul_test")
          .as[(Option[Long], Option[Long], Option[Long], Int, String)],
        (None, None, None, 1, "a"),
        (None, None, None, 2, "b")
      )
    }
  ...
  test("ADD COLUMNS - nested columns") {
    ...
    withLakeSoulTable(df, "default") { tableName =>
      ...
      sql(
        "ALTER TABLE lakesoul_test ADD COLUMNS " +
        ...
        spark
          ...
          .as[
            (
                Int,
                String,
                (Option[Long], Int, Option[Long], String, Option[Long])
            )
          ],
        (1, "a", (None, 1, None, "a", None)),
        (2, "b", (None, 2, None, "b", None))
      )
    }
  }
  ...
  ddlTest("CHANGE COLUMN - add a comment") { ns =>
    withLakeSoulTable(Seq((1, "a"), (2, "b")).toDF("v1", "v2"), ns) {
      tableName =>
        sql(
          s"ALTER TABLE $tableName CHANGE COLUMN v1 v1 integer COMMENT 'a comment'"
        )
  ...
  ddlTest("CHANGE COLUMN - add a comment to a partitioned table") { ns =>
    withLakeSoulTable(Seq((1, "a"), (2, "b")).toDF("v1", "v2"), ns, Seq("v2")) {
      tableName =>
        sql(
          s"ALTER TABLE $tableName CHANGE COLUMN v2 v2 string COMMENT 'a comment'"
        )
  ...
  ddlTest("CHANGE COLUMN - add a comment to a MapType (nested)") { ns =>
    val table = Seq((1, "a"), (2, "b"))
      .toDF("v1", "v2")
      .withColumn(
        "a",
        array(struct(array(struct(map(struct("v1"), struct("v2"))))))
      )
  ...
  ddlTest("CHANGE COLUMN - add a comment to an ArrayType (nested)") { ns =>
    withLakeSoulTable(
      Seq((1, "a"), (2, "b"))
        ...
        .withColumn(
          "m",
          map(struct("v1"), struct(array(struct(struct("v1")))))
        ),
      ns
    ) { tableName =>
      sql(s"""
  ...
  ddlTest("CHANGE COLUMN - add a comment to an ArrayType") { ns =>
    withLakeSoulTable(
      ...
      sql(
        s"ALTER TABLE $tableName CHANGE COLUMN a a ARRAY<int> COMMENT 'a comment'"
      )
  ...
  test("change column - type") {
    withLakeSoulTable(Seq((1, "a"), (2, "b")).toDF("v1", "v2"), "default") {
      tableName =>
  ...
  test("CHANGE COLUMN - move to after an unknown column (nested)") {
    ...
    withLakeSoulTable(df, "default") { tableName =>
      val ex = intercept[AnalysisException] {
        sql(
          s"ALTER TABLE $tableName CHANGE COLUMN struct.v1 v1 integer AFTER unknown"
        )
      }
  ...
  ddlTest("CHANGE COLUMN - add a comment (nested)") { ns =>
    ...
    withLakeSoulTable(df, ns) { tableName =>
      sql(
        s"ALTER TABLE $tableName CHANGE COLUMN struct.v1 v1 integer COMMENT 'a comment'"
      )
  ...
  ddlTest(
    "CHANGE COLUMN - complex types not supported because behavior is ambiguous"
  ) { ns =>
    val df = Seq((1, "a"), (2, "b"))
      .toDF("v1", "v2")
      .withColumn("s", struct("v1", "v2"))
  ...
  test("CHANGE COLUMN - move unknown column") {
    ...
    withLakeSoulTable(df, "default") { tableName =>
      val ex = intercept[AnalysisException] {
        sql(
          s"ALTER TABLE $tableName CHANGE COLUMN unknown unknown string FIRST"
        )
      }
      assert(ex.getMessage.contains("Missing field unknown"))
    }
  ...
  test("CHANGE COLUMN - move unknown column (nested)") {
    ...
    withLakeSoulTable(df, "default") { tableName =>
      val ex = intercept[AnalysisException] {
        sql(
          s"ALTER TABLE $tableName CHANGE COLUMN struct.unknown unknown string FIRST"
        )
      }
      assert(ex.getMessage.contains("Missing field struct.unknown"))
    }
...
trait AlterTableByNameTests extends AlterTableTests {
  ...
  override protected def createTable(
    ...
    val props = tblProperties
      .map { case (key, value) =>
        s"'$key' = '$value'"
      }
      .mkString(", ")
    val propsString =
      if (tblProperties.isEmpty) "" else s" TBLPROPERTIES ($props)"
  ...
  override protected def createTable(
    ...
    df.write
      .option("rangePartitions", partitionedBy.mkString(","))
      .format("lakesoul")
      .saveAsTable(s"$ns.lakesoul_test")
    s"$ns.lakesoul_test"
  }
  ...
  override protected def getSnapshotManagement(
      identifier: String
  ): SnapshotManagement = {
    val parts = identifier.split("\\.")
    if (parts.length == 1) {
  ...
  test("SET TBLPROPERTIES") {
    withTempDir { dir =>
      withTable("lakesoul_test") {
        val path = dir.getCanonicalPath
        Seq((1, "update"), (2, "insert"))
          .toDF("col1", "cdc_change_kind")
          .write
          .format("lakesoul")
          .option("path", path)
          .saveAsTable("lakesoul_test")
        sql(
          "ALTER TABLE lakesoul_test SET TBLPROPERTIES ('lakesoul_cdc_column'='cdc_change_kind')"
        )

        val table =
          catalog.loadTable(Identifier.of(Array("default"), "lakesoul_test"))
        getProperties(table) should contain(
          "lakesoul_cdc_column" -> "cdc_change_kind"
        )
      }
    }
  }
...
/** For ByPath tests, we select a test case per ALTER TABLE command to simply
...
trait AlterTableByPathTests extends AlterTableLakeSoulTestBase {
  override protected def createTable(
    ...
    val newTableInfo =
      tc.tableInfo.copy(table_schema = StructType.fromDDL(schema).json)
  ...
  override protected def dropTable(identifier: String): Unit = {
    LakeSoulTable
      .forPath(identifier.split("\\.").last.stripPrefix("`").stripSuffix("`"))
      .dropTable()
  }
  ...
  override protected def getSnapshotManagement(
    ...
    SnapshotManagement(
      SparkUtil
        .makeQualifiedTablePath(
          new Path(
            identifier.split("\\.").last.stripPrefix("`").stripSuffix("`")
          )
        )
        .toUri
        .toString
      ...
      withTempDir { dir =>
        ...
        assert(
          e.getMessage.contains(
            "`ALTER TABLE xxx SET LOCATION '/xxx'` is not supported"
          )
        )
      }
    }
...
@RunWith(classOf[JUnitRunner])
class AlterTableByPathSuite
    extends AlterTableByPathTests
    with LakeSoulSQLCommandTest
