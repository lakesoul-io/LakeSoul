...
class LakeSoulSchemaUtilsSuite
    extends QueryTest
  ...
  /** Tests change of datatype within a schema.
    *   - the make() function is a "factory" function to create schemas that
    *     vary only by the
    * given datatype in a specific position in the schema.
    *   - other tests will call this method with different make() functions to
    *     test datatype
    * incompatibility in all the different places within a schema (in a
    * top-level struct, in a nested struct, as the element type of an array,
    * etc.)
    */
  def testDatatypeChange(
      scenario: String
  )(make: DataType => StructType): Unit = {
    val schemas = Map(
  ...
  /** Tests change of nullability within a schema (making a field nullable is
    * not allowed, but making a nullable field non-nullable is ok).
    *   - the make() function is a "factory" function to create schemas that
    *     vary only by the
    * nullability (of a field, array elemnt, or map values) in a specific
    * position in the schema.
    *   - other tests will call this method with different make() functions to
    *     test nullability
    * incompatibility in all the different places within a schema (in a
    * top-level struct, in a nested struct, for the element type of an array,
    * etc.)
    */
  def testNullability(scenario: String)(make: Boolean => StructType): Unit = {
    ...
    test(
      s"restricted nullability should not fail read compatibility - $scenario"
    ) {
      assert(isReadCompatible(nullable, nonNullable))
    }
  }

  /** Tests for fields of a struct: adding/dropping fields, changing
    * nullability, case variation
    *   - The make() function is a "factory" method to produce schemas. It takes
    *     a function that
    * mutates a struct (for example, but adding a column, or it could just not
    * make any change).
    *   - Following tests will call this method with different factory methods,
    *     to mutate the
    * various places where a struct can appear (at the top-level, nested in
    * another struct, within an array, etc.)
    *   - This allows us to have one shared code to test compatibility of a
    *     struct field in all the
    * different places where it may occur.
    */
  def testColumnVariations(
      scenario: String
    ...
    val withoutExtra =
      make(struct => struct) // produce struct WITHOUT extra field
    ...
    val withExtraNonNullable =
      make(struct => struct.add("extra", StringType, nullable = false))

    test(s"dropping a field should fail read compatibility - $scenario") {
      assert(!isReadCompatible(withExtraNullable, withoutExtra))
    }
    test(
      s"adding a nullable field should not fail read compatibility - $scenario"
    ) {
      assert(isReadCompatible(withoutExtra, withExtraNullable))
    }
    test(
      s"adding a non-nullable field should not fail read compatibility - $scenario"
    ) {
      assert(isReadCompatible(withoutExtra, withExtraNonNullable))
    }
    test(
      s"case variation of field name should fail read compatibility - $scenario"
    ) {
      assert(!isReadCompatible(withExtraNullable, withExtraMixedCase))
    }
    testNullability(scenario)(b =>
      make(struct => struct.add("extra", StringType, nullable = b))
    )
  ...
  testColumnVariations("top level")(f =>
    f(new StructType().add("a", IntegerType))
  )

  testColumnVariations("nested struct")(f =>
    new StructType()
      .add("a", f(new StructType().add("b", IntegerType)))
  )

  testColumnVariations("nested in array")(f =>
    new StructType()
      .add("array", ArrayType(f(new StructType().add("b", IntegerType))))
  )

  testColumnVariations("nested in map key")(f =>
    new StructType()
      .add(
        "map",
        MapType(f(new StructType().add("b", IntegerType)), StringType)
      )
  )

  testColumnVariations("nested in map value")(f =>
    new StructType()
      .add(
        "map",
        MapType(StringType, f(new StructType().add("b", IntegerType)))
      )
  )
  ...
  testDatatypeChange("array element")(datatype =>
    new StructType()
      .add("array", ArrayType(datatype))
  )

  testDatatypeChange("map key")(datatype =>
    new StructType()
      .add("map", MapType(datatype, StringType))
  )

  testDatatypeChange("map value")(datatype =>
    new StructType()
  ...
  testNullability("array contains null")(b =>
    new StructType()
      .add("array", ArrayType(StringType, containsNull = b))
  )

  testNullability("map contains null values")(b =>
    new StructType()
      .add("map", MapType(IntegerType, StringType, valueContainsNull = b))
  )

  testNullability("map nested in array")(b =>
    new StructType()
      .add(
        "map",
        ArrayType(MapType(IntegerType, StringType, valueContainsNull = b))
      )
  )

  testNullability("array nested in map")(b =>
    new StructType()
      .add("map", MapType(IntegerType, ArrayType(StringType, containsNull = b)))
  )
  ...
  /** @param existing
    *   the existing schema to compare to
    * @param specified
    *   the new specified schema
    * @param expected
    *   an expected list of messages, each describing a schema difference. Every
    *   expected message is actually a regex patterns that is matched against
    *   all diffs that are returned. This is necessary to tolerate variance in
    *   ordering of field names, for example in a message such as "Specified
    *   schema has additional field(s): x, y", we cannot predict the order of x
    *   and y.
    */
  def testReportDifferences(
      testName: String
    ...
    test(testName) {
      ...
      expected foreach ((exp: String) =>
        assert(
          differences.exists(message =>
            exp.r.findFirstMatchIn(message).isDefined
          ),
      ...
      assert(
        expected.size == differences.size,
  ...
  testReportDifferences(
    "making a column nullable should be reported as a difference"
  )(
    existing = new StructType()
    ...
    specified = new StructType()
      ...
      .add("b", StringType, nullable = true),
    expected =
      "a is nullable in specified schema but non-nullable in existing schema"
  )

  testReportDifferences(
    "making a column non-nullable should be reported as a difference"
  )(
    existing = new StructType()
      .add("a", IntegerType, nullable = false)
      .add("b", StringType, nullable = true),
    ...
    expected =
      "b is non-nullable in specified schema but nullable in existing schema"
  )

  testReportDifferences(
    "change in column metadata should be reported as a difference"
  )(
    existing = new StructType()
      .add(
        "a",
    ...
    specified = new StructType()
      .add(
        ...
        IntegerType,
        nullable = true,
        new MetadataBuilder().putString("x", "2").build()
      )
      .add("b", StringType),
  ...
  testReportDifferences(
    "change of column type should be reported as a difference"
  )(
    existing = new StructType()
      .add("a", IntegerType)
      .add("b", StringType),
    specified = new StructType()
      .add("a", IntegerType)
      .add("b", new ArrayType(StringType, containsNull = false)),
    expected = "type for b is different"
  )

  testReportDifferences(
    "change of array nullability should be reported as a difference"
  )(
    existing = new StructType()
      .add("a", IntegerType)
      .add(
        "b",
        new ArrayType(new StructType().add("x", LongType), containsNull = true)
      ),
    specified = new StructType()
      .add("a", IntegerType)
      .add(
        "b",
        new ArrayType(new StructType().add("x", LongType), containsNull = false)
      ),
    expected =
      "b\\[\\] can not contain null in specified schema but can in existing"
  )

  testReportDifferences(
    "change of element type should be reported as a difference"
  )(
    existing = new StructType()
      .add("a", IntegerType)
  ...
  testReportDifferences(
    "change of element struct type should be reported as a difference"
  )(
    existing = new StructType()
      .add("a", IntegerType)
      .add(
  ...
  testReportDifferences(
    "change of map value nullability should be reported as a difference"
  )(
    existing = new StructType()
      .add("a", IntegerType)
      .add(
    ...
    expected =
      "b can not contain null values in specified schema but can in existing"
  )

  testReportDifferences(
    "change of map key type should be reported as a difference"
  )(
    existing = new StructType()
      .add("a", IntegerType)
  ...
  testReportDifferences(
    "change of value struct type should be reported as a difference"
  )(
    existing = new StructType()
      .add("a", IntegerType)
      .add(
  ...
  testReportDifferences(
    "nested extra columns should be reported as a difference"
  )(
    existing = new StructType()
      .add(
        "x",
  ...
  testReportDifferences(
    "nested missing columns should be reported as a difference"
  )(
    existing = new StructType()
      .add(
        "x",
  ...
  testReportDifferences(
    "making a nested column nullable should be reported as a difference"
  )(
    existing = new StructType()
      .add(
        "x",
    ...
    expected =
      "x.a is nullable in specified schema but non-nullable in existing schema"
  )

  testReportDifferences(
    "making a nested column non-nullable should be reported as a difference"
  )(
    existing = new StructType()
      .add(
        "x",
    ...
    expected =
      "x.b is non-nullable in specified schema but nullable in existing schema"
  )

  testReportDifferences(
    "change in nested column metadata should be reported as a difference"
  )(
    existing = new StructType()
      .add(
        "x",
  ...
  testReportDifferences(
    "change of nested column type should be reported as a difference"
  )(
    existing = new StructType()
      .add(
        "x",
  ...
  testReportDifferences(
    "change of nested array nullability should be reported as a difference"
  )(
    existing = new StructType()
      .add(
        "x",
    ...
    expected =
      "x.b\\[\\] can not contain null in specified schema but can in existing"
  )

  testReportDifferences(
    "change of nested element type should be reported as a difference"
  )(
    existing = new StructType()
      .add(
        "x",
  ...
  testReportDifferences(
    "change of nested element struct type should be reported as a difference"
  )(
    existing = new StructType()
      .add(
        "x",
  ...
  private val piiTrue =
    new MetadataBuilder().putBoolean("pii", value = true).build()
  private val piiFalse =
    new MetadataBuilder().putBoolean("pii", value = false).build()
  ...
  testReportDifferences("multiple differences should be reported")(
    existing = new StructType()
      ...
      .add("h", new MapType(LongType, StringType, valueContainsNull = true)),
    specified = new StructType()
      .add("a", FloatType)
      .add("d", StringType)
      .add("e", LongType)
  ...
  test("findColumnPosition") {
    ...
    assert(
      SchemaUtils.findColumnPosition(Seq("a", "b"), schema) === ((Seq(0, 0), 0))
    )
    assert(
      SchemaUtils.findColumnPosition(Seq("A", "b"), schema) === ((Seq(0, 0), 0))
    )
    ...
    assert(
      SchemaUtils.findColumnPosition(Seq("a", "c"), schema) === ((Seq(0, 1), 0))
    )
    assert(SchemaUtils.findColumnPosition(Seq("d"), schema) === ((Seq(1), 2)))
    assert(
      SchemaUtils.findColumnPosition(Seq("d", "element", "B"), schema) === ((
        Seq(1, 0, 0),
        0
      ))
    )
    assert(
      SchemaUtils.findColumnPosition(Seq("d", "element", "c"), schema) === ((
        Seq(1, 0, 1),
        0
      ))
    )
    ...
    assert(
      SchemaUtils.findColumnPosition(Seq("f", "key", "g"), schema) === ((
        Seq(3, 0, 0),
        0
      ))
    )
    assert(
      SchemaUtils.findColumnPosition(Seq("f", "value", "h"), schema) === ((
        Seq(3, 1, 0),
        0
      ))
    )
    ...
    assert(
      SchemaUtils
        .findColumnPosition(Seq("i", "key"), schema) === ((Seq(4, 0), 0))
    )
    assert(
      SchemaUtils.findColumnPosition(Seq("i", "value", "k"), schema) === ((
        Seq(4, 1, 0),
        1
      ))
    )
    ...
    assert(
      SchemaUtils
        .findColumnPosition(Seq("i", "value"), schema) === ((Seq(4, 1), 1))
    )
  ...
  test("addColumn - simple") {
    ...
    assert(
      SchemaUtils
        .addColumn(schema, x, Seq(0)) === new StructType().add(x).add(a).add(b)
    )
    assert(
      SchemaUtils
        .addColumn(schema, x, Seq(1)) === new StructType().add(a).add(x).add(b)
    )
    assert(
      SchemaUtils
        .addColumn(schema, x, Seq(2)) === new StructType().add(a).add(b).add(x)
    )
  ...
  test("addColumn - nested struct") {
    ...
    assert(
      SchemaUtils
        .addColumn(schema, x, Seq(0)) === new StructType().add(x).add(s)
    )
    assert(
      SchemaUtils.addColumn(schema, x, Seq(0, 0)) ===
    ...
    assert(
      SchemaUtils.addColumn(schema, x, Seq(0, 2)) ===
        new StructType().add("s", new StructType().add(a).add(b).add(x))
    )
    assert(
      SchemaUtils
        .addColumn(schema, x, Seq(1)) === new StructType().add(s).add(x)
    )
  ...
  test("dropColumn - simple") {
    ...
    assert(
      SchemaUtils.dropColumn(schema, Seq(0)) === ((new StructType().add(b), a))
    )
    assert(
      SchemaUtils.dropColumn(schema, Seq(1)) === ((new StructType().add(a), b))
    )
  ...
  test("normalize column names") {
    ...
    val schema = new StructType()
      ...
      .add("ghi", IntegerType)
    assert(
      normalizeColumnNames(schema, df).schema.fieldNames === schema.fieldNames
    )
  }
  ...
  test("normalize column names - different ordering") {
    ...
    val schema = new StructType()
      ...
      .add("ghi", IntegerType)
    assert(
      normalizeColumnNames(schema, df).schema.fieldNames === Seq(
        "Def",
        "ghi",
        "abc"
      )
    ...
    expectFailure("'array'", "scale") {
      mergeSchemas(
        ...
        new StructType()
          .add(
            "array",
            ArrayType(new StructType().add("b", DecimalType(18, 12)))
          )
      )
    }
    expectFailure("'array'", "precision") {
      mergeSchemas(
        ...
        new StructType()
          .add(
            "array",
            ArrayType(new StructType().add("b", DecimalType(16, 10)))
          )
      )
    }
  ...
  test(
    "schema merging performs upcast between ByteType, ShortType, and LongType"
  ) {
    ...
    val structInt =
      new StructType().add("top", new StructType().add("leaf", IntegerType))
    val structShort =
      new StructType().add("top", new StructType().add("leaf", ShortType))
    assert(mergeSchemas(structInt, structShort) === structInt)

    val map1 =
      new StructType().add("top", new MapType(IntegerType, ShortType, true))
    val map2 =
      new StructType().add("top", new MapType(ShortType, IntegerType, true))
    val mapMerged =
      new StructType().add("top", new MapType(IntegerType, IntegerType, true))
  ...
  test("Upcast between ByteType, ShortType and IntegerType is OK for parquet") {
    ...
    def testParquetUpcast(): Unit = {
      withTempDir { dir =>
        ...
        spark
          .range(1.toByte)
          .select(col("id") cast ByteType)
          .write
          .save(tempDir + "/byte")
        spark
          .range(1.toShort)
          .select(col("id") cast ShortType)
          .write
          .save(tempDir + "/short")
        spark
          .range(1)
        ...
        spark.read
          .schema(shortSchema)
          .parquet(tempDir + "/byte")
          .collect() === Seq(Row(1.toShort))
        spark.read
          .schema(intSchema)
          .parquet(tempDir + "/short")
          .collect() === Seq(Row(1))
        spark.read
          .schema(intSchema)
          .parquet(tempDir + "/byte")
  ...
  test("transform columns - simple") {
    ...
    // Identity.
    var visitedFields = 0
    val res1 = transformColumns(base) { case (Seq(), field, _) =>
      visitedFields += 1
      field
    }
    ...
    // Rename a -> c
    visitedFields = 0
    val res2 = transformColumns(base) { case (Seq(), field, _) =>
      visitedFields += 1
      val name = field.name
  ...
  test("transform columns - nested") {
    ...
    // Identity.
    var visitedFields = 0
    val res1 = transformColumns(base) { case (_, field, _) =>
      visitedFields += 1
      field
    }
