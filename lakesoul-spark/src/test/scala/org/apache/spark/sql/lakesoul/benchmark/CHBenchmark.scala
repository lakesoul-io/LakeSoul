...
object CHBenchmark {
  ...
  val query_1 =
    "SELECT ol_number, sum(ol_quantity) AS sum_qty, sum(ol_amount) AS sum_amount, avg(ol_quantity) AS avg_qty, avg(ol_amount) AS avg_amount, count(*) AS count_order FROM `order_line` WHERE ol_delivery_d > TIMESTAMP '2007-01-02 00:00:00.000000' GROUP BY ol_number ORDER BY ol_number"
  val query_2 =
  ...
  val query_3 =
    "SELECT ol_o_id, ol_w_id, ol_d_id, sum(ol_amount) AS revenue, o_entry_d FROM `customer`, `new_order`, `oorder`, `order_line` WHERE c_state LIKE 'A%' AND c_id = o_c_id AND c_w_id = o_w_id AND c_d_id = o_d_id AND no_w_id = o_w_id AND no_d_id = o_d_id AND no_o_id = o_id AND ol_w_id = o_w_id AND ol_d_id = o_d_id AND ol_o_id = o_id AND o_entry_d > TIMESTAMP '2007-01-02 00:00:00.000000' GROUP BY ol_o_id, ol_w_id, ol_d_id, o_entry_d ORDER BY revenue DESC , o_entry_d"
  val query_4 =
  ...
  val query_9 =
    "SELECT n_name, extract(YEAR FROM o_entry_d) AS l_year, sum(ol_amount) AS sum_profit FROM `item`, `stock`, `supplier`, `order_line`, `oorder`, `nation` WHERE ol_i_id = s_i_id AND ol_supply_w_id = s_w_id AND MOD ((s_w_id * s_i_id), 10000) = su_suppkey AND ol_w_id = o_w_id AND ol_d_id = o_d_id AND ol_o_id = o_id AND ol_i_id = i_id AND su_nationkey = n_nationkey AND i_data LIKE '%bb' GROUP BY n_name, extract(YEAR FROM o_entry_d) ORDER BY n_name, extract(YEAR FROM o_entry_d)  DESC"
  val query_10 =
    "SELECT c_id, c_last, sum(ol_amount) AS revenue, c_city, c_phone, n_name FROM `customer`, `oorder`, `order_line`, `nation` WHERE c_id = o_c_id AND c_w_id = o_w_id AND c_d_id = o_d_id AND ol_w_id = o_w_id AND ol_d_id = o_d_id AND ol_o_id = o_id AND o_entry_d >= TIMESTAMP '2007-01-02 00:00:00.000000' AND o_entry_d <= ol_delivery_d AND n_nationkey = ascii(substring(c_state from  1  for  1)) GROUP BY c_id, c_last, c_city, c_phone, n_name ORDER BY revenue DESC"
  val query_11 =
  ...
  val query_14 =
    "SELECT (100.00 * sum(CASE WHEN i_data LIKE 'PR%' THEN ol_amount ELSE 0 END) / (1 + sum(ol_amount))) AS promo_revenue FROM `order_line`, `item` WHERE ol_i_id = i_id AND ol_delivery_d >= TIMESTAMP'2007-01-02 00:00:00.000000' AND ol_delivery_d < TIMESTAMP '2020-01-02 00:00:00.000000'"
  val query_16 =
    "SELECT i_name, substring(i_data from  1 for 3) AS brand, i_price, count(DISTINCT (mod((s_w_id * s_i_id),10000))) AS supplier_cnt FROM `stock`, `item` WHERE i_id = s_i_id AND i_data NOT LIKE 'zz%' AND (mod((s_w_id * s_i_id),10000) NOT IN (SELECT su_suppkey FROM `supplier` WHERE su_comment LIKE '%bad%')) GROUP BY i_name, substring(i_data from  1 for 3), i_price ORDER BY supplier_cnt DESC"
  val query_17 =
  ...
  def queryTest(spark: SparkSession, query: String, queryNum: String): Unit = {
    ...
    println(
      printLine + queryNum + ":" + (System
        .currentTimeMillis() - startTime) + printLine
    )
  }
  ...
  def verifyQuery(
    ...
    val jdbcDF = spark.read
      ...
      .option("url", url)
      .option("dbtable", "(" + query.replace("LIKE", "LIKE binary") + ") as t")
    ...
    println(
      printLine + queryNum + " result: " + (jdbcDF.rdd
        .subtract(lakesoulDF.rdd)
        .count() == 0) + printLine
    )
  }
