...
class ManylinuxDockerImageBuilder(object):
    ...
    def _handle_rust_version(self, args):
        print(self._project_root_dir)
        with open(f"{self._project_root_dir}/rust/rust-toolchain.toml") as f:
            match = re.search(r'channel\s*=\s*"([^"]+)"', f.read())
            if match:
                channel_value = match.group(1)
                args += ["--build-arg", "RUST_VRESION=%s" % channel_value]
            else:
                raise RuntimeError("get rust version failed")

    def _build_docker_image(self):
        args = ["docker", "build"]
        self._handle_proxy(args)
        self._handle_rust_version(args)
