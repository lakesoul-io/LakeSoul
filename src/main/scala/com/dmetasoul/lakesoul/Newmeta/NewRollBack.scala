...
import com.dmetasoul.lakesoul.Newmeta.NewRollBack.logInfo
import com.dmetasoul.lakesoul.Newmeta.NewUndolog.{deleteUndoLog, getCommitTimestampAndTag, getUndoLogInfo, markOtherCommitRollBack}
import com.dmetasoul.lakesoul.Newmeta.{NewMetaLock, MetaCommon}
...
object NewRollBack extends Logging {
  def rollBackUpdate(meta_info: MetaInfo,
    ...
    if (tag != -1) {
      ...
      if (commitOptions.shortTableName.isDefined) {
        NewMetaUtil.deleteShortTableName(commitOptions.shortTableName.get, meta_info.table_info.table_name)
  ...
  def cleanRollBackCommit(table_id: String, commit_id: String, lock_id: String): Unit = {
    ...
    NewMetaLock.unlock(lock_id, commit_id)
  }
  ...
  def rollBackPartitionLock(table_id: String, commit_id: String): Unit = {
    ...
    for (partition_undo <- partition_undo_arr) {
      NewMetaLock.unlock(partition_undo.range_id, commit_id)
  ...
  private def rollBackShortTableName(table_id: String, commit_id: String): Unit = {
    ...
    if (info.nonEmpty) {
      NewMetaUtil.deleteShortTableName(info.head.short_table_name, info.head.table_name)
      deleteUndoLog(UndoLogType.ShortTableName.toString, table_id, commit_id)
    }
  }
