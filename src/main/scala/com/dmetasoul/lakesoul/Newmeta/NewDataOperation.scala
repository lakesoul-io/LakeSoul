...
object NewDataOperation extends Logging {
  ...
  def getSinglePartitionDataInfo(table_id: String,
    ...
    val partition_values = if (!range_value.equalsIgnoreCase(MetaCommon.DEFAULT_RANGE_PARTITION_VALUE)) {
      MetaCommon.getPartitionMapFromKey(range_value)
    } else {
