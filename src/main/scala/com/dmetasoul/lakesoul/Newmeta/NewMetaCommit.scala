...
import com.dmetasoul.lakesoul.Newmeta.NewRedo.redoCommit
import com.dmetasoul.lakesoul.Newmeta.NewRollBack.{cleanRollBackCommit, rollBackCommit, rollBackPartitionLock, rollBackUpdate}
import com.dmetasoul.lakesoul.Newmeta.NewUndolog._
...
object NewMetaCommit extends Logging {
  //meta commit process
  ...
  def isCommitTimeout(timestamp: Long): Boolean = {
    ...
    if (timeout > MetaCommon.COMMIT_TIMEOUT) {
      true
    } else {
      false
    }
  ...
  def getCommitState(table_name: String,
                     table_id: String,
                     commit_id: String,
                     time_limit: Long = MetaCommon.COMMIT_TIMEOUT): commitStateInfo = {
  ...
  def takeLockOfShortTableNameAndMaterialView(tableInfo: TableInfo,
    ...
    if (commitOptions.shortTableName.isDefined) {
      ...
      NewMetaUtil.addShortTableName(commitOptions.shortTableName.get, tableInfo.table_name)
    }
  ...
  def processShortTableNameAndMaterialView(new_meta_info: MetaInfo, commitOptions: CommitOptions): Unit = {
    ...
    if (commitOptions.materialInfo.isDefined) {
      ...
      if (materialInfo.isCreatingView) {
        ...
        NewMaterialView.updateMaterialView(
          view_name,
          materialInfo.relationTables.map(_.toString).mkString(","),
          materialInfo.autoUpdate
        )
  ...
  def lockSinglePartition(partition_info: PartitionInfo,
    ...
    //lock
    val (lock_flag, last_commit_id) = NewMetaLock.lock(partition_info.range_id, commit_id)
    ...
    if (!lock_flag) {
      ...
      state_info.state match {
        ...
        case CommitState.Committing =>
          if (times < MetaCommon.GET_LOCK_MAX_ATTEMPTS) {
            TimeUnit.SECONDS.sleep(MetaCommon.WAIT_LOCK_INTERVAL)
    ...
    try {
        ...
        try {
          ...
          case e: MetaRerunException if (e.getMessage.contains("another job drop and create a newer one.")
            ...
            val (partitionExists, now_range_id) =
              NewMetaUtil.getPartitionId(partition_info.table_id, partition_info.range_value)
            if (partitionExists) {
      ...
      case e: MetaNonfatalException =>
        if (times < MetaCommon.GET_LOCK_MAX_ATTEMPTS) {
          ...
          TimeUnit.SECONDS.sleep(scala.util.Random.nextInt((times + 1) * MetaCommon.RETRY_LOCK_INTERVAL))
  ...
  def updateMaterialRelation(table_id: String,
    ...
    val existsViews = NewMaterialView.getMaterialRelationInfo(table_id)
    ...
    NewMaterialView.updateMaterialRelationInfo(table_id, table_name, newViews)
  }
  ...
  def lockMaterialViewName(commit_id: String, materialViewName: String, isCreating: Boolean): Unit = {
    val (lock_flag, last_commit_id) = NewMetaLock.lock(formatLockMaterialView(materialViewName), commit_id)
    lazy val table_exists = NewMaterialView.isMaterialViewExists(materialViewName)
  ...
  def unlockMaterialViewName(commit_id: String, materialViewName: String): Unit = {
    NewMetaLock.unlock(formatLockMaterialView(materialViewName), commit_id)
  }

  def lockMaterialRelation(commit_id: String, table_id: String, table_name: String): Unit = {
    val (lock_flag, last_commit_id) = NewMetaLock.lock(formatLockMaterialView(table_id), commit_id)
    if (!lock_flag) {
  ...
  def unlockMaterialRelation(commit_id: String, table_id: String): Unit = {
    NewMetaLock.unlock(formatLockMaterialView(table_id), commit_id)
  }

  def lockSchema(meta_info: MetaInfo): Unit = {
    val (lock_flag, last_commit_id) = NewMetaLock.lock(meta_info.table_info.table_id, meta_info.commit_id)
    if (!lock_flag) {
      //check state and resolve last commit
  ...
  def takeSchemaLock(meta_info: MetaInfo): Unit = {
    ...
    addSchemaUndoLog(
      ...
      MetaCommon.toCassandraSetting(meta_info.table_info.configuration))

    //lock
    lockSchema(meta_info)

    //if schema had been changed, throw exception directly
    val current_schema_version = NewMetaUtil.getTableInfo(meta_info.table_info.table_name).schema_version
  ...
  def commitMetaInfo(meta_info: MetaInfo,
    ...
    for (partition_info <- meta_info.partitionInfoArray) {
      ...
      for (file <- partition_info.expire_files) {
        NewDataOperation.deleteExpireDataFile(
          table_id,
          range_id,
          file.file_path,
          write_version,
      ...
      for (file <- partition_info.add_files) {
        NewDataOperation.addNewDataFile(
          table_id,
          range_id,
          file.file_path,
          write_version,
      ...
      NewMetaUtil.updatePartitionInfo(
        table_id = table_id,
      ...
      NewMetaLock.unlock(partition_info.range_id, commit_id)
      deleteUndoLog(
    ...
    if (meta_info.query_id.nonEmpty && meta_info.batch_id >= 0) {
      NewStreamingRecord.updateStreamingInfo(table_id, meta_info.query_id, meta_info.batch_id, System.currentTimeMillis())
    }
  ...
  def fileConflictDetection(meta_info: MetaInfo): Unit = {
    ...
    meta_info.commit_type match {
      ...
      case CompactionCommit | PartCompactionCommit =>
        ...
        NewDataOperation.checkDataInfo(meta_info.commit_id, update_file_info_arr, false, true)
      case _ =>
        NewDataOperation.checkDataInfo(meta_info.commit_id, update_file_info_arr, true, true)
    }
  }
  ...
  def checkAndRedoCommit(snapshot: Snapshot): Unit = {
    ...
    val limit_timestamp = System.currentTimeMillis() - MetaCommon.COMMIT_TIMEOUT
    ...
    logInfo.foreach(log => {
      if (log.tag == -1) {
        if (!NewRedo.redoCommit(log.table_name, log.table_id, log.commit_id)) {
          flag = false
        }
      } else {
       NewRollBack.rollBackCommit(log.table_id, log.commit_id, log.tag, log.timestamp)
      }
    })
  ...
  def cleanUndoLog(snapshot: Snapshot): Unit = {
    val table_id: String = snapshot.getTableInfo.table_id
    checkAndRedoCommit(snapshot)
    val limit_timestamp = System.currentTimeMillis() - MetaCommon.UNDO_LOG_TIMEOUT

    val allType = UndoLogType.getAllType
    val log_info_seq = allType
      .map(NewUndolog.getTimeoutUndoLogInfo(_, table_id, limit_timestamp))
      .flatMap(_.toSeq)
    ...
    commit_log_info_seq
      .filter(log => !NewMetaUtil.isTableIdExists(log.table_name, log.table_id))
      ...
      .foreach(log => {
        log.commit_type match {
          case t: String if t.equalsIgnoreCase(UndoLogType.Schema.toString) =>
            NewMetaLock.unlock(log.table_id, log.commit_id)
          case t: String if t.equalsIgnoreCase(UndoLogType.Partition.toString) =>
            NewMetaLock.unlock(log.range_id, log.commit_id)
    ...
    val streaming_expire_timestamp = System.currentTimeMillis() - MetaCommon.STREAMING_INFO_TIMEOUT
    NewStreamingRecord.deleteStreamingInfoByTimestamp(table_id, streaming_expire_timestamp)
  ...
  def updatePartitionInfoAndGetNewMetaInfo(meta_info: MetaInfo): MetaInfo = {
    ...
    while (partition_info_itr.hasNext) {
      ...
      val currentPartitionInfo = NewMetaUtil
      ...
      NewUndolog.updatePartitionLogInfo(
        partition_info.table_id,
        commit_id,
        partition_info.range_id,
        write_version,
