...
public class BucketStateSerializer<BucketID> implements SimpleVersionedSerializer<BucketState<BucketID>> {
    ...
    BucketStateSerializer(SimpleVersionedSerializer<InProgressFileWriter.InProgressFileRecoverable> inProgressFileRecoverableSerializer, SimpleVersionedSerializer<InProgressFileWriter.PendingFileRecoverable> pendingFileRecoverableSerializer, SimpleVersionedSerializer<BucketID> bucketIdSerializer) {
    ...
    private BucketState<BucketID> deserializeV1(DataInputView in) throws IOException {
        ...
        if (in.readBoolean()) {
            current = new OutputStreamBasedPartFileWriter.OutputStreamBasedInProgressFileRecoverable((RecoverableWriter.ResumeRecoverable)SimpleVersionedSerialization.readVersionAndDeSerialize(resumableSerializer, in));
        }
        ...
        for(int i = 0; i < numCheckpoints; ++i) {
            ...
            for(int j = 0; j < noOfResumables; ++j) {
                ...
                in.readFully(bytes);
                pendingFileRecoverables.add(new OutputStreamBasedPartFileWriter.OutputStreamBasedPendingFileRecoverable((RecoverableWriter.CommitRecoverable)commitableSerializer.deserialize(committableVersion, bytes)));
            }
