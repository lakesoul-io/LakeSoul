...
fn test_config(index_name: &str, table: &str, dim: Option<usize>) -> GatewayConfig {
    GatewayConfig {
...
async fn scalar_u64(app_state: &lakesoul_es_gateway::GatewayState, sql: &str) -> u64 {
...
async fn gateway_write_path_contract() {
    ...
    let config = test_config(&index, &table, Some(3));
