...
use crate::common::{OnceExec, assert_spill_count_metric};
...
async fn test_sort_with_limited_memory() -> Result<()> {
    ...
    let record_batch_bytes = pool_size / 16;
    ...
    let spill_count = run_sort_test_with_limited_memory(RunTestWithLimitedMemoryArgs {
        ...
        get_size_of_record_batch_to_generate: Box::pin(move |_| record_batch_bytes),
        memory_behavior: Default::default(),
    })
    .await?;

    let total_spill_files_size = spill_count * record_batch_bytes;
    assert!(
