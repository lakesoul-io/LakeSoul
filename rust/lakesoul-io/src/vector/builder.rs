...
impl VectorShardIndexBuilder {
                ...
                std::thread::spawn(move || {
                    ...
                    rt.block_on(async move {
                        for fp in &fps {
                            ...
                            let cfg = b.build();
                            let mut r = match crate::reader::LakeSoulReader::new(cfg) {
                                Ok(r) => r,
                                Err(_) => continue,
                            };
    ...
    fn create_error_stream(
        &self,
    ) -> std::pin::Pin<Box<dyn futures::Stream<Item = crate::Result<IdAndVecBatch>> + Send>>
    {
        let (tx, rx) = tokio::sync::mpsc::channel::<crate::Result<IdAndVecBatch>>(16);
        let object_store_options = self.object_store_options.clone();
        let default_fs = self.default_fs.clone();
        let file_paths = self.file_paths.clone();
        let pk_column = self.pk_column.clone();
        ...
        std::thread::spawn(move || {
            ...
            rt.block_on(async move {
                for file_path in &file_paths {
                    ...
                    let mut reader = match crate::reader::LakeSoulReader::new(io_config) {
                        Ok(r) => r,
                        Err(e) => {
...
mod tests {
    ...
    async fn test_empty_stream() {
        let (tx, rx) = tokio::sync::mpsc::channel::<crate::Result<IdAndVecBatch>>(1);
