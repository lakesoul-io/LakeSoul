...
mod compaction_tests {
    ...
    use crate::catalog::{compaction, create_table};
    ...
    async fn test_simple_compaction() -> Result<()> {
        ...
        let _record_batch_1 = RecordBatch::try_from_iter_with_nullable(vec![
            (
                "range",
                Arc::new(Int32Array::from(vec![1, 2, 3, 1, 1])) as ArrayRef,
                false,
            ),
            (
                "hash",
                Arc::new(Int32Array::from(vec![1, 1, 1, 2, 3])) as ArrayRef,
                false,
            ),
            (
                "value",
        ...
        create_table(
            client.clone(),
            table_name,
            LakeSoulIOConfigBuilder::default().build(),
        )
        .await?;

        check_data_info().await?;

        let _record_batch_2 = RecordBatch::try_from_iter_with_nullable(vec![
            (
                "range",
                Arc::new(Int32Array::from(vec![1, 2, 3, 1, 1])) as ArrayRef,
                false,
    ...
    async fn test_upsert_after_compaction() -> Result<()> {
        ...
        let _record_batch_3 = RecordBatch::try_from_iter_with_nullable(vec![
            (
                "range",
                Arc::new(Int32Array::from(vec![1, 1, 1, 1])) as ArrayRef,
                false,
    ...
    async fn test_simple_compaction_with_merge_operator() -> Result<()> {
        ...
        let _record_batch_1 = RecordBatch::try_from_iter_with_nullable(vec![
            ...
            (
                "v2",
                Arc::new(StringArray::from(vec!["1", "1", "1", "2", "3"])) as ArrayRef,
                true,
            ),
        ])?;
