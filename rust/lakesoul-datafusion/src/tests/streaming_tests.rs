...
mod streaming_tests {
    ...
    async fn test_snapshot_read_with_single_partition() -> Result<()> {
        ...
        let record_batch_1 = RecordBatch::try_from_iter_with_nullable(vec![
            (
                "range",
                Arc::new(StringArray::from(vec!["range1", "range2"])) as ArrayRef,
                false,
            ),
            (
                "hash",
                Arc::new(StringArray::from(vec!["hash1-1", "hash2-1"])) as ArrayRef,
                false,
            ),
            (
                "op",
                Arc::new(StringArray::from(vec!["insert", "insert"])) as ArrayRef,
                true,
            ),
        ])?;

        let record_batch_2 = RecordBatch::try_from_iter_with_nullable(vec![
            (
                "range",
            ...
            (
                "hash",
                Arc::new(StringArray::from(vec![
                    "hash1-1", "hash1-5", "hash2-1", "hash2-5",
                ])) as ArrayRef,
                false,
            ),
            (
        ...
        let record_batch_3 = RecordBatch::try_from_iter_with_nullable(vec![
            (
                "range",
                Arc::new(StringArray::from(vec!["range1", "range2"])) as ArrayRef,
                false,
            ...
            (
                "hash",
                Arc::new(StringArray::from(vec!["hash1-2", "hash2-2"])) as ArrayRef,
                false,
            ),
            (
                "op",
                Arc::new(StringArray::from(vec!["update", "update"])) as ArrayRef,
                true,
            ),
        ])?;

        let schema = record_batch_1.schema();
        create_table_with_config_map(
            client.clone(),
            table_name,
            [
                (
        ...
        let context =
            create_session_context(&mut LakeSoulIOConfigBuilder::default().build())?;
    ...
    async fn test_streaming_read_without_partition() -> Result<()> {
        ...
        let record_batch_3 = RecordBatch::try_from_iter_with_nullable(vec![
            ...
            (
                "hash",
                Arc::new(Int32Array::from(vec![2, 3, 4, 5])) as ArrayRef,
                false,
            ),
            (
                "value",
                Arc::new(Int32Array::from(vec![222, 333, 444, 555])) as ArrayRef,
                true,
            ),
        ])?;
