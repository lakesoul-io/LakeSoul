...
mod update_tests {
    ...
    async fn execute_append(
        ...
        .unwrap()
        .to_string();
        let builder = create_io_config_builder(client.clone(), Some(table_name))
            .await?
            .with_file(file.clone())
            .with_schema(batch.schema());
        let config = builder.clone().build();

        let writer = SyncSendableMutableLakeSoulWriter::try_new(
            config,
    ...
    async fn perform_update(
        ...
        transaction.table_info = client
            .get_table_info_by_table_name("basic_case", "default")
            .await?;

        // get candidateFiles
        let (metadata_predicates, data_predicates) =
            split_metadata_and_data_predicates()?;
        let candidate_files = transaction.filter_files(
            client.clone(),
            &metadata_predicates,
            &data_predicates,
        )?;
        let path_to_file = candidate_files
            .iter()
            .map(|f| (f.path.clone(), f.clone()))
            .collect::<HashMap<_, _>>();

        let (add_files, expire_files): (Vec<DataFileInfo>, Vec<DataFileInfo>) =
            if candidate_files.is_empty() {
                ...
                let files_to_rewrite = candidate_files
                    .iter()
                ...
                let current = SystemTime::now()
                    ...
                    .unwrap()
                    .as_millis() as u64;
                let mut delete_files = candidate_files
                    .iter()
                    .map(|file| file.clone())
                    .collect::<Vec<_>>();
                delete_files
                    .iter_mut()
                    .for_each(move |file| file.timestamp = current);
                dbg!(&delete_files);
                let rewritten_files =
                    rewrite_files(&mut transaction, files_to_rewrite, path_to_file);
    ...
    fn rewrite_files(
        ...
        input_leaf_files: Vec<String>,
        path_to_file: HashMap<String, DataFileInfo>,
    ) -> Vec<DataFileInfo> {
        let rewrite_file_info = input_leaf_files
            .iter()
    ...
    async fn execute_update(
        ...
        dbg!(&sql);
        let statement = sess_ctx
            .state()
            .sql_to_statement(sql.as_str(), "Generic")
            .unwrap();
