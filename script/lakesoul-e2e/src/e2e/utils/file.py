...
def open_maybe_zipped(fileloc, mode="r"):
    ...
    """
    _, archive, filename = ZIP_REGEX.search(fileloc).groups()  # type: ignore
    if archive and zipfile.is_zipfile(archive):
        return TextIOWrapper(zipfile.ZipFile(archive, mode=mode).open(filename))  # type: ignore
                ...
                patterns += [
                    ...
                    for p in [
                        ignore_rule_type.compile(
                            pattern, Path(base_dir_path), ignore_file_path
                        )
...
def list_py_file_paths(
    directory: str | os.PathLike[str] | None, safe_mode: bool = False
) -> list[str]:
    """
