...
public class SubstraitPlanBuilder {
    ...
    public static io.substrait.proto.Plan buildSubstraitPlan(
            List<FilterPredicate> parFilters,
    ...
    public static Map<String, String> extractEqualityFilters(
            List<FilterPredicate> parFilters, Schema partitionSchema) {
        if (parFilters == null) {
            return null;
        }
            ...
            if (fp instanceof Operators.Eq) {
                ...
                if (value == null) {
                    strValue = LAKESOUL_NULL_STRING;
                        ...
                        && arrowField.getType() instanceof ArrowType.Date) {
    ...
    private static Expression convertToSubstraitExpr(
            FilterPredicate fp, Map<String, ColumnHandle> allColumns) throws Exception {
        if (fp instanceof Operators.Eq) {
            if (((Operators.Eq) fp).getValue() == null) {
                String columnName = ((Operators.Eq) fp).getColumn().getColumnPath().toDotString();
                LakeSoulTableColumnHandle handle =
                        (LakeSoulTableColumnHandle) allColumns.get(columnName);
            ...
            return buildBinaryExpr(
                    ((Operators.Eq) fp).getColumn(),
                    ((Operators.Eq) fp).getValue(),
                    "equal:any_any",
                    allColumns);
        } else if (fp instanceof Operators.NotEq) {
            return buildBinaryExpr(
                    ((Operators.NotEq) fp).getColumn(),
                    ((Operators.NotEq) fp).getValue(),
                    "not_equal:any_any",
                    allColumns);
        } else if (fp instanceof Operators.GtEq) {
            return buildBinaryExpr(
                    ((Operators.GtEq) fp).getColumn(),
                    ((Operators.GtEq) fp).getValue(),
                    "gte:any_any",
                    allColumns);
        } else if (fp instanceof Operators.Lt) {
            return buildBinaryExpr(
                    ((Operators.Lt) fp).getColumn(),
                    ((Operators.Lt) fp).getValue(),
                    "lt:any_any",
                    allColumns);
        } else if (fp instanceof Operators.Gt) {
            return buildBinaryExpr(
                    ((Operators.Gt) fp).getColumn(),
                    ((Operators.Gt) fp).getValue(),
                    "gt:any_any",
                    allColumns);
        } else if (fp instanceof Operators.LtEq) {
            return buildBinaryExpr(
                    ((Operators.LtEq) fp).getColumn(),
                    ((Operators.LtEq) fp).getValue(),
                    "lte:any_any",
                    allColumns);
            ...
            if (innerPredicate instanceof Operators.Eq
                    && ((Operators.Eq) innerPredicate).getValue() == null) { // is not null
                String columnName =
                        ((Operators.Eq) innerPredicate).getColumn().getColumnPath().toDotString();
                ;
                LakeSoulTableColumnHandle handle =
                        (LakeSoulTableColumnHandle) allColumns.get(columnName);
                Field arrowField = handle.getArrowField();
    ...
    private static Expression buildBinaryExpr(
            Operators.Column column,
            Object value,
            String funcKey,
            Map<String, ColumnHandle> allColumns)
            throws Exception {
        ...
        Expression fieldRef = SubstraitUtil.arrowFieldToSubstraitField(arrowField);
        log.info(
                "buildBinaryExpr Successfully built binary expr for columnName = "
                        + columnName
                        + ", fieldRef = "
                        + fieldRef
                        + ", literal = "
        ...
        return SubstraitUtil.makeBinary(
                fieldRef, literal, SubstraitUtil.CompNamespace, funcKey, subType);
    }
}
