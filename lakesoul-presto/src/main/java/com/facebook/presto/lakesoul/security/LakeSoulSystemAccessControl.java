...
public class LakeSoulSystemAccessControl implements SystemAccessControl {
    ...
    private final boolean showSchemasEnableRestrictions;
    private final Set<String> canShowSchemasAllowUsernames;
    private final Set<String> canShowSchemasAllowDomains;
    private final Set<String> systemSchemas;

    private Optional<LakeSoulAuthenticatedPrincipal> principal = Optional.empty();
    ...
    public LakeSoulSystemAccessControl(
            ...
            boolean showSchemasEnableRestrictions,
            Set<String> canShowSchemasAllowUsernames,
            Set<String> canShowSchemasAllowDomains,
        ...
        this.showSchemasEnableRestrictions = showSchemasEnableRestrictions;
        this.canShowSchemasAllowUsernames = canShowSchemasAllowUsernames;
        this.canShowSchemasAllowDomains = canShowSchemasAllowDomains;
    ...
    public void checkCanSetUser(Identity identity, AccessControlContext context, Optional<Principal> principal,
        ...
        if (principal.isPresent()) {
            if (principal.get() instanceof LakeSoulAuthenticatedPrincipal) {
                LakeSoulAuthenticatedPrincipal lakesoulPrincipal = (LakeSoulAuthenticatedPrincipal) principal.get();
                this.principal = Optional.of(lakesoulPrincipal);
            }
        }
        if (principalNeedMatchUsername) {
            if (this.principal.isPresent()) {
                LakeSoulAuthenticatedPrincipal lakesoulPrincipal = this.principal.get();
    ...
    public void checkCanShowSchemas(Identity identity, AccessControlContext context, String catalogName) {
        if (!showSchemasEnableRestrictions) {
            return;
        }
        if (this.principal.isPresent()) {
            LakeSoulAuthenticatedPrincipal lakesoulPrincipal = this.principal.get();
            ...
            if (!lakesoulPrincipal.isExpired()) {
                String username = lakesoulPrincipal.getSub();
                String domain = lakesoulPrincipal.getGroup();
                if (!canShowSchemasAllowUsernames.contains(username)
                        && !canShowSchemasAllowDomains.contains(domain)) {
                    throw new AccessDeniedException(
                            String.format(
                                    "Access denied: principal <group: '%s', sub: '%s'> is not allowed to show schemas in catalog '%s'.",
                                    domain, username, catalogName));
                }
                return;
            }
        }
        if (!canShowSchemasAllowUsernames.contains(identity.getUser())) {
            throw new AccessDeniedException(
                    String.format("Access denied: user '%s' is not allowed to show schemas in catalog '%s'.",
