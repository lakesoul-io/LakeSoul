...
public class LakeSoulSystemAccessControlFactory implements SystemAccessControlFactory {
    ...
    private final String showSchemasEnableRestrictions = "show-schemas.enable-restrictions";
    private final String showSchemasAllowUsernames = "show-schemas.allowed-usernames";
    private final String showSchemasAllowDomains = "show-schemas.allowed-domains";
    ...
    public SystemAccessControl create(Map<String, String> config) {
        ...
        boolean showSchemasEnableRestrictions = Boolean.parseBoolean(
                config.getOrDefault(this.showSchemasEnableRestrictions, "false"));

        Set<String> showSchemasAllowUsernames = Arrays
                .stream(config.getOrDefault(this.showSchemasAllowUsernames, "").split(","))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
                .collect(Collectors.toSet());

        Set<String> showSchemasAllowDomains = Arrays
                .stream(config.getOrDefault(this.showSchemasAllowDomains, "").split(","))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
                .collect(Collectors.toSet());
        ...
        return new LakeSoulSystemAccessControl(
                ...
                showSchemasEnableRestrictions,
                showSchemasAllowUsernames,
                showSchemasAllowDomains,
                systemSchemas);
    }
}