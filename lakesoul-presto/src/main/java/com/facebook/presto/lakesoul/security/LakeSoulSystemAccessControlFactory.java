...
public class LakeSoulSystemAccessControlFactory implements SystemAccessControlFactory {
    ...
    private final String sensitiveSystemSessionPropertiesEnableRestrictions =
            "sensitive.system-session-properties.enable-restrictions";
    private final String sensitiveSystemSessionProperties = "sensitive.system-session-properties";
    private final String sensitiveSystemSessionPropertiesAllowUsername =
            "sensitive.system-session-properties.allowed-usernames";
    private final String sensitiveSystemSessionPropertiesAllowDomain =
            "sensitive.system-session-properties.allowed-domains";
    ...
    public SystemAccessControl create(Map<String, String> config) {
        boolean principalNeedMatchUsername =
                Boolean.parseBoolean(config.getOrDefault(this.principalNeedMatchUsername, "false"));

        boolean sensitiveSystemSessionPropertiesEnableRestrictions =
                Boolean.parseBoolean(
                        config.getOrDefault(
                                this.sensitiveSystemSessionPropertiesEnableRestrictions, "false"));

        Set<String> sensitiveSystemSessionProperties =
                Arrays.stream(
                                config.getOrDefault(this.sensitiveSystemSessionProperties, "")
                                        .split(","))
                        .map(String::trim)
                        .filter(s -> !s.isEmpty())
        ...
        Set<String> sensitiveSystemSessionPropertiesAllowUsernames =
                Arrays.stream(
                                config.getOrDefault(
                                                this.sensitiveSystemSessionPropertiesAllowUsername,
                                                "")
                                        .split(","))
                        .map(String::trim)
                        .filter(s -> !s.isEmpty())
        ...
        Set<String> sensitiveSystemSessionPropertiesAllowDomains =
                Arrays.stream(
                                config.getOrDefault(
                                                this.sensitiveSystemSessionPropertiesAllowDomain,
                                                "")
                                        .split(","))
                        .map(String::trim)
                        .filter(s -> !s.isEmpty())
        ...
        Set<String> systemSchemas =
                Arrays.stream(
                                config.getOrDefault(this.systemSchemas, "information_schema")
                                        .split(","))
                        .map(String::trim)
                        .filter(s -> !s.isEmpty())
                        .collect(Collectors.toSet());
